const _A = "Arn";
const _AA = "AliasArn";
const _AAC = "AttributeAndCondition";
const _AACHGI = "AllowedAccessControlHierarchyGroupId";
const _AACT = "AllowedAccessControlTags";
const _AADS = "AssociateAnalyticsDataSet";
const _AADSR = "AssociateAnalyticsDataSetRequest";
const _AADSRs = "AssociateAnalyticsDataSetResponse";
const _AAMP = "AwaitAnswerMachinePrompt";
const _AAO = "AssociateApprovedOrigin";
const _AAOR = "AssociateApprovedOriginRequest";
const _AAT = "AgentAvailabilityTimer";
const _AAu = "AutoAccept";
const _AB = "AcknowledgedBy";
const _ABAT = "AcceptedByAgentTimestamp";
const _ABR = "AssociateBotRequest";
const _ABs = "AssociateBot";
const _AC = "AuthorizationCode";
const _ACCA = "AssignContactCategoryAction";
const _ACCAD = "AssignContactCategoryActionDefinition";
const _ACL = "AliasConfigurationList";
const _ACR = "AgentContactReference";
const _ACRL = "AgentContactReferenceList";
const _ACS = "AgentContactState";
const _ACSL = "AssociatedContactSummaryList";
const _ACSs = "AssociatedContactSummary";
const _ACWD = "AfterContactWorkDuration";
const _ACWET = "AfterContactWorkEndTimestamp";
const _ACWST = "AfterContactWorkStartTimestamp";
const _ACWTL = "AfterContactWorkTimeLimit";
const _ACWU = "AssociateContactWithUser";
const _ACWUR = "AssociateContactWithUserRequest";
const _ACWURs = "AssociateContactWithUserResponse";
const _ACc = "AcknowledgerComment";
const _ACg = "AgentConfig";
const _ACge = "AgentsCriteria";
const _ACgen = "AgentCriteria";
const _ACl = "AliasConfiguration";
const _ACli = "AliasConfigurations";
const _ACll = "AllowedCapabilities";
const _ACn = "AndConditions";
const _ACnd = "AndCondition";
const _ACt = "AttributeCondition";
const _ACtt = "AttributeConfiguration";
const _AD = "AliasDescription";
const _ADAR = "AnalyticsDataAssociationResult";
const _ADARn = "AnalyticsDataAssociationResults";
const _ADE = "AccessDeniedException";
const _ADSR = "AnalyticsDataSetsResult";
const _ADSRn = "AnalyticsDataSetsResults";
const _ADV = "AssociateDefaultVocabulary";
const _ADVR = "AssociateDefaultVocabularyRequest";
const _ADVRs = "AssociateDefaultVocabularyResponse";
const _ADn = "AnalysisDetails";
const _AE = "AuthenticationError";
const _AEAA = "AssociateEmailAddressAlias";
const _AEAAR = "AssociateEmailAddressAliasRequest";
const _AEAARs = "AssociateEmailAddressAliasResponse";
const _AEC = "AutoEvaluationConfiguration";
const _AED = "AuthenticationErrorDescription";
const _AEDu = "AutoEvaluationDetails";
const _AEE = "AutoEvaluationEnabled";
const _AEF = "ActivateEvaluationForm";
const _AEFR = "ActivateEvaluationFormRequest";
const _AEFRc = "ActivateEvaluationFormResponse";
const _AER = "AdditionalEmailRecipients";
const _AES = "AutoEvaluationStatus";
const _AEn = "AndExpression";
const _AEu = "AutoEvaluation";
const _AF = "AgentFirst";
const _AFC = "AutomaticFailConfiguration";
const _AFE = "AttachedFileError";
const _AFEL = "AttachedFileErrorsList";
const _AFIRER = "AttachedFileInvalidRequestExceptionReason";
const _AFL = "AttachedFilesList";
const _AFN = "AgentFirstName";
const _AFR = "AssociateFlowRequest";
const _AFRT = "AgentFirstResponseTimestamp";
const _AFRTIM = "AgentFirstResponseTimeInMillis";
const _AFRs = "AssociateFlowResponse";
const _AFSQEER = "AttachedFileServiceQuotaExceededExceptionReason";
const _AFU = "AudioFallbackUrl";
const _AFs = "AssociateFlow";
const _AFt = "AttributeFilter";
const _AFtt = "AttachedFile";
const _AFu = "AudioFeatures";
const _AFut = "AutomaticFail";
const _AHG = "AgentHierarchyGroup";
const _AHGg = "AgentHierarchyGroups";
const _AHU = "AudioHostUrl";
const _AI = "AgentInfo";
const _AIHD = "AgentInitiatedHoldDuration";
const _AISC = "AssociateInstanceStorageConfig";
const _AISCR = "AssociateInstanceStorageConfigRequest";
const _AISCRs = "AssociateInstanceStorageConfigResponse";
const _AIg = "AgentIds";
const _AIl = "AllowedIps";
const _AIli = "AliasId";
const _AIs = "AssociationId";
const _AIt = "AttendeeId";
const _AItt = "AttachmentId";
const _AL = "AttributesList";
const _ALB = "AssociateLexBot";
const _ALBR = "AssociateLexBotRequest";
const _ALF = "AssociateLambdaFunction";
const _ALFR = "AssociateLambdaFunctionRequest";
const _ALN = "AgentLastName";
const _AM = "AgentMetrics";
const _AMAQI = "AssociatedManualAssignmentQueueIds";
const _AMC = "AllowedMonitorCapabilities";
const _AMDC = "AnswerMachineDetectionConfig";
const _AMDS = "AnsweringMachineDetectionStatus";
const _AN = "AliasName";
const _ANL = "AvailableNumbersList";
const _ANS = "AvailableNumberSummary";
const _ANt = "AttachmentName";
const _ANtt = "AttributeName";
const _AOCL = "AttributeOrConditionList";
const _AP = "ApplicationPermissions";
const _APDIS = "AgentPauseDurationInSeconds";
const _API = "AuthenticationProfileId";
const _APNCF = "AssociatePhoneNumberContactFlow";
const _APNCFR = "AssociatePhoneNumberContactFlowRequest";
const _APS = "AuthenticationProfileSummary";
const _APSL = "AuthenticationProfileSummaryList";
const _APu = "AuthenticationProfile";
const _AQI = "AssociatedQueueIds";
const _AQM = "AgentQualityMetrics";
const _AQMI = "AudioQualityMetricsInfo";
const _AQQC = "AssociateQueueQuickConnects";
const _AQQCR = "AssociateQueueQuickConnectsRequest";
const _AR = "AttachmentReference";
const _ARA = "AssociatedResourceArn";
const _ARPQ = "AssociateRoutingProfileQueues";
const _ARPQR = "AssociateRoutingProfileQueuesRequest";
const _ARd = "AdditionalRecipients";
const _AS = "ActionSummary";
const _ASA = "AssignSlaAction";
const _ASAD = "AssignSlaActionDefinition";
const _ASARN = "AgentStatusARN";
const _ASBC = "AvailableSlotsByChannel";
const _ASBCc = "ActiveSlotsByChannel";
const _ASI = "AgentStatusId";
const _ASIg = "AgentStatusIdentifier";
const _ASK = "AssociateSecurityKey";
const _ASKR = "AssociateSecurityKeyRequest";
const _ASKRs = "AssociateSecurityKeyResponse";
const _ASL = "AgentStatusList";
const _ASR = "AgentStatusReference";
const _ASS = "AgentStatusSummary";
const _ASSC = "AgentStatusSearchCriteria";
const _ASSCL = "AgentStatusSearchConditionList";
const _ASSF = "AgentStatusSearchFilter";
const _ASSL = "AgentStatusSummaryList";
const _AST = "AgentStatusTypes";
const _ASc = "ActionSummaries";
const _ASg = "AgentStatus";
const _ASge = "AgentStatuses";
const _ASn = "AnswerSource";
const _AT = "ActionType";
const _ATC = "ApproximateTotalCount";
const _ATDGU = "AssociateTrafficDistributionGroupUser";
const _ATDGUR = "AssociateTrafficDistributionGroupUserRequest";
const _ATDGURs = "AssociateTrafficDistributionGroupUserResponse";
const _ATE = "AccessTokenExpiration";
const _ATR = "AdditionalTimeRange";
const _ATb = "AbsoluteTime";
const _ATc = "AccessToken";
const _ATck = "AcknowledgedTime";
const _ATct = "ActivationTimestamp";
const _ATn = "AnalysisType";
const _ATt = "AttributeType";
const _AUA = "AllowedUserActions";
const _AUP = "AssociateUserProficiencies";
const _AUPR = "AssociateUserProficienciesRequest";
const _AV = "ActiveVersion";
const _AVt = "AttributeValue";
const _AWSIA = "AWSIdentityArn";
const _Ac = "Actions";
const _Ack = "Acknowledgement";
const _Act = "Action";
const _Ad = "Address";
const _Ag = "Agent";
const _Age = "Agents";
const _An = "Answers";
const _Ap = "Application";
const _App = "Applications";
const _At = "Attendee";
const _Att = "Attribute";
const _Atta = "Attachments";
const _Attac = "Attachment";
const _Attr = "Attributes";
const _Au = "Audio";
const _Aut = "Automation";
const _B = "Body";
const _BAADS = "BatchAssociateAnalyticsDataSet";
const _BAADSR = "BatchAssociateAnalyticsDataSetRequest";
const _BAADSRa = "BatchAssociateAnalyticsDataSetResponse";
const _BC = "BooleanCondition";
const _BDADS = "BatchDisassociateAnalyticsDataSet";
const _BDADSR = "BatchDisassociateAnalyticsDataSetRequest";
const _BDADSRa = "BatchDisassociateAnalyticsDataSetResponse";
const _BGAFM = "BatchGetAttachedFileMetadata";
const _BGAFMR = "BatchGetAttachedFileMetadataRequest";
const _BGAFMRa = "BatchGetAttachedFileMetadataResponse";
const _BGFA = "BatchGetFlowAssociation";
const _BGFAR = "BatchGetFlowAssociationRequest";
const _BGFARa = "BatchGetFlowAssociationResponse";
const _BI = "BlockedIps";
const _BN = "BotName";
const _BNu = "BucketName";
const _BOC = "BeginOffsetChar";
const _BOM = "BeginOffsetMillis";
const _BP = "BucketPrefix";
const _BPC = "BatchPutContact";
const _BPCR = "BatchPutContactRequest";
const _BPCRa = "BatchPutContactResponse";
const _BT = "BehaviorType";
const _BV = "BooleanValue";
const _C = "Channel";
const _CA = "ContactArn";
const _CAAC = "CommonAttributeAndCondition";
const _CAFU = "CompleteAttachedFileUpload";
const _CAFUR = "CompleteAttachedFileUploadRequest";
const _CAFURo = "CompleteAttachedFileUploadResponse";
const _CAI = "ContactAssociationId";
const _CAIo = "ContactAgentId";
const _CAOCL = "CommonAttributeOrConditionList";
const _CAS = "CreateAgentStatus";
const _CASR = "CreateAgentStatusRequest";
const _CASRr = "CreateAgentStatusResponse";
const _CAc = "CcAddresses";
const _CAo = "ContactAnalysis";
const _CAon = "ConversationAbandon";
const _CAu = "CustomAttributes";
const _CB = "CreatedBy";
const _CBI = "CreatedByInfo";
const _CC = "ContactConfiguration";
const _CCA = "CreateCaseAction";
const _CCAD = "CreateCaseActionDefinition";
const _CCB = "CrossChannelBehavior";
const _CCF = "CreateContactFlow";
const _CCFM = "CreateContactFlowModule";
const _CCFMA = "CreateContactFlowModuleAlias";
const _CCFMAR = "CreateContactFlowModuleAliasRequest";
const _CCFMARr = "CreateContactFlowModuleAliasResponse";
const _CCFMR = "CreateContactFlowModuleRequest";
const _CCFMRr = "CreateContactFlowModuleResponse";
const _CCFMV = "CreateContactFlowModuleVersion";
const _CCFMVR = "CreateContactFlowModuleVersionRequest";
const _CCFMVRr = "CreateContactFlowModuleVersionResponse";
const _CCFR = "CreateContactFlowRequest";
const _CCFRr = "CreateContactFlowResponse";
const _CCFV = "CreateContactFlowVersion";
const _CCFVR = "CreateContactFlowVersionRequest";
const _CCFVRr = "CreateContactFlowVersionResponse";
const _CCM = "ChatContactMetrics";
const _CCR = "CreateContactRequest";
const _CCRr = "CreateContactResponse";
const _CCTIM = "ConversationCloseTimeInMillis";
const _CCh = "ChannelConfiguration";
const _CCr = "CreateContact";
const _CD = "ConnectionData";
const _CDIM = "ChatDurationInMinutes";
const _CDR = "ContactDataRequest";
const _CDRL = "ContactDataRequestList";
const _CDo = "ContactDetails";
const _CE = "ChatEvent";
const _CEA = "CcEmailAddresses";
const _CEAR = "CreateEmailAddressRequest";
const _CEARr = "CreateEmailAddressResponse";
const _CEAr = "CreateEmailAddress";
const _CEF = "CreateEvaluationForm";
const _CEFR = "CreateEvaluationFormRequest";
const _CEFRr = "CreateEvaluationFormResponse";
const _CEo = "ConflictException";
const _CEon = "ContactEvaluations";
const _CEont = "ContactEvaluation";
const _CEu = "CustomerEndpoint";
const _CF = "ContactFilter";
const _CFA = "ContactFlowArn";
const _CFCI = "ContinuedFromContactId";
const _CFI = "ContactFlowId";
const _CFM = "ContactFlowModule";
const _CFMA = "ContactFlowModuleArn";
const _CFMAI = "ContactFlowModuleAliasInfo";
const _CFMAS = "ContactFlowModuleAliasSummary";
const _CFMASL = "ContactFlowModuleAliasSummaryList";
const _CFMAo = "ContactFlowModuleAlias";
const _CFMI = "ContactFlowModuleId";
const _CFMS = "ContactFlowModuleSummary";
const _CFMSC = "ContactFlowModuleSearchCriteria";
const _CFMSCL = "ContactFlowModuleSearchConditionList";
const _CFMSF = "ContactFlowModuleSearchFilter";
const _CFMSL = "ContactFlowModulesSummaryList";
const _CFMSSL = "ContactFlowModuleSearchSummaryList";
const _CFMSo = "ContactFlowModuleState";
const _CFMV = "ContactFlowModuleVersion";
const _CFMVS = "ContactFlowModuleVersionSummary";
const _CFMVSL = "ContactFlowModuleVersionSummaryList";
const _CFMo = "ContactFlowModules";
const _CFNPE = "ContactFlowNotPublishedException";
const _CFS = "ContactFlowSummary";
const _CFSC = "ContactFlowSearchCriteria";
const _CFSCL = "ContactFlowSearchConditionList";
const _CFSF = "ContactFlowSearchFilter";
const _CFSL = "ContactFlowSummaryList";
const _CFSSL = "ContactFlowSearchSummaryList";
const _CFSo = "ContactFlowState";
const _CFSon = "ContactFlowStatus";
const _CFT = "ContactFlowType";
const _CFTo = "ContactFlowTypes";
const _CFV = "ContactFlowVersion";
const _CFVS = "ContactFlowVersionSummary";
const _CFVSL = "ContactFlowVersionSummaryList";
const _CFo = "ContactFlow";
const _CFon = "ContactFlows";
const _CHOO = "CreateHoursOfOperation";
const _CHOOO = "CreateHoursOfOperationOverride";
const _CHOOOR = "CreateHoursOfOperationOverrideRequest";
const _CHOOORr = "CreateHoursOfOperationOverrideResponse";
const _CHOOR = "CreateHoursOfOperationRequest";
const _CHOORr = "CreateHoursOfOperationResponse";
const _CI = "ContactId";
const _CIA = "CreateIntegrationAssociation";
const _CIAR = "CreateIntegrationAssociationRequest";
const _CIARr = "CreateIntegrationAssociationResponse";
const _CINE = "CustomerIdNonEmpty";
const _CIR = "CreateInstanceRequest";
const _CIRr = "CreateInstanceResponse";
const _CIa = "CampaignId";
const _CIr = "CreateInstance";
const _CIu = "CustomerId";
const _CL = "CcList";
const _CLo = "ContactLens";
const _CM = "ChatMessage";
const _CMD = "CurrentMetricData";
const _CMDC = "CurrentMetricDataCollections";
const _CMI = "ContactMetricInfo";
const _CMR = "ContactMetricResult";
const _CMRo = "ContactMetricResults";
const _CMRu = "CurrentMetricResult";
const _CMRur = "CurrentMetricResults";
const _CMSC = "CurrentMetricSortCriteria";
const _CMSCMO = "CurrentMetricSortCriteriaMaxOne";
const _CMV = "ContactMetricValue";
const _CMh = "ChatMetrics";
const _CMo = "ContactMetrics";
const _CMu = "CustomerMetrics";
const _CMur = "CurrentMetric";
const _CMurr = "CurrentMetrics";
const _CNFE = "ContactNotFoundException";
const _CNV = "CreateNewVersion";
const _CO = "ComparisonOperator";
const _COFE = "ConditionalOperationFailedException";
const _COh = "CharacterOffsets";
const _CP = "CreateParticipant";
const _CPA = "CustomerProfileAttributes";
const _CPAF = "ControlPlaneAttributeFilter";
const _CPAR = "CreatePredefinedAttributeRequest";
const _CPAr = "CreatePredefinedAttribute";
const _CPCA = "CreatePersistentContactAssociation";
const _CPCAR = "CreatePersistentContactAssociationRequest";
const _CPCARr = "CreatePersistentContactAssociationResponse";
const _CPN = "ClaimPhoneNumber";
const _CPNR = "ClaimPhoneNumberRequest";
const _CPNRR = "CreatePushNotificationRegistrationRequest";
const _CPNRRr = "CreatePushNotificationRegistrationResponse";
const _CPNRl = "ClaimPhoneNumberResponse";
const _CPNRr = "CreatePushNotificationRegistration";
const _CPNS = "ClaimedPhoneNumberSummary";
const _CPR = "CreateParticipantRequest";
const _CPRC = "ChatParticipantRoleConfig";
const _CPRr = "CreateParticipantResponse";
const _CPRre = "CreatePromptRequest";
const _CPRrea = "CreatePromptResponse";
const _CPTF = "ControlPlaneTagFilter";
const _CPUAF = "ControlPlaneUserAttributeFilter";
const _CPr = "CreatePrompt";
const _CQ = "CreateQueue";
const _CQC = "CreateQuickConnect";
const _CQCR = "CreateQuickConnectRequest";
const _CQCRr = "CreateQuickConnectResponse";
const _CQM = "CustomerQualityMetrics";
const _CQR = "CreateQueueRequest";
const _CQRr = "CreateQueueResponse";
const _CR = "ContactReferences";
const _CRP = "CreateRoutingProfile";
const _CRPR = "CreateRoutingProfileRequest";
const _CRPRr = "CreateRoutingProfileResponse";
const _CRR = "CreateRuleRequest";
const _CRRr = "CreateRuleResponse";
const _CRT = "ContactRecordingType";
const _CRr = "CreateRule";
const _CS = "ContactStates";
const _CSC = "CaseSlaConfiguration";
const _CSCh = "ChatStreamingConfiguration";
const _CSI = "CalibrationSessionId";
const _CSL = "ContactSummaryList";
const _CSP = "CreateSecurityProfile";
const _CSPR = "CreateSecurityProfileRequest";
const _CSPRr = "CreateSecurityProfileResponse";
const _CSS = "ContactSearchSummary";
const _CSSAI = "ContactSearchSummaryAgentInfo";
const _CSSQI = "ContactSearchSummaryQueueInfo";
const _CSSSA = "ContactSearchSummarySegmentAttributes";
const _CSSSAV = "ContactSearchSummarySegmentAttributeValue";
const _CT = "ClientToken";
const _CTAT = "ConnectedToAgentTimestamp";
const _CTC = "ConversationTurnCount";
const _CTDG = "CreateTrafficDistributionGroup";
const _CTDGR = "CreateTrafficDistributionGroupRequest";
const _CTDGRr = "CreateTrafficDistributionGroupResponse";
const _CTST = "ConnectedToSystemTimestamp";
const _CTT = "CreateTaskTemplate";
const _CTTR = "CreateTaskTemplateRequest";
const _CTTRr = "CreateTaskTemplateResponse";
const _CTo = "ComparisonType";
const _CTon = "ContentType";
const _CTond = "ConditionType";
const _CTr = "CreationTime";
const _CTre = "CreatedTime";
const _CTrea = "CreateTimestamp";
const _CU = "CreateUser";
const _CUA = "ConnectUserArn";
const _CUC = "CreateUseCase";
const _CUCR = "CreateUseCaseRequest";
const _CUCRr = "CreateUseCaseResponse";
const _CUHG = "CreateUserHierarchyGroup";
const _CUHGR = "CreateUserHierarchyGroupRequest";
const _CUHGRr = "CreateUserHierarchyGroupResponse";
const _CUR = "CreateUserRequest";
const _CURr = "CreateUserResponse";
const _CV = "CreateView";
const _CVA = "CustomerVoiceActivity";
const _CVR = "CreateViewRequest";
const _CVRr = "CreateViewResponse";
const _CVRre = "CreateVocabularyRequest";
const _CVRrea = "CreateVocabularyResponse";
const _CVV = "CreateViewVersion";
const _CVVR = "CreateViewVersionRequest";
const _CVVRr = "CreateViewVersionResponse";
const _CVr = "CreateVocabulary";
const _Ca = "Capabilities";
const _Cam = "Campaign";
const _Cat = "Category";
const _Cate = "Categories";
const _Ch = "Channels";
const _Cha = "Chat";
const _Co = "Content";
const _Cod = "Code";
const _Col = "Collections";
const _Com = "Comparator";
const _Comp = "Comparison";
const _Con = "Condition";
const _Conc = "Concurrency";
const _Cond = "Conditions";
const _Conf = "Config";
const _Cons = "Constraints";
const _Cont = "Contact";
const _Conta = "Contacts";
const _Cr = "Created";
const _Cre = "Credentials";
const _Cri = "Criteria";
const _Cu = "Customer";
const _D = "Description";
const _DA = "DirectoryAlias";
const _DADS = "DisassociateAnalyticsDataSet";
const _DADSR = "DisassociateAnalyticsDataSetRequest";
const _DAF = "DeleteAttachedFile";
const _DAFR = "DeleteAttachedFileRequest";
const _DAFRe = "DeleteAttachedFileResponse";
const _DAO = "DisassociateApprovedOrigin";
const _DAOR = "DisassociateApprovedOriginRequest";
const _DAP = "DescribeAuthenticationProfile";
const _DAPR = "DescribeAuthenticationProfileRequest";
const _DAPRe = "DescribeAuthenticationProfileResponse";
const _DAS = "DescribeAgentStatus";
const _DASR = "DescribeAgentStatusRequest";
const _DASRe = "DescribeAgentStatusResponse";
const _DAe = "DefaultAction";
const _DAi = "DisplayAs";
const _DB = "DisassociateBot";
const _DBR = "DisassociateBotRequest";
const _DC = "DateCondition";
const _DCE = "DeleteContactEvaluation";
const _DCER = "DeleteContactEvaluationRequest";
const _DCERe = "DescribeContactEvaluationRequest";
const _DCERes = "DescribeContactEvaluationResponse";
const _DCEe = "DescribeContactEvaluation";
const _DCF = "DeleteContactFlow";
const _DCFM = "DeleteContactFlowModule";
const _DCFMA = "DeleteContactFlowModuleAlias";
const _DCFMAR = "DeleteContactFlowModuleAliasRequest";
const _DCFMARe = "DeleteContactFlowModuleAliasResponse";
const _DCFMARes = "DescribeContactFlowModuleAliasRequest";
const _DCFMAResc = "DescribeContactFlowModuleAliasResponse";
const _DCFMAe = "DescribeContactFlowModuleAlias";
const _DCFMR = "DeleteContactFlowModuleRequest";
const _DCFMRe = "DeleteContactFlowModuleResponse";
const _DCFMRes = "DescribeContactFlowModuleRequest";
const _DCFMResc = "DescribeContactFlowModuleResponse";
const _DCFMV = "DeleteContactFlowModuleVersion";
const _DCFMVR = "DeleteContactFlowModuleVersionRequest";
const _DCFMVRe = "DeleteContactFlowModuleVersionResponse";
const _DCFMe = "DescribeContactFlowModule";
const _DCFR = "DeleteContactFlowRequest";
const _DCFRe = "DeleteContactFlowResponse";
const _DCFRes = "DescribeContactFlowRequest";
const _DCFResc = "DescribeContactFlowResponse";
const _DCFV = "DeleteContactFlowVersion";
const _DCFVR = "DeleteContactFlowVersionRequest";
const _DCFVRe = "DeleteContactFlowVersionResponse";
const _DCFe = "DescribeContactFlow";
const _DCR = "DescribeContactRequest";
const _DCRe = "DescribeContactResponse";
const _DCe = "DecimalCondition";
const _DCes = "DescribeContact";
const _DD = "DisconnectDetails";
const _DE = "DestinationEndpoint";
const _DEA = "DestinationEmailAddress";
const _DEAA = "DisassociateEmailAddressAlias";
const _DEAAR = "DisassociateEmailAddressAliasRequest";
const _DEAARi = "DisassociateEmailAddressAliasResponse";
const _DEAR = "DeleteEmailAddressRequest";
const _DEARe = "DeleteEmailAddressResponse";
const _DEARes = "DescribeEmailAddressRequest";
const _DEAResc = "DescribeEmailAddressResponse";
const _DEAe = "DeleteEmailAddress";
const _DEAes = "DescribeEmailAddress";
const _DEF = "DeactivateEvaluationForm";
const _DEFR = "DeactivateEvaluationFormRequest";
const _DEFRe = "DeactivateEvaluationFormResponse";
const _DEFRel = "DeleteEvaluationFormRequest";
const _DEFRes = "DescribeEvaluationFormRequest";
const _DEFResc = "DescribeEvaluationFormResponse";
const _DEFe = "DeleteEvaluationForm";
const _DEFes = "DescribeEvaluationForm";
const _DF = "DisassociateFlow";
const _DFR = "DisassociateFlowRequest";
const _DFRi = "DisassociateFlowResponse";
const _DFV = "DefaultFieldValues";
const _DHOO = "DeleteHoursOfOperation";
const _DHOOO = "DeleteHoursOfOperationOverride";
const _DHOOOR = "DeleteHoursOfOperationOverrideRequest";
const _DHOOORe = "DescribeHoursOfOperationOverrideRequest";
const _DHOOORes = "DescribeHoursOfOperationOverrideResponse";
const _DHOOOe = "DescribeHoursOfOperationOverride";
const _DHOOR = "DeleteHoursOfOperationRequest";
const _DHOORe = "DescribeHoursOfOperationRequest";
const _DHOORes = "DescribeHoursOfOperationResponse";
const _DHOOe = "DescribeHoursOfOperation";
const _DI = "DeviceInfo";
const _DIA = "DeleteIntegrationAssociation";
const _DIAR = "DeleteIntegrationAssociationRequest";
const _DIARe = "DescribeInstanceAttributeRequest";
const _DIARes = "DescribeInstanceAttributeResponse";
const _DIAe = "DescribeInstanceAttribute";
const _DIR = "DeleteInstanceRequest";
const _DIRe = "DescribeInstanceRequest";
const _DIRes = "DescribeInstanceResponse";
const _DIS = "DurationInSeconds";
const _DISC = "DescribeInstanceStorageConfig";
const _DISCR = "DescribeInstanceStorageConfigRequest";
const _DISCRe = "DescribeInstanceStorageConfigResponse";
const _DISCRi = "DisassociateInstanceStorageConfigRequest";
const _DISCi = "DisassociateInstanceStorageConfig";
const _DIe = "DestinationId";
const _DIel = "DeleteInstance";
const _DIes = "DescribeInstance";
const _DIi = "DirectoryId";
const _DL = "DistributionList";
const _DLB = "DisassociateLexBot";
const _DLBR = "DisassociateLexBotRequest";
const _DLF = "DisassociateLambdaFunction";
const _DLFR = "DisassociateLambdaFunctionRequest";
const _DM = "DeliveryMethod";
const _DN = "DisplayName";
const _DNAE = "DestinationNotAllowedException";
const _DO = "DisplayOrder";
const _DOQI = "DefaultOutboundQueueId";
const _DORI = "DefaultOptionRefId";
const _DP = "DeletePrompt";
const _DPA = "DeletePredefinedAttribute";
const _DPAR = "DeletePredefinedAttributeRequest";
const _DPARe = "DescribePredefinedAttributeRequest";
const _DPARes = "DescribePredefinedAttributeResponse";
const _DPAe = "DescribePredefinedAttribute";
const _DPN = "DestinationPhoneNumber";
const _DPNCF = "DisassociatePhoneNumberContactFlow";
const _DPNCFR = "DisassociatePhoneNumberContactFlowRequest";
const _DPNR = "DescribePhoneNumberRequest";
const _DPNRR = "DeletePushNotificationRegistrationRequest";
const _DPNRRe = "DeletePushNotificationRegistrationResponse";
const _DPNRe = "DescribePhoneNumberResponse";
const _DPNRel = "DeletePushNotificationRegistration";
const _DPNe = "DeskPhoneNumber";
const _DPNes = "DescribePhoneNumber";
const _DPR = "DeletePromptRequest";
const _DPRe = "DescribePromptRequest";
const _DPRes = "DescribePromptResponse";
const _DPe = "DescribePrompt";
const _DQ = "DeleteQueue";
const _DQC = "DeleteQuickConnect";
const _DQCR = "DeleteQuickConnectRequest";
const _DQCRe = "DescribeQuickConnectRequest";
const _DQCRes = "DescribeQuickConnectResponse";
const _DQCe = "DescribeQuickConnect";
const _DQQC = "DisassociateQueueQuickConnects";
const _DQQCR = "DisassociateQueueQuickConnectsRequest";
const _DQR = "DeleteQueueRequest";
const _DQRe = "DescribeQueueRequest";
const _DQRes = "DescribeQueueResponse";
const _DQe = "DescribeQueue";
const _DR = "DisconnectReason";
const _DRE = "DuplicateResourceException";
const _DRP = "DeleteRoutingProfile";
const _DRPQ = "DisassociateRoutingProfileQueues";
const _DRPQR = "DisassociateRoutingProfileQueuesRequest";
const _DRPR = "DeleteRoutingProfileRequest";
const _DRPRe = "DescribeRoutingProfileRequest";
const _DRPRes = "DescribeRoutingProfileResponse";
const _DRPe = "DescribeRoutingProfile";
const _DRR = "DeleteRuleRequest";
const _DRRe = "DescribeRuleRequest";
const _DRRes = "DescribeRuleResponse";
const _DRa = "DateReference";
const _DRe = "DeletionReason";
const _DRel = "DeleteRule";
const _DRes = "DescribeRule";
const _DSI = "DataSetId";
const _DSIa = "DataSetIds";
const _DSK = "DisassociateSecurityKey";
const _DSKR = "DisassociateSecurityKeyRequest";
const _DSN = "DataSetName";
const _DSP = "DeleteSecurityProfile";
const _DSPR = "DeleteSecurityProfileRequest";
const _DSPRe = "DescribeSecurityProfileRequest";
const _DSPRes = "DescribeSecurityProfileResponse";
const _DSPe = "DescribeSecurityProfile";
const _DST = "DataSnapshotTime";
const _DT = "DisconnectTimestamp";
const _DTC = "DateTimeCondition";
const _DTDG = "DeleteTrafficDistributionGroup";
const _DTDGR = "DeleteTrafficDistributionGroupRequest";
const _DTDGRe = "DeleteTrafficDistributionGroupResponse";
const _DTDGRes = "DescribeTrafficDistributionGroupRequest";
const _DTDGResc = "DescribeTrafficDistributionGroupResponse";
const _DTDGU = "DisassociateTrafficDistributionGroupUser";
const _DTDGUR = "DisassociateTrafficDistributionGroupUserRequest";
const _DTDGURi = "DisassociateTrafficDistributionGroupUserResponse";
const _DTDGe = "DescribeTrafficDistributionGroup";
const _DTT = "DeleteTaskTemplate";
const _DTTR = "DeleteTaskTemplateRequest";
const _DTTRe = "DeleteTaskTemplateResponse";
const _DTe = "DeleteTimestamp";
const _DTev = "DeviceToken";
const _DTevi = "DeviceType";
const _DU = "DeleteUser";
const _DUC = "DeleteUseCase";
const _DUCR = "DeleteUseCaseRequest";
const _DUCRi = "DismissUserContactRequest";
const _DUCRis = "DismissUserContactResponse";
const _DUCi = "DismissUserContact";
const _DUHG = "DeleteUserHierarchyGroup";
const _DUHGR = "DeleteUserHierarchyGroupRequest";
const _DUHGRe = "DescribeUserHierarchyGroupRequest";
const _DUHGRes = "DescribeUserHierarchyGroupResponse";
const _DUHGe = "DescribeUserHierarchyGroup";
const _DUHS = "DescribeUserHierarchyStructure";
const _DUHSR = "DescribeUserHierarchyStructureRequest";
const _DUHSRe = "DescribeUserHierarchyStructureResponse";
const _DUI = "DirectoryUserId";
const _DUM = "DownloadUrlMetadata";
const _DUP = "DisassociateUserProficiencies";
const _DUPR = "DisassociateUserProficienciesRequest";
const _DUR = "DeleteUserRequest";
const _DURe = "DescribeUserRequest";
const _DURes = "DescribeUserResponse";
const _DUe = "DescribeUser";
const _DV = "DefaultVocabulary";
const _DVL = "DefaultVocabularyList";
const _DVR = "DeleteViewRequest";
const _DVRe = "DeleteViewResponse";
const _DVRel = "DeleteVocabularyRequest";
const _DVRele = "DeleteVocabularyResponse";
const _DVRes = "DescribeViewRequest";
const _DVResc = "DescribeViewResponse";
const _DVRescr = "DescribeVocabularyRequest";
const _DVRescri = "DescribeVocabularyResponse";
const _DVV = "DeleteViewVersion";
const _DVVR = "DeleteViewVersionRequest";
const _DVVRe = "DeleteViewVersionResponse";
const _DVe = "DefaultValue";
const _DVel = "DeleteView";
const _DVele = "DeleteVocabulary";
const _DVes = "DescribeView";
const _DVesc = "DescribeVocabulary";
const _DVo = "DoubleValue";
const _Da = "Date";
const _Day = "Day";
const _De = "Deleted";
const _Def = "Defaults";
const _Del = "Delay";
const _Di = "Distributions";
const _Dim = "Dimensions";
const _Dis = "Distribution";
const _E = "Email";
const _EA = "EmailAddress";
const _EAA = "EmailAddressArn";
const _EAD = "EvaluationAnswerData";
const _EADN = "EmailAddressDisplayName";
const _EAI = "EmailAddressId";
const _EAIM = "EvaluationAnswersInputMap";
const _EAIm = "EmailAddressInfo";
const _EAIv = "EvaluationAnswerInput";
const _EAL = "EmailAddressList";
const _EAM = "EmailAddressMetadata";
const _EAMD = "EnableAnswerMachineDetection";
const _EAO = "EvaluationAnswerOutput";
const _EAOM = "EvaluationAnswersOutputMap";
const _EARC = "EvaluationAutomationRuleCategory";
const _EARCL = "EvaluationAutomationRuleCategoryList";
const _EARL = "EmailAddressRecipientList";
const _EAS = "EvaluationAcknowledgementSummary";
const _EASC = "EmailAddressSearchCriteria";
const _EASCL = "EmailAddressSearchConditionList";
const _EASF = "EmailAddressSearchFilter";
const _EATA = "EndAssociatedTasksAction";
const _EATAD = "EndAssociatedTasksActionDefinition";
const _EAm = "EmailAttachment";
const _EAma = "EmailAddresses";
const _EAmai = "EmailAttachments";
const _EAv = "EvaluationArn";
const _EAva = "EvaluationAcknowledgement";
const _EAval = "EvaluatorArn";
const _EBA = "EventBridgeAction";
const _EBAD = "EventBridgeActionDefinition";
const _EC = "ErrorCode";
const _ECLAAD = "EvaluationContactLensAnswerAnalysisDetails";
const _ECn = "EncryptionConfig";
const _ED = "ErrorDescription";
const _EDIM = "ExpiryDurationInMinutes";
const _EF = "EffectiveFrom";
const _EFA = "EvaluationFormArn";
const _EFAEC = "EvaluationFormAutoEvaluationConfiguration";
const _EFC = "EvaluationFormContent";
const _EFI = "EvaluationFormId";
const _EFIEC = "EvaluationFormItemEnablementCondition";
const _EFIECO = "EvaluationFormItemEnablementConditionOperand";
const _EFIECOL = "EvaluationFormItemEnablementConditionOperandList";
const _EFIECv = "EvaluationFormItemEnablementConfiguration";
const _EFIEE = "EvaluationFormItemEnablementExpression";
const _EFIES = "EvaluationFormItemEnablementSource";
const _EFIESV = "EvaluationFormItemEnablementSourceValue";
const _EFIESVL = "EvaluationFormItemEnablementSourceValueList";
const _EFIL = "EvaluationFormItemsList";
const _EFIv = "EvaluationFormItem";
const _EFNQA = "EvaluationFormNumericQuestionAutomation";
const _EFNQO = "EvaluationFormNumericQuestionOption";
const _EFNQOL = "EvaluationFormNumericQuestionOptionList";
const _EFNQP = "EvaluationFormNumericQuestionProperties";
const _EFQ = "EvaluationFormQuestion";
const _EFQAAS = "EvaluationFormQuestionAutomationAnswerSource";
const _EFQTP = "EvaluationFormQuestionTypeProperties";
const _EFS = "EvaluationFormSection";
const _EFSC = "EvaluationFormSearchCriteria";
const _EFSCL = "EvaluationFormSearchConditionList";
const _EFSF = "EvaluationFormSearchFilter";
const _EFSL = "EvaluationFormSummaryList";
const _EFSS = "EvaluationFormScoringStrategy";
const _EFSSL = "EvaluationFormSearchSummaryList";
const _EFSSQA = "EvaluationFormSingleSelectQuestionAutomation";
const _EFSSQAO = "EvaluationFormSingleSelectQuestionAutomationOption";
const _EFSSQAOL = "EvaluationFormSingleSelectQuestionAutomationOptionList";
const _EFSSQO = "EvaluationFormSingleSelectQuestionOption";
const _EFSSQOL = "EvaluationFormSingleSelectQuestionOptionList";
const _EFSSQP = "EvaluationFormSingleSelectQuestionProperties";
const _EFSSv = "EvaluationFormSearchSummary";
const _EFSv = "EvaluationFormSummary";
const _EFT = "EvaluationFormTitle";
const _EFTQA = "EvaluationFormTextQuestionAutomation";
const _EFTQP = "EvaluationFormTextQuestionProperties";
const _EFV = "EvaluationFormVersion";
const _EFVS = "EvaluationFormVersionSummary";
const _EFVSL = "EvaluationFormVersionSummaryList";
const _EFVm = "EmptyFieldValue";
const _EFv = "EvaluationForm";
const _EGAIAAD = "EvaluationGenAIAnswerAnalysisDetails";
const _EHOO = "EffectiveHoursOfOperations";
const _EHOOL = "EffectiveHoursOfOperationList";
const _EI = "EvaluationId";
const _EIC = "ExternalInvocationConfiguration";
const _EIU = "EventIngestionUrl";
const _EIn = "EndpointInfo";
const _EL = "ExportLocation";
const _EM = "ErrorMessage";
const _EMR = "EmailMessageReference";
const _EMm = "EmailMessage";
const _EMv = "EvaluationMetadata";
const _EN = "EvaluationNote";
const _ENM = "EvaluationNotesMap";
const _EOC = "EndOffsetChar";
const _EQAAD = "EvaluationQuestionAnswerAnalysisDetails";
const _EQID = "EvaluationQuestionInputDetails";
const _ER = "EchoReduction";
const _ERL = "EmailRecipientsList";
const _ERm = "EmailRecipient";
const _ERma = "EmailReference";
const _ERr = "ErrorResult";
const _ERrr = "ErrorResults";
const _ES = "EvaluationScore";
const _ESA = "EvaluationSuggestedAnswer";
const _ESAL = "EvaluationSuggestedAnswersList";
const _ESATMO = "EvaluationSuggestedAnswerTranscriptMillisecondOffsets";
const _ESC = "EvaluationSearchCriteria";
const _ESCL = "EvaluationSearchConditionList";
const _ESF = "EvaluationSearchFilter";
const _ESL = "EvaluationSummaryList";
const _ESM = "EvaluationSearchMetadata";
const _ESMv = "EvaluationScoresMap";
const _ESN = "EventSourceName";
const _ESS = "EvaluationSearchSummary";
const _ESSL = "EvaluationSearchSummaryList";
const _ESv = "EvaluationSummary";
const _ET = "EndTimestamp";
const _ETPOI = "EvaluationTranscriptPointOfInterest";
const _ETPOIv = "EvaluationTranscriptPointsOfInterest";
const _ETf = "EffectiveTill";
const _ETn = "EnqueueTimestamp";
const _ETnc = "EncryptionType";
const _ETnd = "EndTime";
const _ETv = "EvaluationType";
const _ETve = "EventType";
const _ETx = "ExpiryTimestamp";
const _EUU = "EvaluatorUserUnion";
const _EV = "EmptyValue";
const _EVVOA = "EnableValueValidationOnAssociation";
const _En = "Enabled";
const _Ena = "Enablement";
const _End = "Endpoint";
const _End_ = "End";
const _Er = "Errors";
const _Err = "Error";
const _Ev = "Evaluation";
const _Eve = "Event";
const _Ex = "Expiry";
const _Exc = "Exclusion";
const _Exp = "Expression";
const _Expr = "Expressions";
const _F = "Files";
const _FA = "FunctionArn";
const _FAS = "FlowAssociationSummary";
const _FASL = "FlowAssociationSummaryList";
const _FAi = "FileArn";
const _FAir = "FirehoseArn";
const _FC = "FailureCode";
const _FCS = "FlowContentSha256";
const _FD = "FromDate";
const _FEA = "FromEmailAddress";
const _FI = "FlowId";
const _FIi = "FileId";
const _FIie = "FieldId";
const _FIil = "FileIds";
const _FIo = "FormId";
const _FK = "FilterKey";
const _FMCS = "FlowModuleContentSha256";
const _FN = "FileName";
const _FNi = "FieldName";
const _FNir = "FirstName";
const _FR = "FailedRequest";
const _FRC = "FailureReasonCode";
const _FRL = "FailedRequestList";
const _FRM = "FailureReasonMessage";
const _FRa = "FailureReason";
const _FS = "FileStatus";
const _FSIB = "FileSizeInBytes";
const _FSN = "FragmentStartNumber";
const _FSNr = "FragmentStopNumber";
const _FUCT = "FileUseCaseType";
const _FV = "FieldValue";
const _FVL = "FiltersV2List";
const _FVU = "FieldValueUnion";
const _FVi = "FilterV2";
const _FVie = "FieldValues";
const _FVil = "FilterValues";
const _Fi = "Fields";
const _Fil = "Filters";
const _Fu = "Function";
const _G = "Groupings";
const _GAF = "GetAttachedFile";
const _GAFR = "GetAttachedFileRequest";
const _GAFRe = "GetAttachedFileResponse";
const _GAI = "GenAI";
const _GCA = "GetContactAttributes";
const _GCAR = "GetContactAttributesRequest";
const _GCARe = "GetContactAttributesResponse";
const _GCM = "GetContactMetrics";
const _GCMD = "GetCurrentMetricData";
const _GCMDR = "GetCurrentMetricDataRequest";
const _GCMDRe = "GetCurrentMetricDataResponse";
const _GCMR = "GetContactMetricsRequest";
const _GCMRe = "GetContactMetricsResponse";
const _GCUD = "GetCurrentUserData";
const _GCUDR = "GetCurrentUserDataRequest";
const _GCUDRe = "GetCurrentUserDataResponse";
const _GEHOO = "GetEffectiveHoursOfOperations";
const _GEHOOR = "GetEffectiveHoursOfOperationsRequest";
const _GEHOORe = "GetEffectiveHoursOfOperationsResponse";
const _GET = "GreetingEndTimestamp";
const _GFA = "GetFlowAssociation";
const _GFAR = "GetFlowAssociationRequest";
const _GFARe = "GetFlowAssociationResponse";
const _GFT = "GetFederationToken";
const _GFTR = "GetFederationTokenRequest";
const _GFTRe = "GetFederationTokenResponse";
const _GMD = "GetMetricData";
const _GMDR = "GetMetricDataRequest";
const _GMDRe = "GetMetricDataResponse";
const _GMDV = "GetMetricDataV2";
const _GMDVR = "GetMetricDataV2Request";
const _GMDVRe = "GetMetricDataV2Response";
const _GPF = "GetPromptFile";
const _GPFR = "GetPromptFileRequest";
const _GPFRe = "GetPromptFileResponse";
const _GSIE = "GlobalSignInEndpoint";
const _GST = "GreetingStartTimestamp";
const _GTD = "GetTrafficDistribution";
const _GTDR = "GetTrafficDistributionRequest";
const _GTDRe = "GetTrafficDistributionResponse";
const _GTT = "GetTaskTemplate";
const _GTTR = "GetTaskTemplateRequest";
const _GTTRe = "GetTaskTemplateResponse";
const _H = "Hours";
const _HG = "HierarchyGroups";
const _HGA = "HierarchyGroupArn";
const _HGC = "HierarchyGroupCondition";
const _HGI = "HierarchyGroupId";
const _HGMT = "HierarchyGroupMatchType";
const _HGS = "HierarchyGroupSummary";
const _HGSL = "HierarchyGroupSummaryList";
const _HGSR = "HierarchyGroupSummaryReference";
const _HGi = "HierarchyGroup";
const _HL = "HierarchyLevel";
const _HLU = "HierarchyLevelUpdate";
const _HM = "HistoricalMetrics";
const _HMD = "HistoricalMetricData";
const _HMDC = "HistoricalMetricDataCollections";
const _HMR = "HistoricalMetricResult";
const _HMRi = "HistoricalMetricResults";
const _HMi = "HistoricalMetric";
const _HOO = "HoursOfOperation";
const _HOOA = "HoursOfOperationArn";
const _HOOC = "HoursOfOperationConfig";
const _HOOCL = "HoursOfOperationConfigList";
const _HOOI = "HoursOfOperationId";
const _HOOL = "HoursOfOperationList";
const _HOOO = "HoursOfOperationOverride";
const _HOOOC = "HoursOfOperationOverrideConfig";
const _HOOOCL = "HoursOfOperationOverrideConfigList";
const _HOOOI = "HoursOfOperationOverrideId";
const _HOOOL = "HoursOfOperationOverrideList";
const _HOOOSC = "HoursOfOperationOverrideSearchCriteria";
const _HOOOSCL = "HoursOfOperationOverrideSearchConditionList";
const _HOOOo = "HoursOfOperationOverrides";
const _HOOS = "HoursOfOperationSummary";
const _HOOSC = "HoursOfOperationSearchCriteria";
const _HOOSCL = "HoursOfOperationSearchConditionList";
const _HOOSF = "HoursOfOperationSearchFilter";
const _HOOSL = "HoursOfOperationSummaryList";
const _HOOTS = "HoursOfOperationTimeSlice";
const _HOOo = "HoursOfOperations";
const _HP = "HierarchyPath";
const _HPR = "HierarchyPathReference";
const _HRR = "HierarchyRestrictedResources";
const _HS = "HierarchyStructure";
const _HSU = "HierarchyStructureUpdate";
const _HTI = "HeadersToInclude";
const _He = "Headers";
const _I = "Id";
const _IA = "InitiateAs";
const _IAA = "IntegrationAssociationArn";
const _IAI = "IntegrationAssociationId";
const _IAR = "InboundAdditionalRecipients";
const _IAS = "IntegrationAssociationSummary";
const _IASL = "IntegrationAssociationSummaryList";
const _IAU = "InstanceAccessUrl";
const _IAn = "InstanceAlias";
const _IAns = "InstanceArn";
const _IAnt = "IntegrationArn";
const _ICE = "InboundCallsEnabled";
const _ICFE = "InvalidContactFlowException";
const _ICFME = "InvalidContactFlowModuleException";
const _ICI = "InitialContactId";
const _ID = "IsDefault";
const _IDn = "IntervalDetails";
const _IDs = "IssuesDetected";
const _IE = "IdempotencyException";
const _IEC = "InboundEmailContent";
const _IF = "InvisibleFields";
const _IFI = "InvisibleFieldInfo";
const _II = "InstanceId";
const _IId = "IdentityInfo";
const _IM = "InitiationMethod";
const _IMT = "IdentityManagementType";
const _IMn = "InitiationMethods";
const _IMni = "InitialMessage";
const _IP = "IntervalPeriod";
const _IPAC = "InputPredefinedAttributeConfiguration";
const _IPE = "InvalidParameterException";
const _IPN = "ImportPhoneNumber";
const _IPNR = "ImportPhoneNumberRequest";
const _IPNRm = "ImportPhoneNumberResponse";
const _IRE = "InvalidRequestException";
const _IRER = "InvalidRequestExceptionReason";
const _IRM = "IncludeRawMessage";
const _IRMn = "InboundRawMessage";
const _IRO = "IsReadOnly";
const _IRT = "IvrRecordingTrack";
const _IS = "InboundSubject";
const _ISC = "InstanceStorageConfig";
const _ISCn = "InstanceStorageConfigs";
const _ISE = "InternalServiceException";
const _ISL = "InstanceSummaryList";
const _ISM = "InitialSystemMessage";
const _ISR = "InstanceStatusReason";
const _ISn = "InstanceStatus";
const _ISnp = "InputSchema";
const _ISns = "InstanceSummary";
const _IT = "InitiationTimestamp";
const _ITTF = "InvisibleTaskTemplateFields";
const _ITn = "IntegrationType";
const _In = "Instance";
const _Ind = "Index";
const _Inp = "Input";
const _Ins = "Instructions";
const _Int = "Interval";
const _Is = "Issues";
const _It = "Items";
const _J = "Justification";
const _JT = "JoinToken";
const _K = "Key";
const _KBI = "KnowledgeBaseId";
const _KFC = "KinesisFirehoseConfig";
const _KI = "KeyId";
const _KSC = "KinesisStreamConfig";
const _KVSC = "KinesisVideoStreamConfig";
const _L = "Locked";
const _LAB = "LastActivatedBy";
const _LAC = "ListAssociatedContacts";
const _LACR = "ListAssociatedContactsRequest";
const _LACRi = "ListAssociatedContactsResponse";
const _LADA = "ListAnalyticsDataAssociations";
const _LADAR = "ListAnalyticsDataAssociationsRequest";
const _LADARi = "ListAnalyticsDataAssociationsResponse";
const _LADLDS = "ListAnalyticsDataLakeDataSets";
const _LADLDSR = "ListAnalyticsDataLakeDataSetsRequest";
const _LADLDSRi = "ListAnalyticsDataLakeDataSetsResponse";
const _LAO = "ListApprovedOrigins";
const _LAOR = "ListApprovedOriginsRequest";
const _LAORi = "ListApprovedOriginsResponse";
const _LAP = "ListAuthenticationProfiles";
const _LAPR = "ListAuthenticationProfilesRequest";
const _LAPRi = "ListAuthenticationProfilesResponse";
const _LAS = "ListAgentStatuses";
const _LASR = "ListAgentStatusRequest";
const _LASRi = "ListAgentStatusResponse";
const _LAT = "LastActivatedTime";
const _LB = "LexBot";
const _LBC = "LexBotConfig";
const _LBCL = "LexBotConfigList";
const _LBL = "LexBotsList";
const _LBR = "ListBotsRequest";
const _LBRi = "ListBotsResponse";
const _LBe = "LexBots";
const _LBi = "ListBots";
const _LC = "LanguageCode";
const _LCE = "ListContactEvaluations";
const _LCER = "ListContactEvaluationsRequest";
const _LCERi = "ListContactEvaluationsResponse";
const _LCF = "ListContactFlows";
const _LCFM = "ListContactFlowModules";
const _LCFMA = "ListContactFlowModuleAliases";
const _LCFMAR = "ListContactFlowModuleAliasesRequest";
const _LCFMARi = "ListContactFlowModuleAliasesResponse";
const _LCFMR = "ListContactFlowModulesRequest";
const _LCFMRi = "ListContactFlowModulesResponse";
const _LCFMV = "ListContactFlowModuleVersions";
const _LCFMVR = "ListContactFlowModuleVersionsRequest";
const _LCFMVRi = "ListContactFlowModuleVersionsResponse";
const _LCFR = "ListContactFlowsRequest";
const _LCFRi = "ListContactFlowsResponse";
const _LCFV = "ListContactFlowVersions";
const _LCFVR = "ListContactFlowVersionsRequest";
const _LCFVRi = "ListContactFlowVersionsResponse";
const _LCR = "ListContactReferences";
const _LCRR = "ListContactReferencesRequest";
const _LCRRi = "ListContactReferencesResponse";
const _LCi = "ListCondition";
const _LDV = "ListDefaultVocabularies";
const _LDVR = "ListDefaultVocabulariesRequest";
const _LDVRi = "ListDefaultVocabulariesResponse";
const _LEE = "LimitExceededException";
const _LEF = "ListEvaluationForms";
const _LEFR = "ListEvaluationFormsRequest";
const _LEFRi = "ListEvaluationFormsResponse";
const _LEFV = "ListEvaluationFormVersions";
const _LEFVR = "ListEvaluationFormVersionsRequest";
const _LEFVRi = "ListEvaluationFormVersionsResponse";
const _LF = "LevelFour";
const _LFA = "ListFlowAssociations";
const _LFAR = "ListFlowAssociationsRequest";
const _LFARi = "ListFlowAssociationsResponse";
const _LFa = "LambdaFunctions";
const _LFe = "LevelFive";
const _LHOO = "ListHoursOfOperations";
const _LHOOO = "ListHoursOfOperationOverrides";
const _LHOOOR = "ListHoursOfOperationOverridesRequest";
const _LHOOORi = "ListHoursOfOperationOverridesResponse";
const _LHOOR = "ListHoursOfOperationsRequest";
const _LHOORi = "ListHoursOfOperationsResponse";
const _LI = "L1Ids";
const _LIA = "ListInstanceAttributes";
const _LIAR = "ListInstanceAttributesRequest";
const _LIARi = "ListInstanceAttributesResponse";
const _LIARis = "ListIntegrationAssociationsRequest";
const _LIARist = "ListIntegrationAssociationsResponse";
const _LIAi = "ListIntegrationAssociations";
const _LIR = "ListInstancesRequest";
const _LIRi = "ListInstancesResponse";
const _LISC = "ListInstanceStorageConfigs";
const _LISCR = "ListInstanceStorageConfigsRequest";
const _LISCRi = "ListInstanceStorageConfigsResponse";
const _LId = "L2Ids";
const _LIds = "L3Ids";
const _LIds_ = "L4Ids";
const _LIds__ = "L5Ids";
const _LIe = "LevelId";
const _LIi = "ListInstances";
const _LLB = "ListLexBots";
const _LLBR = "ListLexBotsRequest";
const _LLBRi = "ListLexBotsResponse";
const _LLF = "ListLambdaFunctions";
const _LLFR = "ListLambdaFunctionsRequest";
const _LLFRi = "ListLambdaFunctionsResponse";
const _LMB = "LastModifiedBy";
const _LMR = "LastModifiedRegion";
const _LMT = "LastModifiedTime";
const _LMTa = "LastMessageTimestamp";
const _LN = "LastName";
const _LO = "LevelOne";
const _LP = "ListPrompts";
const _LPA = "ListPredefinedAttributes";
const _LPAR = "ListPredefinedAttributesRequest";
const _LPARi = "ListPredefinedAttributesResponse";
const _LPN = "ListPhoneNumbers";
const _LPNR = "ListPhoneNumbersRequest";
const _LPNRi = "ListPhoneNumbersResponse";
const _LPNS = "ListPhoneNumbersSummary";
const _LPNSL = "ListPhoneNumbersSummaryList";
const _LPNV = "ListPhoneNumbersV2";
const _LPNVR = "ListPhoneNumbersV2Request";
const _LPNVRi = "ListPhoneNumbersV2Response";
const _LPR = "ListPromptsRequest";
const _LPRi = "ListPromptsResponse";
const _LPT = "LastPausedTimestamp";
const _LQ = "ListQueues";
const _LQC = "ListQuickConnects";
const _LQCR = "ListQuickConnectsRequest";
const _LQCRi = "ListQuickConnectsResponse";
const _LQQC = "ListQueueQuickConnects";
const _LQQCR = "ListQueueQuickConnectsRequest";
const _LQQCRi = "ListQueueQuickConnectsResponse";
const _LQR = "ListQueuesRequest";
const _LQRi = "ListQueuesResponse";
const _LR = "LexRegion";
const _LRCASV = "ListRealtimeContactAnalysisSegmentsV2";
const _LRCASVR = "ListRealtimeContactAnalysisSegmentsV2Request";
const _LRCASVRi = "ListRealtimeContactAnalysisSegmentsV2Response";
const _LRP = "ListRoutingProfiles";
const _LRPMAQ = "ListRoutingProfileManualAssignmentQueues";
const _LRPMAQR = "ListRoutingProfileManualAssignmentQueuesRequest";
const _LRPMAQRi = "ListRoutingProfileManualAssignmentQueuesResponse";
const _LRPQ = "ListRoutingProfileQueues";
const _LRPQR = "ListRoutingProfileQueuesRequest";
const _LRPQRi = "ListRoutingProfileQueuesResponse";
const _LRPR = "ListRoutingProfilesRequest";
const _LRPRi = "ListRoutingProfilesResponse";
const _LRR = "ListRulesRequest";
const _LRRi = "ListRulesResponse";
const _LRT = "LastResumedTimestamp";
const _LRi = "ListRules";
const _LSK = "ListSecurityKeys";
const _LSKR = "ListSecurityKeysRequest";
const _LSKRi = "ListSecurityKeysResponse";
const _LSP = "ListSecurityProfiles";
const _LSPA = "ListSecurityProfileApplications";
const _LSPAR = "ListSecurityProfileApplicationsRequest";
const _LSPARi = "ListSecurityProfileApplicationsResponse";
const _LSPP = "ListSecurityProfilePermissions";
const _LSPPR = "ListSecurityProfilePermissionsRequest";
const _LSPPRi = "ListSecurityProfilePermissionsResponse";
const _LSPR = "ListSecurityProfilesRequest";
const _LSPRi = "ListSecurityProfilesResponse";
const _LT = "LevelTwo";
const _LTDG = "ListTrafficDistributionGroups";
const _LTDGR = "ListTrafficDistributionGroupsRequest";
const _LTDGRi = "ListTrafficDistributionGroupsResponse";
const _LTDGU = "ListTrafficDistributionGroupUsers";
const _LTDGUR = "ListTrafficDistributionGroupUsersRequest";
const _LTDGURi = "ListTrafficDistributionGroupUsersResponse";
const _LTFR = "ListTagsForResource";
const _LTFRR = "ListTagsForResourceRequest";
const _LTFRRi = "ListTagsForResourceResponse";
const _LTT = "ListTaskTemplates";
const _LTTR = "ListTaskTemplatesRequest";
const _LTTRi = "ListTaskTemplatesResponse";
const _LTe = "LevelThree";
const _LU = "ListUsers";
const _LUB = "LastUpdatedBy";
const _LUC = "ListUseCases";
const _LUCR = "ListUseCasesRequest";
const _LUCRi = "ListUseCasesResponse";
const _LUHG = "ListUserHierarchyGroups";
const _LUHGR = "ListUserHierarchyGroupsRequest";
const _LUHGRi = "ListUserHierarchyGroupsResponse";
const _LUP = "ListUserProficiencies";
const _LUPR = "ListUserProficienciesRequest";
const _LUPRi = "ListUserProficienciesResponse";
const _LUR = "ListUsersRequest";
const _LURi = "ListUsersResponse";
const _LUT = "LastUpdateTimestamp";
const _LUTa = "LastUpdatedTime";
const _LV = "LatestVersion";
const _LVB = "LexV2Bot";
const _LVR = "ListViewsRequest";
const _LVRi = "ListViewsResponse";
const _LVV = "ListViewVersions";
const _LVVR = "ListViewVersionsRequest";
const _LVVRi = "ListViewVersionsResponse";
const _LVe = "LexVersion";
const _LVi = "ListViews";
const _La = "Label";
const _Le = "Level1";
const _Lev = "Level2";
const _Leve = "Level3";
const _Level = "Level4";
const _Level_ = "Level5";
const _Level__ = "Level";
const _Lo = "Location";
const _M = "Message";
const _MAQC = "ManualAssignmentQueueConfigs";
const _MAQR = "ManualAssignmentQueueReferences";
const _MC = "MatchCriteria";
const _MCR = "MonitorContactRequest";
const _MCRo = "MonitorContactResponse";
const _MCa = "MaxContacts";
const _MCe = "MediaConcurrencies";
const _MCed = "MediaConcurrency";
const _MCo = "MonitorContact";
const _MD = "MatchedDetails";
const _MDCV = "MetricDataCollectionsV2";
const _MDV = "MetricDataV2";
const _MF = "MeetingFeatures";
const _MFC = "MeetingFeaturesConfiguration";
const _MFK = "MetricFilterKey";
const _MFV = "MetricFilterV2";
const _MFVL = "MetricFiltersV2List";
const _MFVe = "MetricFilterValues";
const _MFe = "MetricFilters";
const _MI = "MeetingId";
const _MIe = "MetricInterval";
const _MLIC = "MessageLengthInChars";
const _MO = "MillisecondOffsets";
const _MP = "MultiParty";
const _MPL = "MinProficiencyLevel";
const _MPLa = "MaxProficiencyLevel";
const _MPe = "MediaPlacement";
const _MR = "MetricResults";
const _MRC = "MatchedRuleCategories";
const _MRRE = "MaximumResultReturnedException";
const _MRTIM = "MaxResponseTimeInMillis";
const _MRV = "MetricResultV2";
const _MRVe = "MetricResultsV2";
const _MRa = "MaxResults";
const _MRe = "MediaRegion";
const _MS = "MessagesSent";
const _MSBC = "MaxSlotsByChannel";
const _MSD = "MaxSessionDuration";
const _MST = "MessageSourceType";
const _MSTe = "MediaStreamType";
const _MT = "ModifiedTimestamp";
const _MTI = "MessageTemplateId";
const _MTa = "MatchType";
const _MV = "MinValue";
const _MVa = "MaxValue";
const _MVe = "MetricV2";
const _MVet = "MetricsV2";
const _Me = "Meeting";
const _Met = "Metric";
const _Meta = "Metadata";
const _Metr = "Metrics";
const _Mi = "Minutes";
const _Mo = "Mode";
const _Mob = "Mobile";
const _N = "Name";
const _NA = "NotApplicable";
const _NAC = "NotAttributeCondition";
const _NAE = "NotApplicableEnabled";
const _NC = "NumberCondition";
const _NCC = "NewChatCreated";
const _NCa = "NameCriteria";
const _NOAMAQ = "NumberOfAssociatedManualAssignmentQueues";
const _NOAQ = "NumberOfAssociatedQueues";
const _NOAU = "NumberOfAssociatedUsers";
const _NQPVA = "NumericQuestionPropertyValueAutomation";
const _NR = "NumberReference";
const _NRT = "NotificationRecipientType";
const _NRu = "NumResponses";
const _NS = "NextStatus";
const _NSD = "NewSessionDetails";
const _NSW = "NameStartsWith";
const _NT = "NextToken";
const _NV = "NumericValue";
const _Na = "Namespace";
const _Ne = "Negate";
const _No = "Notes";
const _Nu = "Number";
const _Num = "Numeric";
const _O = "Origin";
const _OAR = "OutboundAdditionalRecipients";
const _OC = "OrConditions";
const _OCC = "OutboundCallerConfig";
const _OCE = "OutboundCallsEnabled";
const _OCIN = "OutboundCallerIdName";
const _OCINI = "OutboundCallerIdNumberId";
const _OCNPE = "OutboundContactNotPermittedException";
const _OE = "OrExpression";
const _OEAI = "OutboundEmailAddressId";
const _OEC = "OutboundEmailConfig";
const _OECu = "OutboundEmailContent";
const _OFI = "OutboundFlowId";
const _OH = "OperationalHours";
const _OHp = "OperationalHour";
const _ORI = "OrganizationResourceId";
const _ORIp = "OptionRefId";
const _ORIu = "OutboundRequestId";
const _ORM = "OutboundRawMessage";
const _OS = "OutboundSubject";
const _OSC = "OutboundStrategyConfig";
const _OSp = "OperatingSystem";
const _OSu = "OutboundStrategy";
const _OT = "OutputType";
const _OTNFE = "OutputTypeNotFoundException";
const _OTS = "OverrideTimeSlice";
const _Op = "Operands";
const _Ope = "Operator";
const _Opt = "Options";
const _Or = "Origins";
const _Ord = "Order";
const _P = "Password";
const _PA = "PredefinedAttribute";
const _PAA = "PinpointAppArn";
const _PAC = "PredefinedAttributeConfiguration";
const _PARN = "PromptARN";
const _PAS = "PredefinedAttributeSummary";
const _PASC = "PredefinedAttributeSearchCriteria";
const _PASCL = "PredefinedAttributeSearchConditionList";
const _PASL = "PredefinedAttributeSummaryList";
const _PASSL = "PredefinedAttributeSearchSummaryList";
const _PATC = "PostAcceptTimeoutConfig";
const _PAV = "PredefinedAttributeValues";
const _PAr = "PredefinedAttributes";
const _PC = "ParticipantCredentials";
const _PCI = "PreviousContactId";
const _PCR = "PauseContactRequest";
const _PCRa = "PauseContactResponse";
const _PCS = "PostContactSummary";
const _PCa = "ParticipantCapabilities";
const _PCau = "PauseContact";
const _PCe = "PersistentChat";
const _PCer = "PersistentConnection";
const _PCh = "PhoneConfig";
const _PD = "ParticipantDetails";
const _PDI = "PotentialDisconnectIssue";
const _PDTA = "ParticipantDetailsToAdd";
const _PDr = "ProblemDetail";
const _PET = "PreviewEndTimestamp";
const _PGI = "ParentGroupId";
const _PI = "ParticipantId";
const _PIr = "PromptId";
const _PL = "ProficiencyLevel";
const _PLr = "PropertyList";
const _PLro = "PromptList";
const _PM = "ParticipantMetrics";
const _PN = "PhoneNumber";
const _PNA = "PhoneNumberArn";
const _PNCC = "PhoneNumberCountryCode";
const _PNCCh = "PhoneNumberCountryCodes";
const _PND = "PhoneNumberDescription";
const _PNI = "PhoneNumberId";
const _PNP = "PhoneNumberPrefix";
const _PNQCC = "PhoneNumberQuickConnectConfig";
const _PNS = "PhoneNumberStatus";
const _PNSL = "PhoneNumberSummaryList";
const _PNSh = "PhoneNumberSummary";
const _PNT = "PhoneNumberType";
const _PNTh = "PhoneNumberTypes";
const _PNl = "PlatformName";
const _POI = "PointsOfInterest";
const _PP = "PropertyPath";
const _PPU = "PromptPresignedUrl";
const _PQI = "PotentialQualityIssues";
const _PR = "ParticipantRole";
const _PS = "PublishStatus";
const _PSC = "PromptSearchCriteria";
const _PSCL = "PromptSearchConditionList";
const _PSD = "PeriodicSessionDuration";
const _PSF = "PromptSearchFilter";
const _PSL = "PromptSummaryList";
const _PSr = "PromptSummary";
const _PT = "ParticipantType";
const _PTA = "ParticipantTimerAction";
const _PTC = "ParticipantTimerConfiguration";
const _PTCL = "ParticipantTimerConfigList";
const _PTCa = "ParticipantTokenCredentials";
const _PTDIM = "ParticipantTimerDurationInMinutes";
const _PTV = "ParticipantTimerValue";
const _PTa = "ParticipantToken";
const _PTh = "PhoneType";
const _PUS = "PutUserStatus";
const _PUSR = "PutUserStatusRequest";
const _PUSRu = "PutUserStatusResponse";
const _PV = "PlatformVersion";
const _PVE = "PropertyValidationException";
const _PVEP = "PropertyValidationExceptionProperty";
const _PVEPL = "PropertyValidationExceptionPropertyList";
const _PVr = "PropertyValue";
const _Pe = "Permissions";
const _Per = "Percentage";
const _Pr = "Preview";
const _Pre = "Prefix";
const _Pri = "Priority";
const _Pro = "Prompt";
const _Prob = "Problems";
const _Prom = "Prompts";
const _Pu = "Purposes";
const _Q = "Queue";
const _QA = "QueueArn";
const _QC = "QueueConfigs";
const _QCARN = "QuickConnectARN";
const _QCC = "QuickConnectConfig";
const _QCI = "QuickConnectIds";
const _QCIu = "QuickConnectId";
const _QCS = "QuickConnectSummary";
const _QCSC = "QuickConnectSearchCriteria";
const _QCSCL = "QuickConnectSearchConditionList";
const _QCSF = "QuickConnectSearchFilter";
const _QCSL = "QuickConnectSummaryList";
const _QCSSL = "QuickConnectSearchSummaryList";
const _QCT = "QuickConnectTypes";
const _QCTu = "QuickConnectType";
const _QCu = "QuickConnect";
const _QCue = "QueueConfig";
const _QCui = "QuickConnects";
const _QI = "QueueId";
const _QII = "QueueInfoInput";
const _QIu = "QueueInfo";
const _QIue = "QueueIds";
const _QM = "QualityMetrics";
const _QN = "QueueName";
const _QP = "QueuePriority";
const _QQCC = "QueueQuickConnectConfig";
const _QR = "QueueReferences";
const _QRu = "QueueReference";
const _QS = "QualityScore";
const _QSC = "QueueSearchCriteria";
const _QSCL = "QueueSearchConditionList";
const _QSF = "QueueSearchFilter";
const _QSL = "QueueSummaryList";
const _QSSL = "QueueSearchSummaryList";
const _QSu = "QueueSummary";
const _QT = "QuestionType";
const _QTAS = "QueueTimeAdjustmentSeconds";
const _QTC = "QueueTypeCondition";
const _QTP = "QuestionTypeProperties";
const _QTu = "QueueTypes";
const _QTue = "QueueType";
const _Qu = "Queues";
const _Que = "Question";
const _R = "Range";
const _RA = "RuleArn";
const _RAe = "ResourceArn";
const _RAep = "ReplicaAlias";
const _RAu = "RuleAction";
const _RAul = "RuleActions";
const _RC = "RoutingCriteria";
const _RCAS = "RealtimeContactAnalysisSegments";
const _RCASe = "RealtimeContactAnalysisSegment";
const _RCE = "ResourceConflictException";
const _RCI = "RelatedContactId";
const _RCIS = "RoutingCriteriaInputStep";
const _RCISE = "RoutingCriteriaInputStepExpiry";
const _RCISo = "RoutingCriteriaInputSteps";
const _RCIo = "RoutingCriteriaInput";
const _RCR = "ResumeContactRequest";
const _RCRR = "ResumeContactRecordingRequest";
const _RCRRe = "ResumeContactRecordingResponse";
const _RCRe = "ResumeContactResponse";
const _RCRes = "ResumeContactRecording";
const _RCe = "ReplicationConfiguration";
const _RCes = "ResumeContact";
const _RCu = "RuleCategory";
const _RF = "RequiredFields";
const _RFI = "RequiredFieldInfo";
const _RI = "ResourceId";
const _RIR = "ReplicateInstanceRequest";
const _RIRe = "ReplicateInstanceResponse";
const _RIUE = "ResourceInUseException";
const _RIe = "ResourceIds";
const _RIec = "RecordingInfo";
const _RIef = "RefId";
const _RIeg = "RegistrationId";
const _RIep = "ReplicateInstance";
const _RIeq = "RequestIdentifier";
const _RIu = "RuleId";
const _RM = "RawMessage";
const _RNFE = "ResourceNotFoundException";
const _RNRE = "ResourceNotReadyException";
const _ROF = "ReadOnlyFields";
const _ROFI = "ReadOnlyFieldInfo";
const _RON = "ResetOrderNumber";
const _ROTTF = "ReadOnlyTaskTemplateFields";
const _RP = "RoutingProfile";
const _RPA = "RoutingProfileArn";
const _RPH = "RetentionPeriodHours";
const _RPI = "RoutingProfileId";
const _RPL = "RoutingProfileList";
const _RPMAQC = "RoutingProfileManualAssignmentQueueConfig";
const _RPMAQCL = "RoutingProfileManualAssignmentQueueConfigList";
const _RPMAQCS = "RoutingProfileManualAssignmentQueueConfigSummary";
const _RPMAQCSL = "RoutingProfileManualAssignmentQueueConfigSummaryList";
const _RPN = "ReleasePhoneNumber";
const _RPNR = "ReleasePhoneNumberRequest";
const _RPQC = "RoutingProfileQueueConfig";
const _RPQCL = "RoutingProfileQueueConfigList";
const _RPQCS = "RoutingProfileQueueConfigSummary";
const _RPQCSL = "RoutingProfileQueueConfigSummaryList";
const _RPQR = "RoutingProfileQueueReference";
const _RPQRL = "RoutingProfileQueueReferenceList";
const _RPR = "RoutingProfileReference";
const _RPS = "RoutingProfileSummary";
const _RPSC = "RoutingProfileSearchCriteria";
const _RPSCL = "RoutingProfileSearchConditionList";
const _RPSF = "RoutingProfileSearchFilter";
const _RPSL = "RoutingProfileSummaryList";
const _RPo = "RoutingProfiles";
const _RR = "ReplicaRegion";
const _RS = "ReplicationStatus";
const _RSA = "ResourceShareArn";
const _RSE = "RoutingStepExpression";
const _RSEo = "RoutingStepExpressions";
const _RSI = "ResourceShareId";
const _RSL = "ReferenceSummaryList";
const _RSLu = "RuleSummaryList";
const _RSR = "ReplicationStatusReason";
const _RSS = "ResourceShareStatus";
const _RSSL = "ReplicationStatusSummaryList";
const _RSSe = "ReplicationStatusSummary";
const _RST = "RingStartTimestamp";
const _RSe = "ReferenceSummary";
const _RSu = "RuleSummary";
const _RT = "ResourceType";
const _RTCAA = "RealTimeContactAnalysisAttachment";
const _RTCAAe = "RealTimeContactAnalysisAttachments";
const _RTCACD = "RealTimeContactAnalysisCategoryDetails";
const _RTCACI = "RealTimeContactAnalysisCharacterInterval";
const _RTCACIe = "RealTimeContactAnalysisCharacterIntervals";
const _RTCAID = "RealTimeContactAnalysisIssueDetected";
const _RTCAIDe = "RealTimeContactAnalysisIssuesDetected";
const _RTCAMD = "RealTimeContactAnalysisMatchedDetails";
const _RTCAPOI = "RealTimeContactAnalysisPointOfInterest";
const _RTCAPOIe = "RealTimeContactAnalysisPointsOfInterest";
const _RTCASA = "RealTimeContactAnalysisSegmentAttachments";
const _RTCASC = "RealTimeContactAnalysisSegmentCategories";
const _RTCASE = "RealTimeContactAnalysisSegmentEvent";
const _RTCASI = "RealTimeContactAnalysisSegmentIssues";
const _RTCASPCS = "RealTimeContactAnalysisSegmentPostContactSummary";
const _RTCAST = "RealTimeContactAnalysisSegmentTranscript";
const _RTCATD = "RealTimeContactAnalysisTimeData";
const _RTCATIR = "RealTimeContactAnalysisTranscriptItemRedaction";
const _RTCATIWC = "RealTimeContactAnalysisTranscriptItemWithContent";
const _RTCATIWCO = "RealTimeContactAnalysisTranscriptItemWithCharacterOffsets";
const _RTCATIWCOe = "RealTimeContactAnalysisTranscriptItemsWithCharacterOffsets";
const _RTCATIWCe = "RealTimeContactAnalysisTranscriptItemsWithContent";
const _RTE = "RefreshTokenExpiration";
const _RTES = "RuleTriggerEventSource";
const _RTIS = "RingTimeoutInSeconds";
const _RTSC = "ResourceTagsSearchCriteria";
const _RTTF = "RequiredTaskTemplateFields";
const _RTe = "RehydrationType";
const _RTef = "RefreshToken";
const _RTefe = "ReferenceTypes";
const _RTes = "ResourceTypes";
const _Re = "Recordings";
const _Rea = "Reason";
const _Rec = "Recipient";
const _Red = "Redaction";
const _Ref = "References";
const _Refe = "Reference";
const _Reg = "Region";
const _Res = "Results";
const _Ru = "Rule";
const _S = "State";
const _SA = "StatusArn";
const _SACS = "SearchableAgentCriteriaStep";
const _SAEA = "SubmitAutoEvaluationAction";
const _SAEAD = "SubmitAutoEvaluationActionDefinition";
const _SAF = "ScoreAutomaticFail";
const _SAFI = "SelfAssignFlowId";
const _SAFU = "StartAttachedFileUpload";
const _SAFUR = "StartAttachedFileUploadRequest";
const _SAFURt = "StartAttachedFileUploadResponse";
const _SAN = "SourceApplicationName";
const _SAPN = "SearchAvailablePhoneNumbers";
const _SAPNR = "SearchAvailablePhoneNumbersRequest";
const _SAPNRe = "SearchAvailablePhoneNumbersResponse";
const _SAS = "SearchAgentStatuses";
const _SASR = "SearchAgentStatusesRequest";
const _SASRe = "SearchAgentStatusesResponse";
const _SAT = "SlaAssignmentType";
const _SAU = "SourceApplicationUrl";
const _SAV = "SegmentAttributeValue";
const _SAVL = "SegmentAttributeValueList";
const _SAVM = "SegmentAttributeValueMap";
const _SAe = "SegmentAttributes";
const _SAes = "SessionArn";
const _SAt = "StreamArn";
const _SAu = "SuggestedAnswers";
const _SB = "SubmittedBy";
const _SBM = "SortByMetric";
const _SC = "StringCondition";
const _SCA = "SearchableContactAttributes";
const _SCAC = "SearchableContactAttributesCriteria";
const _SCACL = "SearchableContactAttributesCriteriaList";
const _SCAK = "SearchableContactAttributeKey";
const _SCATR = "SearchContactsAdditionalTimeRange";
const _SCATRC = "SearchContactsAdditionalTimeRangeCriteria";
const _SCATRCL = "SearchContactsAdditionalTimeRangeCriteriaList";
const _SCAV = "SearchableContactAttributeValue";
const _SCAVL = "SearchableContactAttributeValueList";
const _SCC = "StartChatContact";
const _SCCR = "StartChatContactRequest";
const _SCCRt = "StartChatContactResponse";
const _SCE = "SearchContactEvaluations";
const _SCER = "SearchContactEvaluationsRequest";
const _SCERe = "SearchContactEvaluationsResponse";
const _SCERt = "StartContactEvaluationRequest";
const _SCERta = "StartContactEvaluationResponse";
const _SCERu = "SubmitContactEvaluationRequest";
const _SCERub = "SubmitContactEvaluationResponse";
const _SCEt = "StartContactEvaluation";
const _SCEu = "SubmitContactEvaluation";
const _SCF = "SearchContactFlows";
const _SCFM = "SearchContactFlowModules";
const _SCFMR = "SearchContactFlowModulesRequest";
const _SCFMRe = "SearchContactFlowModulesResponse";
const _SCFR = "SearchContactFlowsRequest";
const _SCFRe = "SearchContactFlowsResponse";
const _SCI = "SourceContactId";
const _SCIE = "SendChatIntegrationEvent";
const _SCIER = "SendChatIntegrationEventRequest";
const _SCIERe = "SendChatIntegrationEventResponse";
const _SCR = "SearchContactsRequest";
const _SCRR = "StartContactRecordingRequest";
const _SCRRt = "StartContactRecordingResponse";
const _SCRRto = "StopContactRecordingRequest";
const _SCRRtop = "StopContactRecordingResponse";
const _SCRRu = "SuspendContactRecordingRequest";
const _SCRRus = "SuspendContactRecordingResponse";
const _SCRe = "SearchContactsResponse";
const _SCRt = "StopContactRequest";
const _SCRta = "StartContactRecording";
const _SCRto = "StopContactResponse";
const _SCRtop = "StopContactRecording";
const _SCRu = "SuspendContactRecording";
const _SCS = "StartContactStreaming";
const _SCSR = "StartContactStreamingRequest";
const _SCSRt = "StartContactStreamingResponse";
const _SCSRto = "StopContactStreamingRequest";
const _SCSRtop = "StopContactStreamingResponse";
const _SCSt = "StopContactStreaming";
const _SCTC = "SearchContactsTimestampCondition";
const _SCTR = "SearchContactsTimeRange";
const _SCe = "SearchCriteria";
const _SCea = "SearchContacts";
const _SCo = "SortCriteria";
const _SCon = "S3Config";
const _SCou = "SourceCampaign";
const _SCt = "StorageConfig";
const _SCta = "StateCondition";
const _SCtat = "StatusCondition";
const _SCto = "StorageConfigs";
const _SCtop = "StopContact";
const _SCtr = "StreamingConfiguration";
const _SE = "SystemEndpoint";
const _SEA = "StreamingEndpointArn";
const _SEAR = "SearchEmailAddressesRequest";
const _SEARe = "SearchEmailAddressesResponse";
const _SEAe = "SearchEmailAddresses";
const _SEC = "StartEmailContact";
const _SECR = "StartEmailContactRequest";
const _SECRt = "StartEmailContactResponse";
const _SEF = "SearchEvaluationForms";
const _SEFR = "SearchEvaluationFormsRequest";
const _SEFRe = "SearchEvaluationFormsResponse";
const _SET = "StateEndTimestamp";
const _SEe = "SecondaryEmail";
const _SEo = "SourceEndpoint";
const _SF = "SearchFilter";
const _SFVUL = "SlaFieldValueUnionList";
const _SHOO = "SearchHoursOfOperations";
const _SHOOO = "SearchHoursOfOperationOverrides";
const _SHOOOR = "SearchHoursOfOperationOverridesRequest";
const _SHOOORe = "SearchHoursOfOperationOverridesResponse";
const _SHOOR = "SearchHoursOfOperationsRequest";
const _SHOORe = "SearchHoursOfOperationsResponse";
const _SI = "SourceId";
const _SIC = "SignInConfig";
const _SID = "SessionInactivityDuration";
const _SIDL = "SignInDistributionList";
const _SIDi = "SignInDistribution";
const _SIHE = "SessionInactivityHandlingEnabled";
const _SIU = "SignInUrl";
const _SIt = "StreamingId";
const _SK = "SecurityKeys";
const _SKL = "SecurityKeysList";
const _SKe = "SecurityKey";
const _SL = "StringList";
const _SMCT = "SupportedMessagingContentTypes";
const _SN = "StatusName";
const _SNA = "ScoreNotApplicable";
const _SNAD = "SendNotificationActionDefinition";
const _SNAe = "SendNotificationAction";
const _SO = "SortOrder";
const _SOCC = "StartOutboundChatContact";
const _SOCCR = "StartOutboundChatContactRequest";
const _SOCCRt = "StartOutboundChatContactResponse";
const _SOE = "SendOutboundEmail";
const _SOEC = "StartOutboundEmailContact";
const _SOECR = "StartOutboundEmailContactRequest";
const _SOECRt = "StartOutboundEmailContactResponse";
const _SOER = "SendOutboundEmailRequest";
const _SOERe = "SendOutboundEmailResponse";
const _SOVC = "StartOutboundVoiceContact";
const _SOVCR = "StartOutboundVoiceContactRequest";
const _SOVCRt = "StartOutboundVoiceContactResponse";
const _SP = "SecurityProfile";
const _SPA = "SecurityProfileArn";
const _SPAR = "SearchPredefinedAttributesRequest";
const _SPARe = "SearchPredefinedAttributesResponse";
const _SPAe = "SearchPredefinedAttributes";
const _SPI = "SecurityProfileId";
const _SPIe = "SecurityProfileIds";
const _SPN = "SecurityProfileName";
const _SPNA = "SourcePhoneNumberArn";
const _SPNo = "SourcePhoneNumber";
const _SPR = "SearchPromptsRequest";
const _SPRe = "SearchPromptsResponse";
const _SPS = "SecurityProfileSummary";
const _SPSC = "SecurityProfileSearchCriteria";
const _SPSCL = "SecurityProfileSearchConditionList";
const _SPSF = "SecurityProfilesSearchFilter";
const _SPSL = "SecurityProfileSummaryList";
const _SPSS = "SecurityProfileSearchSummary";
const _SPSSL = "SecurityProfilesSearchSummaryList";
const _SPc = "ScorePercentage";
const _SPe = "SecurityProfiles";
const _SPea = "SearchPrompts";
const _SQ = "SearchQueues";
const _SQC = "SearchQuickConnects";
const _SQCR = "SearchQuickConnectsRequest";
const _SQCRe = "SearchQuickConnectsResponse";
const _SQEE = "ServiceQuotaExceededException";
const _SQEER = "ServiceQuotaExceededExceptionReason";
const _SQR = "SearchQueuesRequest";
const _SQRe = "SearchQueuesResponse";
const _SR = "ServiceRole";
const _SRC = "SearchableRoutingCriteria";
const _SRCS = "SearchableRoutingCriteriaStep";
const _SRCSL = "SearchableRoutingCriteriaStepList";
const _SRL = "SuccessfulRequestList";
const _SRP = "SearchRoutingProfiles";
const _SRPR = "SearchRoutingProfilesRequest";
const _SRPRe = "SearchRoutingProfilesResponse";
const _SRT = "SearchResourceTags";
const _SRTR = "SearchResourceTagsRequest";
const _SRTRe = "SearchResourceTagsResponse";
const _SRo = "SourceRegion";
const _SRt = "StatusReason";
const _SRtr = "StringReference";
const _SRu = "SuccessfulRequest";
const _SS = "ScoringStrategy";
const _SSA = "SearchableSegmentAttributes";
const _SSAC = "SearchableSegmentAttributesCriteria";
const _SSACL = "SearchableSegmentAttributesCriteriaList";
const _SSAK = "SearchableSegmentAttributeKey";
const _SSAV = "SearchableSegmentAttributeValue";
const _SSAVL = "SearchableSegmentAttributeValueList";
const _SSO = "SingleSelectOptions";
const _SSP = "SearchSecurityProfiles";
const _SSPR = "SearchSecurityProfilesRequest";
const _SSPRe = "SearchSecurityProfilesResponse";
const _SSQRCA = "SingleSelectQuestionRuleCategoryAutomation";
const _SSS = "StartScreenSharing";
const _SSSR = "StartScreenSharingRequest";
const _SSSRt = "StartScreenSharingResponse";
const _SST = "StateStartTimestamp";
const _SSTt = "StatusStartTimestamp";
const _SSV = "SystemSuggestedValue";
const _SSc = "ScreenShare";
const _SSi = "SingleSelect";
const _ST = "SearchText";
const _STC = "StartTaskContact";
const _STCR = "StartTaskContactRequest";
const _STCRt = "StartTaskContactResponse";
const _STL = "SearchTextList";
const _STc = "ScheduledTimestamp";
const _STch = "ScheduledTime";
const _STe = "SecurityToken";
const _STeg = "SegmentTypes";
const _STo = "SourceType";
const _STt = "StateTransitions";
const _STta = "StartTimestamp";
const _STtar = "StartTime";
const _STtat = "StateTransition";
const _STto = "StorageType";
const _STtop = "StopTimestamp";
const _SU = "S3Uri";
const _SUHG = "SearchUserHierarchyGroups";
const _SUHGR = "SearchUserHierarchyGroupsRequest";
const _SUHGRe = "SearchUserHierarchyGroupsResponse";
const _SUR = "SearchUsersRequest";
const _SURe = "SearchUsersResponse";
const _SUe = "SearchUsers";
const _SUi = "SignalingUrl";
const _SUr = "S3Url";
const _SV = "StringValue";
const _SVR = "SearchVocabulariesRequest";
const _SVRe = "SearchVocabulariesResponse";
const _SVe = "SearchVocabularies";
const _SVn = "SnapshotVersion";
const _SWRTCC = "StartWebRTCContact";
const _SWRTCCR = "StartWebRTCContactRequest";
const _SWRTCCRt = "StartWebRTCContactResponse";
const _Sc = "Scores";
const _Sco = "Score";
const _Se = "Settings";
const _Sec = "Section";
const _Seg = "Segments";
const _Sen = "Sentiment";
const _So = "Source";
const _Sor = "Sort";
const _St = "Status";
const _Sta = "Statistic";
const _Star = "Start";
const _Ste = "Steps";
const _Step = "Step";
const _Str = "String";
const _Su = "Subject";
const _Sub = "Subtype";
const _T = "Type";
const _TA = "TargetArn";
const _TACL = "TagAndConditionList";
const _TAD = "TaskActionDefinition";
const _TAI = "TargetAccountId";
const _TAa = "TaskAction";
const _TAe = "TemplateAttributes";
const _TAo = "ToAddresses";
const _TBM = "TotalBotMessages";
const _TBMLIC = "TotalBotMessageLengthInChars";
const _TC = "TagConditions";
const _TCL = "TranscriptCriteriaList";
const _TCR = "TagContactRequest";
const _TCRa = "TagContactResponse";
const _TCRr = "TransferContactRequest";
const _TCRra = "TransferContactResponse";
const _TCU = "TurnControlUrl";
const _TCa = "TagCondition";
const _TCag = "TagContact";
const _TCe = "TelephonyConfig";
const _TCh = "ThresholdCollections";
const _TCi = "TimestampCondition";
const _TCo = "TotalCount";
const _TCr = "TranscriptCriteria";
const _TCra = "TransferContact";
const _TCy = "TypeCondition";
const _TD = "ToDate";
const _TDG = "TrafficDistributionGroup";
const _TDGI = "TrafficDistributionGroupId";
const _TDGS = "TrafficDistributionGroupSummary";
const _TDGSL = "TrafficDistributionGroupSummaryList";
const _TDGUS = "TrafficDistributionGroupUserSummary";
const _TDGUSL = "TrafficDistributionGroupUserSummaryList";
const _TE = "ThrottlingException";
const _TES = "TriggerEventSource";
const _TF = "TagFilter";
const _TFV = "TargetFieldValues";
const _TI = "TemplateId";
const _TIr = "TranscriptItems";
const _TK = "TagKey";
const _TKa = "TagKeys";
const _TL = "ToList";
const _TLT = "TargetListType";
const _TLa = "TagsList";
const _TM = "TotalMessages";
const _TMC = "TemplatedMessageConfig";
const _TMRE = "TooManyRequestsException";
const _TOCL = "TagOrConditionList";
const _TPC = "TotalPauseCount";
const _TPDIS = "TotalPauseDurationInSeconds";
const _TR = "TimeRange";
const _TRR = "TagRestrictedResources";
const _TRRa = "TagResourceRequest";
const _TRTIM = "TotalResponseTimeInMillis";
const _TRa = "TagResource";
const _TS = "TargetSection";
const _TSC = "TagSearchCondition";
const _TSM = "TargetSlaMinutes";
const _TSa = "TagSet";
const _TSr = "TranscriptSegment";
const _TT = "TranscriptType";
const _TTC = "TaskTemplateConstraints";
const _TTD = "TaskTemplateDefaults";
const _TTDFV = "TaskTemplateDefaultFieldValue";
const _TTDFVL = "TaskTemplateDefaultFieldValueList";
const _TTF = "TaskTemplateField";
const _TTFI = "TaskTemplateFieldIdentifier";
const _TTFa = "TaskTemplateFields";
const _TTI = "TaskTemplateInfo";
const _TTIV = "TaskTemplateInfoV2";
const _TTIa = "TaskTemplateId";
const _TTL = "TaskTemplateList";
const _TTM = "TaskTemplateMetadata";
const _TTa = "TaskTemplates";
const _TTi = "TimerType";
const _TTr = "TrafficType";
const _TV = "TimerValue";
const _TVa = "TagValue";
const _TVh = "ThresholdValue";
const _TVhr = "ThresholdV2";
const _TZ = "TimeZone";
const _Ta = "Tags";
const _Te = "Text";
const _Tem = "Template";
const _Th = "Threshold";
const _Ti = "Title";
const _Tim = "Time";
const _Tr = "Transcript";
const _U = "Username";
const _UA = "UserArn";
const _UAF = "UserAttributeFilter";
const _UAP = "UpdateAuthenticationProfile";
const _UAPR = "UpdateAuthenticationProfileRequest";
const _UAS = "UpdateAgentStatus";
const _UASR = "UpdateAgentStatusRequest";
const _UB = "UpdatedBy";
const _UC = "UserConfig";
const _UCA = "UseCaseArn";
const _UCAD = "UpdateCaseActionDefinition";
const _UCAR = "UpdateContactAttributesRequest";
const _UCARp = "UpdateContactAttributesResponse";
const _UCAp = "UpdateCaseAction";
const _UCApd = "UpdateContactAttributes";
const _UCE = "UpdateContactEvaluation";
const _UCER = "UpdateContactEvaluationRequest";
const _UCERp = "UpdateContactEvaluationResponse";
const _UCFC = "UpdateContactFlowContent";
const _UCFCR = "UpdateContactFlowContentRequest";
const _UCFCRp = "UpdateContactFlowContentResponse";
const _UCFM = "UpdateContactFlowMetadata";
const _UCFMA = "UpdateContactFlowModuleAlias";
const _UCFMAR = "UpdateContactFlowModuleAliasRequest";
const _UCFMARp = "UpdateContactFlowModuleAliasResponse";
const _UCFMC = "UpdateContactFlowModuleContent";
const _UCFMCR = "UpdateContactFlowModuleContentRequest";
const _UCFMCRp = "UpdateContactFlowModuleContentResponse";
const _UCFMM = "UpdateContactFlowModuleMetadata";
const _UCFMMR = "UpdateContactFlowModuleMetadataRequest";
const _UCFMMRp = "UpdateContactFlowModuleMetadataResponse";
const _UCFMR = "UpdateContactFlowMetadataRequest";
const _UCFMRp = "UpdateContactFlowMetadataResponse";
const _UCFN = "UpdateContactFlowName";
const _UCFNR = "UpdateContactFlowNameRequest";
const _UCFNRp = "UpdateContactFlowNameResponse";
const _UCI = "UseCaseId";
const _UCR = "UntagContactRequest";
const _UCRD = "UpdateContactRoutingData";
const _UCRDR = "UpdateContactRoutingDataRequest";
const _UCRDRp = "UpdateContactRoutingDataResponse";
const _UCRn = "UntagContactResponse";
const _UCRp = "UpdateContactRequest";
const _UCRpd = "UpdateContactResponse";
const _UCS = "UpdateContactSchedule";
const _UCSL = "UseCaseSummaryList";
const _UCSR = "UpdateContactScheduleRequest";
const _UCSRp = "UpdateContactScheduleResponse";
const _UCT = "UseCaseType";
const _UCn = "UntagContact";
const _UCp = "UpdateContact";
const _UCs = "UseCase";
const _UD = "UserData";
const _UDF = "UserDataFilters";
const _UDL = "UserDataList";
const _UE = "UrlExpiry";
const _UEAM = "UpdateEmailAddressMetadata";
const _UEAMR = "UpdateEmailAddressMetadataRequest";
const _UEAMRp = "UpdateEmailAddressMetadataResponse";
const _UEF = "UpdateEvaluationForm";
const _UEFR = "UpdateEvaluationFormRequest";
const _UEFRp = "UpdateEvaluationFormResponse";
const _UEIS = "UrlExpiryInSeconds";
const _UHG = "UserHierarchyGroups";
const _UHGL = "UserHierarchyGroupList";
const _UHGSC = "UserHierarchyGroupSearchCriteria";
const _UHGSCL = "UserHierarchyGroupSearchConditionList";
const _UHGSF = "UserHierarchyGroupSearchFilter";
const _UHGSL = "UserHierarchyGroupSummaryList";
const _UHOO = "UpdateHoursOfOperation";
const _UHOOO = "UpdateHoursOfOperationOverride";
const _UHOOOR = "UpdateHoursOfOperationOverrideRequest";
const _UHOOR = "UpdateHoursOfOperationRequest";
const _UI = "UserId";
const _UIA = "UpdateInstanceAttribute";
const _UIAR = "UpdateInstanceAttributeRequest";
const _UII = "UserIdentityInfo";
const _UIIL = "UserIdentityInfoLite";
const _UISC = "UpdateInstanceStorageConfig";
const _UISCR = "UpdateInstanceStorageConfigRequest";
const _UIs = "UserInfo";
const _UIse = "UserIds";
const _UNFE = "UserNotFoundException";
const _UP = "UserProficiencies";
const _UPA = "UpdateParticipantAuthentication";
const _UPAR = "UpdateParticipantAuthenticationRequest";
const _UPARp = "UpdateParticipantAuthenticationResponse";
const _UPARpd = "UpdatePredefinedAttributeRequest";
const _UPAp = "UpdatePredefinedAttribute";
const _UPC = "UserPhoneConfig";
const _UPD = "UserProficiencyDisassociate";
const _UPDL = "UserProficiencyDisassociateList";
const _UPL = "UserProficiencyList";
const _UPN = "UpdatePhoneNumber";
const _UPNM = "UpdatePhoneNumberMetadata";
const _UPNMR = "UpdatePhoneNumberMetadataRequest";
const _UPNR = "UpdatePhoneNumberRequest";
const _UPNRp = "UpdatePhoneNumberResponse";
const _UPR = "UpdatePromptRequest";
const _UPRC = "UpdateParticipantRoleConfig";
const _UPRCCI = "UpdateParticipantRoleConfigChannelInfo";
const _UPRCR = "UpdateParticipantRoleConfigRequest";
const _UPRCRp = "UpdateParticipantRoleConfigResponse";
const _UPRp = "UpdatePromptResponse";
const _UPp = "UpdatePrompt";
const _UPs = "UserProficiency";
const _UQCC = "UserQuickConnectConfig";
const _UQCCR = "UpdateQuickConnectConfigRequest";
const _UQCCp = "UpdateQuickConnectConfig";
const _UQCN = "UpdateQuickConnectName";
const _UQCNR = "UpdateQuickConnectNameRequest";
const _UQHOO = "UpdateQueueHoursOfOperation";
const _UQHOOR = "UpdateQueueHoursOfOperationRequest";
const _UQMC = "UpdateQueueMaxContacts";
const _UQMCR = "UpdateQueueMaxContactsRequest";
const _UQN = "UpdateQueueName";
const _UQNR = "UpdateQueueNameRequest";
const _UQOCC = "UpdateQueueOutboundCallerConfig";
const _UQOCCR = "UpdateQueueOutboundCallerConfigRequest";
const _UQOEC = "UpdateQueueOutboundEmailConfig";
const _UQOECR = "UpdateQueueOutboundEmailConfigRequest";
const _UQS = "UpdateQueueStatus";
const _UQSR = "UpdateQueueStatusRequest";
const _UR = "UrlReference";
const _URPAAT = "UpdateRoutingProfileAgentAvailabilityTimer";
const _URPAATR = "UpdateRoutingProfileAgentAvailabilityTimerRequest";
const _URPC = "UpdateRoutingProfileConcurrency";
const _URPCR = "UpdateRoutingProfileConcurrencyRequest";
const _URPDOQ = "UpdateRoutingProfileDefaultOutboundQueue";
const _URPDOQR = "UpdateRoutingProfileDefaultOutboundQueueRequest";
const _URPN = "UpdateRoutingProfileName";
const _URPNR = "UpdateRoutingProfileNameRequest";
const _URPQ = "UpdateRoutingProfileQueues";
const _URPQR = "UpdateRoutingProfileQueuesRequest";
const _URR = "UntagResourceRequest";
const _URRp = "UpdateRuleRequest";
const _URn = "UntagResource";
const _URp = "UpdateRule";
const _URs = "UserReference";
const _US = "UserSummary";
const _USC = "UserSearchCriteria";
const _USCL = "UserSearchConditionList";
const _USF = "UserSearchFilter";
const _USL = "UserSummaryList";
const _USP = "UpdateSecurityProfile";
const _USPR = "UpdateSecurityProfileRequest";
const _USS = "UserSearchSummary";
const _USSL = "UserSearchSummaryList";
const _UT = "UserTags";
const _UTD = "UpdateTrafficDistribution";
const _UTDR = "UpdateTrafficDistributionRequest";
const _UTDRp = "UpdateTrafficDistributionResponse";
const _UTT = "UpdateTaskTemplate";
const _UTTR = "UpdateTaskTemplateRequest";
const _UTTRp = "UpdateTaskTemplateResponse";
const _UUH = "UpdateUserHierarchy";
const _UUHGN = "UpdateUserHierarchyGroupName";
const _UUHGNR = "UpdateUserHierarchyGroupNameRequest";
const _UUHR = "UpdateUserHierarchyRequest";
const _UUHS = "UpdateUserHierarchyStructure";
const _UUHSR = "UpdateUserHierarchyStructureRequest";
const _UUII = "UpdateUserIdentityInfo";
const _UUIIR = "UpdateUserIdentityInfoRequest";
const _UUM = "UploadUrlMetadata";
const _UUP = "UpdateUserProficiencies";
const _UUPC = "UpdateUserPhoneConfig";
const _UUPCR = "UpdateUserPhoneConfigRequest";
const _UUPR = "UpdateUserProficienciesRequest";
const _UURP = "UpdateUserRoutingProfile";
const _UURPR = "UpdateUserRoutingProfileRequest";
const _UUSP = "UpdateUserSecurityProfiles";
const _UUSPR = "UpdateUserSecurityProfilesRequest";
const _UVC = "UpdateViewContent";
const _UVCR = "UpdateViewContentRequest";
const _UVCRp = "UpdateViewContentResponse";
const _UVM = "UpdateViewMetadata";
const _UVMR = "UpdateViewMetadataRequest";
const _UVMRp = "UpdateViewMetadataResponse";
const _Un = "Unit";
const _Ur = "Url";
const _Us = "User";
const _Use = "Users";
const _V = "Value";
const _VA = "ViewAction";
const _VAa = "ValueArn";
const _VAi = "ViewActions";
const _VAo = "VocabularyArn";
const _VC = "ViewContent";
const _VCS = "ViewContentSha256";
const _VD = "VersionDescription";
const _VI = "VocabularyId";
const _VIC = "ViewInputContent";
const _VIS = "ViewInputSchema";
const _VIa = "ValueInteger";
const _VIi = "ViewId";
const _VL = "ValueList";
const _VM = "ValueMap";
const _VN = "ViewName";
const _VNo = "VocabularyName";
const _VRC = "VoiceRecordingConfiguration";
const _VRT = "VoiceRecordingTrack";
const _VS = "ValueString";
const _VSL = "ViewsSummaryList";
const _VSLo = "VocabularySummaryList";
const _VSi = "ViewSummary";
const _VSo = "VocabularySummary";
const _VV = "ViewVersion";
const _VVS = "ViewVersionSummary";
const _VVSL = "ViewVersionSummaryList";
const _Va = "Values";
const _Ve = "Version";
const _Vi = "View";
const _Vid = "Video";
const _Vo = "Vocabulary";
const _W = "Weight";
const _WI = "WisdomInfo";
const _aQE = "awsQueryError";
const _aRA = "associatedResourceArn";
const _bN = "botName";
const _c = "client";
const _cFT = "contactFlowTypes";
const _cI = "contactId";
const _cT = "clientToken";
const _e = "error";
const _eSN = "eventSourceName";
const _fA = "functionArn";
const _fD = "fromDate";
const _h = "http";
const _hE = "httpError";
const _hQ = "httpQuery";
const _iA = "integrationArn";
const _iI = "instanceId";
const _iT = "idempotencyToken";
const _iTn = "integrationType";
const _k = "key";
const _lR = "lexRegion";
const _lV = "lexVersion";
const _m = "message";
const _mR = "maxResults";
const _n = "name";
const _nT = "nextToken";
const _o = "origin";
const _p = "problems";
const _pNCC = "phoneNumberCountryCodes";
const _pNT = "phoneNumberTypes";
const _pS = "publishStatus";
const _qT = "queueTypes";
const _rA = "resourceArn";
const _rT = "resourceType";
const _rTe = "referenceTypes";
const _s = "server";
const _sV = "snapshotVersion";
const _sm = "smithy.ts.sdk.synthetic.com.amazonaws.connect";
const _st = "state";
const _sta = "status";
const _t = "tags";
const _tD = "toDate";
const _tK = "tagKey";
const _tKCT = "tagKeyComparisonType";
const _tKa = "tagKeys";
const _tV = "tagValue";
const _tVCT = "tagValueComparisonType";
const _ty = "type";
const _uEIS = "urlExpiryInSeconds";
const _v = "version";
const _va = "value";
const n0 = "com.amazonaws.connect";
import { TypeRegistry } from "@smithy/core/schema";
import { ConnectServiceException as __ConnectServiceException } from "../models/ConnectServiceException";
import { AccessDeniedException as __AccessDeniedException, ConditionalOperationFailedException as __ConditionalOperationFailedException, ConflictException as __ConflictException, ContactFlowNotPublishedException as __ContactFlowNotPublishedException, ContactNotFoundException as __ContactNotFoundException, DestinationNotAllowedException as __DestinationNotAllowedException, DuplicateResourceException as __DuplicateResourceException, IdempotencyException as __IdempotencyException, InternalServiceException as __InternalServiceException, InvalidContactFlowException as __InvalidContactFlowException, InvalidContactFlowModuleException as __InvalidContactFlowModuleException, InvalidParameterException as __InvalidParameterException, InvalidRequestException as __InvalidRequestException, LimitExceededException as __LimitExceededException, MaximumResultReturnedException as __MaximumResultReturnedException, OutboundContactNotPermittedException as __OutboundContactNotPermittedException, OutputTypeNotFoundException as __OutputTypeNotFoundException, PropertyValidationException as __PropertyValidationException, ResourceConflictException as __ResourceConflictException, ResourceInUseException as __ResourceInUseException, ResourceNotFoundException as __ResourceNotFoundException, ResourceNotReadyException as __ResourceNotReadyException, ServiceQuotaExceededException as __ServiceQuotaExceededException, ThrottlingException as __ThrottlingException, TooManyRequestsException as __TooManyRequestsException, UserNotFoundException as __UserNotFoundException, } from "../models/errors";
export var AgentFirstName = [0, n0, _AFN, 8, 0];
export var AgentLastName = [0, n0, _ALN, 8, 0];
export var AuthenticationError = [0, n0, _AE, 8, 0];
export var AuthenticationErrorDescription = [0, n0, _AED, 8, 0];
export var AuthorizationCode = [0, n0, _AC, 8, 0];
export var Body = [0, n0, _B, 8, 0];
export var CustomerIdNonEmpty = [0, n0, _CINE, 8, 0];
export var Description = [0, n0, _D, 8, 0];
export var DirectoryAlias = [0, n0, _DA, 8, 0];
export var Email = [0, n0, _E, 8, 0];
export var EmailAddress = [0, n0, _EA, 8, 0];
export var EmailAddressDisplayName = [0, n0, _EADN, 8, 0];
export var InboundSubject = [0, n0, _IS, 8, 0];
export var JoinToken = [0, n0, _JT, 8, 0];
export var Name = [0, n0, _N, 8, 0];
export var OutboundSubject = [0, n0, _OS, 8, 0];
export var Password = [0, n0, _P, 8, 0];
export var SearchableContactAttributeKey = [0, n0, _SCAK, 8, 0];
export var SearchableContactAttributeValue = [0, n0, _SCAV, 8, 0];
export var SearchableSegmentAttributeKey = [0, n0, _SSAK, 8, 0];
export var SearchableSegmentAttributeValue = [0, n0, _SSAV, 8, 0];
export var SearchText = [0, n0, _ST, 8, 0];
export var SecurityToken = [0, n0, _STe, 8, 0];
export var ViewAction = [0, n0, _VA, 8, 0];
export var ViewInputSchema = [0, n0, _VIS, 8, 0];
export var ViewName = [0, n0, _VN, 8, 0];
export var AccessDeniedException = [
    -3,
    n0,
    _ADE,
    {
        [_e]: _c,
        [_hE]: 403,
        [_aQE]: [`AccessDeniedException`, 403],
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(AccessDeniedException, __AccessDeniedException);
export var ActionSummary = [3, n0, _AS, 0, [_AT], [0]];
export var ActivateEvaluationFormRequest = [
    3,
    n0,
    _AEFR,
    0,
    [_II, _EFI, _EFV],
    [[0, 1], [0, 1], 1],
];
export var ActivateEvaluationFormResponse = [3, n0, _AEFRc, 0, [_EFI, _EFA, _EFV], [0, 0, 1]];
export var AdditionalEmailRecipients = [
    3,
    n0,
    _AER,
    0,
    [_TL, _CL],
    [() => EmailRecipientsList, () => EmailRecipientsList],
];
export var AgentConfig = [3, n0, _ACg, 0, [_Di], [() => DistributionList]];
export var AgentContactReference = [
    3,
    n0,
    _ACR,
    0,
    [_CI, _C, _IM, _ACS, _SST, _CTAT, _Q],
    [0, 0, 0, 0, 4, 4, () => QueueReference],
];
export var AgentFirst = [3, n0, _AF, 0, [_Pr], [() => Preview]];
export var AgentHierarchyGroup = [3, n0, _AHG, 0, [_A], [0]];
export var AgentHierarchyGroups = [
    3,
    n0,
    _AHGg,
    0,
    [_LI, _LId, _LIds, _LIds_, _LIds__],
    [64 | 0, 64 | 0, 64 | 0, 64 | 0, 64 | 0],
];
export var AgentInfo = [
    3,
    n0,
    _AI,
    0,
    [_I, _ABAT, _PET, _CTAT, _APDIS, _HG, _DI, _Ca, _ACWD, _ACWST, _ACWET, _AIHD, _STt],
    [
        0,
        4,
        4,
        4,
        1,
        () => HierarchyGroups,
        () => DeviceInfo,
        () => ParticipantCapabilities,
        1,
        4,
        4,
        1,
        () => StateTransitions,
    ],
];
export var AgentQualityMetrics = [3, n0, _AQM, 0, [_Au], [() => AudioQualityMetricsInfo]];
export var AgentsCriteria = [3, n0, _ACge, 0, [_AIg], [64 | 0]];
export var AgentStatus = [
    3,
    n0,
    _ASg,
    0,
    [_ASARN, _ASI, _N, _D, _T, _DO, _S, _Ta, _LMT, _LMR],
    [0, 0, 0, 0, 0, 1, 0, 128 | 0, 4, 0],
];
export var AgentStatusIdentifier = [3, n0, _ASIg, 0, [_A, _I], [0, 0]];
export var AgentStatusReference = [3, n0, _ASR, 0, [_SSTt, _SA, _SN], [4, 0, 0]];
export var AgentStatusSearchCriteria = [
    3,
    n0,
    _ASSC,
    0,
    [_OC, _ACn, _SC],
    [() => AgentStatusSearchConditionList, () => AgentStatusSearchConditionList, () => StringCondition],
];
export var AgentStatusSearchFilter = [
    3,
    n0,
    _ASSF,
    0,
    [_AFt],
    [() => ControlPlaneAttributeFilter],
];
export var AgentStatusSummary = [
    3,
    n0,
    _ASS,
    0,
    [_I, _A, _N, _T, _LMT, _LMR],
    [0, 0, 0, 0, 4, 0],
];
export var AliasConfiguration = [3, n0, _ACl, 0, [_EAI], [0]];
export var AllowedCapabilities = [
    3,
    n0,
    _ACll,
    0,
    [_Cu, _Ag],
    [() => ParticipantCapabilities, () => ParticipantCapabilities],
];
export var AnalyticsDataAssociationResult = [
    3,
    n0,
    _ADAR,
    0,
    [_DSI, _TAI, _RSI, _RSA, _RSS],
    [0, 0, 0, 0, 0],
];
export var AnalyticsDataSetsResult = [3, n0, _ADSR, 0, [_DSI, _DSN], [0, 0]];
export var AnswerMachineDetectionConfig = [3, n0, _AMDC, 0, [_EAMD, _AAMP], [2, 2]];
export var Application = [3, n0, _Ap, 0, [_Na, _AP], [0, 64 | 0]];
export var AssignContactCategoryActionDefinition = [3, n0, _ACCAD, 0, [], []];
export var AssignSlaActionDefinition = [
    3,
    n0,
    _ASAD,
    0,
    [_SAT, _CSC],
    [0, () => CaseSlaConfiguration],
];
export var AssociateAnalyticsDataSetRequest = [
    3,
    n0,
    _AADSR,
    0,
    [_II, _DSI, _TAI],
    [[0, 1], 0, 0],
];
export var AssociateAnalyticsDataSetResponse = [
    3,
    n0,
    _AADSRs,
    0,
    [_DSI, _TAI, _RSI, _RSA],
    [0, 0, 0, 0],
];
export var AssociateApprovedOriginRequest = [
    3,
    n0,
    _AAOR,
    0,
    [_II, _O, _CT],
    [[0, 1], 0, [0, 4]],
];
export var AssociateBotRequest = [
    3,
    n0,
    _ABR,
    0,
    [_II, _LB, _LVB, _CT],
    [[0, 1], () => LexBot, () => LexV2Bot, [0, 4]],
];
export var AssociateContactWithUserRequest = [
    3,
    n0,
    _ACWUR,
    0,
    [_II, _CI, _UI],
    [[0, 1], [0, 1], 0],
];
export var AssociateContactWithUserResponse = [3, n0, _ACWURs, 0, [], []];
export var AssociatedContactSummary = [
    3,
    n0,
    _ACSs,
    0,
    [_CI, _CA, _IT, _DT, _ICI, _PCI, _RCI, _IM, _C],
    [0, 0, 4, 4, 0, 0, 0, 0, 0],
];
export var AssociateDefaultVocabularyRequest = [
    3,
    n0,
    _ADVR,
    0,
    [_II, _LC, _VI],
    [[0, 1], [0, 1], 0],
];
export var AssociateDefaultVocabularyResponse = [3, n0, _ADVRs, 0, [], []];
export var AssociateEmailAddressAliasRequest = [
    3,
    n0,
    _AEAAR,
    0,
    [_EAI, _II, _ACl, _CT],
    [[0, 1], [0, 1], () => AliasConfiguration, [0, 4]],
];
export var AssociateEmailAddressAliasResponse = [3, n0, _AEAARs, 0, [], []];
export var AssociateFlowRequest = [3, n0, _AFR, 0, [_II, _RI, _FI, _RT], [[0, 1], 0, 0, 0]];
export var AssociateFlowResponse = [3, n0, _AFRs, 0, [], []];
export var AssociateInstanceStorageConfigRequest = [
    3,
    n0,
    _AISCR,
    0,
    [_II, _RT, _SCt, _CT],
    [[0, 1], 0, () => InstanceStorageConfig, [0, 4]],
];
export var AssociateInstanceStorageConfigResponse = [3, n0, _AISCRs, 0, [_AIs], [0]];
export var AssociateLambdaFunctionRequest = [
    3,
    n0,
    _ALFR,
    0,
    [_II, _FA, _CT],
    [[0, 1], 0, [0, 4]],
];
export var AssociateLexBotRequest = [
    3,
    n0,
    _ALBR,
    0,
    [_II, _LB, _CT],
    [[0, 1], () => LexBot, [0, 4]],
];
export var AssociatePhoneNumberContactFlowRequest = [
    3,
    n0,
    _APNCFR,
    0,
    [_PNI, _II, _CFI],
    [[0, 1], 0, 0],
];
export var AssociateQueueQuickConnectsRequest = [
    3,
    n0,
    _AQQCR,
    0,
    [_II, _QI, _QCI],
    [[0, 1], [0, 1], 64 | 0],
];
export var AssociateRoutingProfileQueuesRequest = [
    3,
    n0,
    _ARPQR,
    0,
    [_II, _RPI, _QC, _MAQC],
    [[0, 1], [0, 1], () => RoutingProfileQueueConfigList, () => RoutingProfileManualAssignmentQueueConfigList],
];
export var AssociateSecurityKeyRequest = [3, n0, _ASKR, 0, [_II, _K, _CT], [[0, 1], 0, [0, 4]]];
export var AssociateSecurityKeyResponse = [3, n0, _ASKRs, 0, [_AIs], [0]];
export var AssociateTrafficDistributionGroupUserRequest = [
    3,
    n0,
    _ATDGUR,
    0,
    [_TDGI, _UI, _II],
    [[0, 1], 0, 0],
];
export var AssociateTrafficDistributionGroupUserResponse = [3, n0, _ATDGURs, 0, [], []];
export var AssociateUserProficienciesRequest = [
    3,
    n0,
    _AUPR,
    0,
    [_II, _UI, _UP],
    [[0, 1], [0, 1], () => UserProficiencyList],
];
export var AttachedFile = [
    3,
    n0,
    _AFtt,
    0,
    [_CTr, _FAi, _FIi, _FN, _FSIB, _FS, _CB, _FUCT, _ARA, _Ta],
    [0, 0, 0, 0, 1, 0, () => CreatedByInfo, 0, 0, 128 | 0],
];
export var AttachedFileError = [3, n0, _AFE, 0, [_EC, _EM, _FIi], [0, 0, 0]];
export var AttachmentReference = [3, n0, _AR, 0, [_N, _V, _St, _A], [0, 0, 0, 0]];
export var Attendee = [3, n0, _At, 0, [_AIt, _JT], [0, [() => JoinToken, 0]]];
export var Attribute = [3, n0, _Att, 0, [_ATt, _V], [0, 0]];
export var AttributeAndCondition = [
    3,
    n0,
    _AAC,
    0,
    [_TC, _HGC],
    [() => TagAndConditionList, () => HierarchyGroupCondition],
];
export var AttributeCondition = [
    3,
    n0,
    _ACt,
    0,
    [_N, _V, _PL, _R, _MC, _CO],
    [0, 0, 1, () => Range, () => MatchCriteria, 0],
];
export var AudioFeatures = [3, n0, _AFu, 0, [_ER], [0]];
export var AudioQualityMetricsInfo = [3, n0, _AQMI, 0, [_QS, _PQI], [1, 64 | 0]];
export var AuthenticationProfile = [
    3,
    n0,
    _APu,
    0,
    [_I, _A, _N, _D, _AIl, _BI, _ID, _CTre, _LMT, _LMR, _PSD, _MSD, _SID, _SIHE],
    [0, 0, 0, 0, 64 | 0, 64 | 0, 2, 4, 4, 0, 1, 1, 1, 2],
];
export var AuthenticationProfileSummary = [
    3,
    n0,
    _APS,
    0,
    [_I, _A, _N, _ID, _LMT, _LMR],
    [0, 0, 0, 2, 4, 0],
];
export var AutoEvaluationConfiguration = [3, n0, _AEC, 0, [_En], [2]];
export var AutoEvaluationDetails = [3, n0, _AEDu, 0, [_AEE, _AES], [2, 0]];
export var AutomaticFailConfiguration = [3, n0, _AFC, 0, [_TS], [0]];
export var AvailableNumberSummary = [3, n0, _ANS, 0, [_PN, _PNCC, _PNT], [0, 0, 0]];
export var BatchAssociateAnalyticsDataSetRequest = [
    3,
    n0,
    _BAADSR,
    0,
    [_II, _DSIa, _TAI],
    [[0, 1], 64 | 0, 0],
];
export var BatchAssociateAnalyticsDataSetResponse = [
    3,
    n0,
    _BAADSRa,
    0,
    [_Cr, _Er],
    [() => AnalyticsDataAssociationResults, () => ErrorResults],
];
export var BatchDisassociateAnalyticsDataSetRequest = [
    3,
    n0,
    _BDADSR,
    0,
    [_II, _DSIa, _TAI],
    [[0, 1], 64 | 0, 0],
];
export var BatchDisassociateAnalyticsDataSetResponse = [
    3,
    n0,
    _BDADSRa,
    0,
    [_De, _Er],
    [64 | 0, () => ErrorResults],
];
export var BatchGetAttachedFileMetadataRequest = [
    3,
    n0,
    _BGAFMR,
    0,
    [_FIil, _II, _ARA],
    [
        64 | 0,
        [0, 1],
        [
            0,
            {
                [_hQ]: _aRA,
            },
        ],
    ],
];
export var BatchGetAttachedFileMetadataResponse = [
    3,
    n0,
    _BGAFMRa,
    0,
    [_F, _Er],
    [() => AttachedFilesList, () => AttachedFileErrorsList],
];
export var BatchGetFlowAssociationRequest = [
    3,
    n0,
    _BGFAR,
    0,
    [_II, _RIe, _RT],
    [[0, 1], 64 | 0, 0],
];
export var BatchGetFlowAssociationResponse = [
    3,
    n0,
    _BGFARa,
    0,
    [_FASL],
    [() => FlowAssociationSummaryList],
];
export var BatchPutContactRequest = [
    3,
    n0,
    _BPCR,
    0,
    [_CT, _II, _CDRL],
    [[0, 4], [0, 1], () => ContactDataRequestList],
];
export var BatchPutContactResponse = [
    3,
    n0,
    _BPCRa,
    0,
    [_SRL, _FRL],
    [() => SuccessfulRequestList, () => FailedRequestList],
];
export var BooleanCondition = [3, n0, _BC, 0, [_FNi, _CTo], [0, 0]];
export var Campaign = [3, n0, _Cam, 0, [_CIa], [0]];
export var CaseSlaConfiguration = [
    3,
    n0,
    _CSC,
    0,
    [_N, _T, _FIie, _TFV, _TSM],
    [0, 0, 0, () => SlaFieldValueUnionList, 1],
];
export var ChatContactMetrics = [
    3,
    n0,
    _CCM,
    0,
    [_MP, _TM, _TBM, _TBMLIC, _CCTIM, _CTC, _AFRT, _AFRTIM],
    [2, 1, 1, 1, 1, 1, 4, 1],
];
export var ChatEvent = [3, n0, _CE, 0, [_T, _CTon, _Co], [0, 0, 0]];
export var ChatMessage = [3, n0, _CM, 0, [_CTon, _Co], [0, 0]];
export var ChatMetrics = [
    3,
    n0,
    _CMh,
    0,
    [_CCM, _AM, _CMu],
    [() => ChatContactMetrics, () => ParticipantMetrics, () => ParticipantMetrics],
];
export var ChatParticipantRoleConfig = [
    3,
    n0,
    _CPRC,
    0,
    [_PTCL],
    [() => ParticipantTimerConfigList],
];
export var ChatStreamingConfiguration = [3, n0, _CSCh, 0, [_SEA], [0]];
export var ClaimedPhoneNumberSummary = [
    3,
    n0,
    _CPNS,
    0,
    [_PNI, _PNA, _PN, _PNCC, _PNT, _PND, _TA, _II, _Ta, _PNS, _SPNA],
    [0, 0, 0, 0, 0, 0, 0, 0, 128 | 0, () => PhoneNumberStatus, 0],
];
export var ClaimPhoneNumberRequest = [
    3,
    n0,
    _CPNR,
    0,
    [_TA, _II, _PN, _PND, _Ta, _CT],
    [0, 0, 0, 0, 128 | 0, [0, 4]],
];
export var ClaimPhoneNumberResponse = [3, n0, _CPNRl, 0, [_PNI, _PNA], [0, 0]];
export var CommonAttributeAndCondition = [3, n0, _CAAC, 0, [_TC], [() => TagAndConditionList]];
export var CompleteAttachedFileUploadRequest = [
    3,
    n0,
    _CAFUR,
    0,
    [_II, _FIi, _ARA],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _aRA,
            },
        ],
    ],
];
export var CompleteAttachedFileUploadResponse = [3, n0, _CAFURo, 0, [], []];
export var Condition = [
    3,
    n0,
    _Con,
    0,
    [_SC, _NC],
    [() => StringCondition, () => NumberCondition],
];
export var ConditionalOperationFailedException = [
    -3,
    n0,
    _COFE,
    {
        [_e]: _c,
        [_hE]: 409,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(ConditionalOperationFailedException, __ConditionalOperationFailedException);
export var ConflictException = [
    -3,
    n0,
    _CEo,
    {
        [_e]: _c,
        [_hE]: 409,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(ConflictException, __ConflictException);
export var ConnectionData = [3, n0, _CD, 0, [_At, _Me], [[() => Attendee, 0], () => Meeting]];
export var Contact = [
    3,
    n0,
    _Cont,
    0,
    [
        _A,
        _I,
        _ICI,
        _PCI,
        _CAI,
        _IM,
        _N,
        _D,
        _C,
        _QIu,
        _AI,
        _IT,
        _DT,
        _LUT,
        _LPT,
        _LRT,
        _RST,
        _TPC,
        _TPDIS,
        _STc,
        _RCI,
        _WI,
        _CIu,
        _CEu,
        _SE,
        _QTAS,
        _QP,
        _Ta,
        _CTST,
        _RC,
        _Cu,
        _Cam,
        _AMDS,
        _CVA,
        _QM,
        _CMh,
        _DD,
        _AER,
        _SAe,
        _Re,
        _DR,
        _CEon,
        _TTI,
        _CDo,
        _OSu,
        _Attr,
    ],
    [
        0,
        0,
        0,
        0,
        0,
        0,
        [() => Name, 0],
        [() => Description, 0],
        0,
        () => QueueInfo,
        () => AgentInfo,
        4,
        4,
        4,
        4,
        4,
        4,
        1,
        1,
        4,
        0,
        () => WisdomInfo,
        0,
        () => EndpointInfo,
        () => EndpointInfo,
        1,
        1,
        128 | 0,
        4,
        () => RoutingCriteria,
        () => Customer,
        () => Campaign,
        0,
        () => CustomerVoiceActivity,
        () => QualityMetrics,
        () => ChatMetrics,
        () => DisconnectDetails,
        () => AdditionalEmailRecipients,
        () => SegmentAttributes,
        () => Recordings,
        0,
        () => ContactEvaluations,
        () => TaskTemplateInfoV2,
        () => ContactDetails,
        () => OutboundStrategy,
        128 | 0,
    ],
];
export var ContactAnalysis = [3, n0, _CAo, 0, [_Tr], [[() => Transcript, 0]]];
export var ContactConfiguration = [3, n0, _CC, 0, [_CI, _PR, _IRM], [0, 0, 2]];
export var ContactDataRequest = [
    3,
    n0,
    _CDR,
    0,
    [_SE, _CEu, _RIeq, _QI, _Attr, _Cam, _OSu],
    [() => Endpoint, () => Endpoint, 0, 0, 128 | 0, () => Campaign, () => OutboundStrategy],
];
export var ContactDetails = [3, n0, _CDo, 0, [_N, _D], [0, 0]];
export var ContactEvaluation = [
    3,
    n0,
    _CEont,
    0,
    [_FIo, _EAv, _St, _STta, _ET, _DTe, _EL],
    [0, 0, 0, 4, 4, 4, 0],
];
export var ContactFilter = [3, n0, _CF, 0, [_CS], [64 | 0]];
export var ContactFlow = [
    3,
    n0,
    _CFo,
    0,
    [_A, _I, _N, _T, _S, _St, _D, _Co, _Ta, _FCS, _Ve, _VD, _LMT, _LMR],
    [0, 0, 0, 0, 0, 0, 0, 0, 128 | 0, 0, 1, 0, 4, 0],
];
export var ContactFlowModule = [
    3,
    n0,
    _CFM,
    0,
    [_A, _I, _N, _Co, _D, _S, _St, _Ta, _FMCS, _Ve, _VD, _Se, _EIC],
    [0, 0, 0, 0, 0, 0, 0, 128 | 0, 0, 1, 0, 0, () => ExternalInvocationConfiguration],
];
export var ContactFlowModuleAliasInfo = [
    3,
    n0,
    _CFMAI,
    0,
    [_CFMI, _CFMA, _AIli, _Ve, _N, _D, _LMR, _LMT],
    [0, 0, 0, 1, 0, 0, 0, 4],
];
export var ContactFlowModuleAliasSummary = [
    3,
    n0,
    _CFMAS,
    0,
    [_A, _AIli, _Ve, _AN, _AD, _LMT],
    [0, 0, 1, 0, 0, 4],
];
export var ContactFlowModuleSearchCriteria = [
    3,
    n0,
    _CFMSC,
    0,
    [_OC, _ACn, _SC, _SCta, _SCtat],
    [() => ContactFlowModuleSearchConditionList, () => ContactFlowModuleSearchConditionList, () => StringCondition, 0, 0],
];
export var ContactFlowModuleSearchFilter = [
    3,
    n0,
    _CFMSF,
    0,
    [_TF],
    [() => ControlPlaneTagFilter],
];
export var ContactFlowModuleSummary = [3, n0, _CFMS, 0, [_I, _A, _N, _S], [0, 0, 0, 0]];
export var ContactFlowModuleVersionSummary = [3, n0, _CFMVS, 0, [_A, _VD, _Ve], [0, 0, 1]];
export var ContactFlowNotPublishedException = [
    -3,
    n0,
    _CFNPE,
    {
        [_e]: _c,
        [_hE]: 404,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(ContactFlowNotPublishedException, __ContactFlowNotPublishedException);
export var ContactFlowSearchCriteria = [
    3,
    n0,
    _CFSC,
    0,
    [_OC, _ACn, _SC, _TCy, _SCta, _SCtat],
    [() => ContactFlowSearchConditionList, () => ContactFlowSearchConditionList, () => StringCondition, 0, 0, 0],
];
export var ContactFlowSearchFilter = [3, n0, _CFSF, 0, [_TF], [() => ControlPlaneTagFilter]];
export var ContactFlowSummary = [
    3,
    n0,
    _CFS,
    0,
    [_I, _A, _N, _CFT, _CFSo, _CFSon],
    [0, 0, 0, 0, 0, 0],
];
export var ContactFlowVersionSummary = [3, n0, _CFVS, 0, [_A, _VD, _Ve], [0, 0, 1]];
export var ContactMetricInfo = [3, n0, _CMI, 0, [_N], [0]];
export var ContactMetricResult = [3, n0, _CMR, 0, [_N, _V], [0, () => ContactMetricValue]];
export var ContactNotFoundException = [
    -3,
    n0,
    _CNFE,
    {
        [_e]: _c,
        [_hE]: 410,
        [_aQE]: [`ContactNotFoundException`, 410],
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(ContactNotFoundException, __ContactNotFoundException);
export var ContactSearchSummary = [
    3,
    n0,
    _CSS,
    0,
    [_A, _I, _ICI, _PCI, _IM, _C, _QIu, _AI, _IT, _DT, _STc, _SAe, _N, _RC],
    [
        0,
        0,
        0,
        0,
        0,
        0,
        () => ContactSearchSummaryQueueInfo,
        () => ContactSearchSummaryAgentInfo,
        4,
        4,
        4,
        [() => ContactSearchSummarySegmentAttributes, 0],
        [() => Name, 0],
        () => RoutingCriteria,
    ],
];
export var ContactSearchSummaryAgentInfo = [3, n0, _CSSAI, 0, [_I, _CTAT], [0, 4]];
export var ContactSearchSummaryQueueInfo = [3, n0, _CSSQI, 0, [_I, _ETn], [0, 4]];
export var ContactSearchSummarySegmentAttributeValue = [
    3,
    n0,
    _CSSSAV,
    0,
    [_VS, _VM],
    [0, () => SegmentAttributeValueMap],
];
export var ControlPlaneAttributeFilter = [
    3,
    n0,
    _CPAF,
    0,
    [_OC, _ACnd, _TCa],
    [() => CommonAttributeOrConditionList, () => CommonAttributeAndCondition, () => TagCondition],
];
export var ControlPlaneTagFilter = [
    3,
    n0,
    _CPTF,
    0,
    [_OC, _ACn, _TCa],
    [() => TagOrConditionList, () => TagAndConditionList, () => TagCondition],
];
export var ControlPlaneUserAttributeFilter = [
    3,
    n0,
    _CPUAF,
    0,
    [_OC, _ACnd, _TCa, _HGC],
    [() => AttributeOrConditionList, () => AttributeAndCondition, () => TagCondition, () => HierarchyGroupCondition],
];
export var CreateAgentStatusRequest = [
    3,
    n0,
    _CASR,
    0,
    [_II, _N, _D, _S, _DO, _Ta],
    [[0, 1], 0, 0, 0, 1, 128 | 0],
];
export var CreateAgentStatusResponse = [3, n0, _CASRr, 0, [_ASARN, _ASI], [0, 0]];
export var CreateCaseActionDefinition = [3, n0, _CCAD, 0, [_Fi, _TI], [() => FieldValues, 0]];
export var CreateContactFlowModuleAliasRequest = [
    3,
    n0,
    _CCFMAR,
    0,
    [_II, _D, _CFMI, _CFMV, _AN],
    [[0, 1], 0, [0, 1], 1, 0],
];
export var CreateContactFlowModuleAliasResponse = [3, n0, _CCFMARr, 0, [_CFMA, _I], [0, 0]];
export var CreateContactFlowModuleRequest = [
    3,
    n0,
    _CCFMR,
    0,
    [_II, _N, _D, _Co, _Ta, _CT, _Se, _EIC],
    [[0, 1], 0, 0, 0, 128 | 0, [0, 4], 0, () => ExternalInvocationConfiguration],
];
export var CreateContactFlowModuleResponse = [3, n0, _CCFMRr, 0, [_I, _A], [0, 0]];
export var CreateContactFlowModuleVersionRequest = [
    3,
    n0,
    _CCFMVR,
    0,
    [_II, _D, _CFMI, _FMCS],
    [[0, 1], 0, [0, 1], 0],
];
export var CreateContactFlowModuleVersionResponse = [3, n0, _CCFMVRr, 0, [_CFMA, _Ve], [0, 1]];
export var CreateContactFlowRequest = [
    3,
    n0,
    _CCFR,
    0,
    [_II, _N, _T, _D, _Co, _St, _Ta],
    [[0, 1], 0, 0, 0, 0, 0, 128 | 0],
];
export var CreateContactFlowResponse = [3, n0, _CCFRr, 0, [_CFI, _CFA, _FCS], [0, 0, 0]];
export var CreateContactFlowVersionRequest = [
    3,
    n0,
    _CCFVR,
    0,
    [_II, _D, _CFI, _FCS, _CFV, _LMT, _LMR],
    [[0, 1], 0, [0, 1], 0, 1, 4, 0],
];
export var CreateContactFlowVersionResponse = [3, n0, _CCFVRr, 0, [_CFA, _Ve], [0, 1]];
export var CreateContactRequest = [
    3,
    n0,
    _CCR,
    0,
    [_II, _CT, _RCI, _Attr, _Ref, _C, _IM, _EDIM, _UIs, _IA, _N, _D, _SAe, _PCI],
    [
        0,
        [0, 4],
        0,
        128 | 0,
        () => ContactReferences,
        0,
        0,
        1,
        () => UserInfo,
        0,
        [() => Name, 0],
        [() => Description, 0],
        () => SegmentAttributes,
        0,
    ],
];
export var CreateContactResponse = [3, n0, _CCRr, 0, [_CI, _CA], [0, 0]];
export var CreateEmailAddressRequest = [
    3,
    n0,
    _CEAR,
    0,
    [_D, _II, _EA, _DN, _Ta, _CT],
    [[() => Description, 0], [0, 1], [() => EmailAddress, 0], [() => EmailAddressDisplayName, 0], 128 | 0, 0],
];
export var CreateEmailAddressResponse = [3, n0, _CEARr, 0, [_EAI, _EAA], [0, 0]];
export var CreateEvaluationFormRequest = [
    3,
    n0,
    _CEFR,
    0,
    [_II, _Ti, _D, _It, _SS, _AEC, _CT, _Ta],
    [
        [0, 1],
        0,
        0,
        () => EvaluationFormItemsList,
        () => EvaluationFormScoringStrategy,
        () => EvaluationFormAutoEvaluationConfiguration,
        [0, 4],
        128 | 0,
    ],
];
export var CreateEvaluationFormResponse = [3, n0, _CEFRr, 0, [_EFI, _EFA], [0, 0]];
export var CreateHoursOfOperationOverrideRequest = [
    3,
    n0,
    _CHOOOR,
    0,
    [_II, _HOOI, _N, _D, _Conf, _EF, _ETf],
    [[0, 1], [0, 1], 0, 0, () => HoursOfOperationOverrideConfigList, 0, 0],
];
export var CreateHoursOfOperationOverrideResponse = [3, n0, _CHOOORr, 0, [_HOOOI], [0]];
export var CreateHoursOfOperationRequest = [
    3,
    n0,
    _CHOOR,
    0,
    [_II, _N, _D, _TZ, _Conf, _Ta],
    [[0, 1], 0, 0, 0, () => HoursOfOperationConfigList, 128 | 0],
];
export var CreateHoursOfOperationResponse = [3, n0, _CHOORr, 0, [_HOOI, _HOOA], [0, 0]];
export var CreateInstanceRequest = [
    3,
    n0,
    _CIR,
    0,
    [_CT, _IMT, _IAn, _DIi, _ICE, _OCE, _Ta],
    [[0, 4], 0, [() => DirectoryAlias, 0], 0, 2, 2, 128 | 0],
];
export var CreateInstanceResponse = [3, n0, _CIRr, 0, [_I, _A], [0, 0]];
export var CreateIntegrationAssociationRequest = [
    3,
    n0,
    _CIAR,
    0,
    [_II, _ITn, _IAnt, _SAU, _SAN, _STo, _Ta],
    [[0, 1], 0, 0, 0, 0, 0, 128 | 0],
];
export var CreateIntegrationAssociationResponse = [3, n0, _CIARr, 0, [_IAI, _IAA], [0, 0]];
export var CreateParticipantRequest = [
    3,
    n0,
    _CPR,
    0,
    [_II, _CI, _CT, _PD],
    [0, 0, [0, 4], () => ParticipantDetailsToAdd],
];
export var CreateParticipantResponse = [
    3,
    n0,
    _CPRr,
    0,
    [_PC, _PI],
    [() => ParticipantTokenCredentials, 0],
];
export var CreatePersistentContactAssociationRequest = [
    3,
    n0,
    _CPCAR,
    0,
    [_II, _ICI, _RTe, _SCI, _CT],
    [[0, 1], [0, 1], 0, 0, 0],
];
export var CreatePersistentContactAssociationResponse = [3, n0, _CPCARr, 0, [_CFCI], [0]];
export var CreatePredefinedAttributeRequest = [
    3,
    n0,
    _CPAR,
    0,
    [_II, _N, _Va, _Pu, _ACtt],
    [[0, 1], 0, () => PredefinedAttributeValues, 64 | 0, () => InputPredefinedAttributeConfiguration],
];
export var CreatePromptRequest = [
    3,
    n0,
    _CPRre,
    0,
    [_II, _N, _D, _SU, _Ta],
    [[0, 1], 0, 0, 0, 128 | 0],
];
export var CreatePromptResponse = [3, n0, _CPRrea, 0, [_PARN, _PIr], [0, 0]];
export var CreatePushNotificationRegistrationRequest = [
    3,
    n0,
    _CPNRR,
    0,
    [_II, _CT, _PAA, _DTev, _DTevi, _CC],
    [[0, 1], [0, 4], 0, 0, 0, () => ContactConfiguration],
];
export var CreatePushNotificationRegistrationResponse = [3, n0, _CPNRRr, 0, [_RIeg], [0]];
export var CreateQueueRequest = [
    3,
    n0,
    _CQR,
    0,
    [_II, _N, _D, _OCC, _OEC, _HOOI, _MCa, _QCI, _Ta],
    [[0, 1], 0, 0, () => OutboundCallerConfig, () => OutboundEmailConfig, 0, 1, 64 | 0, 128 | 0],
];
export var CreateQueueResponse = [3, n0, _CQRr, 0, [_QA, _QI], [0, 0]];
export var CreateQuickConnectRequest = [
    3,
    n0,
    _CQCR,
    0,
    [_II, _N, _D, _QCC, _Ta],
    [[0, 1], 0, 0, () => QuickConnectConfig, 128 | 0],
];
export var CreateQuickConnectResponse = [3, n0, _CQCRr, 0, [_QCARN, _QCIu], [0, 0]];
export var CreateRoutingProfileRequest = [
    3,
    n0,
    _CRPR,
    0,
    [_II, _N, _D, _DOQI, _QC, _MAQC, _MCe, _Ta, _AAT],
    [
        [0, 1],
        0,
        0,
        0,
        () => RoutingProfileQueueConfigList,
        () => RoutingProfileManualAssignmentQueueConfigList,
        () => MediaConcurrencies,
        128 | 0,
        0,
    ],
];
export var CreateRoutingProfileResponse = [3, n0, _CRPRr, 0, [_RPA, _RPI], [0, 0]];
export var CreateRuleRequest = [
    3,
    n0,
    _CRR,
    0,
    [_II, _N, _TES, _Fu, _Ac, _PS, _CT],
    [[0, 1], 0, () => RuleTriggerEventSource, 0, () => RuleActions, 0, [0, 4]],
];
export var CreateRuleResponse = [3, n0, _CRRr, 0, [_RA, _RIu], [0, 0]];
export var CreateSecurityProfileRequest = [
    3,
    n0,
    _CSPR,
    0,
    [_SPN, _D, _Pe, _II, _Ta, _AACT, _TRR, _App, _HRR, _AACHGI],
    [0, 0, 64 | 0, [0, 1], 128 | 0, 128 | 0, 64 | 0, () => Applications, 64 | 0, 0],
];
export var CreateSecurityProfileResponse = [3, n0, _CSPRr, 0, [_SPI, _SPA], [0, 0]];
export var CreateTaskTemplateRequest = [
    3,
    n0,
    _CTTR,
    0,
    [_II, _N, _D, _CFI, _SAFI, _Cons, _Def, _St, _Fi, _CT],
    [[0, 1], 0, 0, 0, 0, () => TaskTemplateConstraints, () => TaskTemplateDefaults, 0, () => TaskTemplateFields, [0, 4]],
];
export var CreateTaskTemplateResponse = [3, n0, _CTTRr, 0, [_I, _A], [0, 0]];
export var CreateTrafficDistributionGroupRequest = [
    3,
    n0,
    _CTDGR,
    0,
    [_N, _D, _II, _CT, _Ta],
    [0, 0, 0, [0, 4], 128 | 0],
];
export var CreateTrafficDistributionGroupResponse = [3, n0, _CTDGRr, 0, [_I, _A], [0, 0]];
export var CreateUseCaseRequest = [
    3,
    n0,
    _CUCR,
    0,
    [_II, _IAI, _UCT, _Ta],
    [[0, 1], [0, 1], 0, 128 | 0],
];
export var CreateUseCaseResponse = [3, n0, _CUCRr, 0, [_UCI, _UCA], [0, 0]];
export var CreateUserHierarchyGroupRequest = [
    3,
    n0,
    _CUHGR,
    0,
    [_N, _PGI, _II, _Ta],
    [0, 0, [0, 1], 128 | 0],
];
export var CreateUserHierarchyGroupResponse = [3, n0, _CUHGRr, 0, [_HGI, _HGA], [0, 0]];
export var CreateUserRequest = [
    3,
    n0,
    _CUR,
    0,
    [_U, _P, _IId, _PCh, _DUI, _SPIe, _RPI, _HGI, _II, _Ta],
    [0, [() => Password, 0], [() => UserIdentityInfo, 0], () => UserPhoneConfig, 0, 64 | 0, 0, 0, [0, 1], 128 | 0],
];
export var CreateUserResponse = [3, n0, _CURr, 0, [_UI, _UA], [0, 0]];
export var CreateViewRequest = [
    3,
    n0,
    _CVR,
    0,
    [_II, _CT, _St, _Co, _D, _N, _Ta],
    [[0, 1], 0, 0, [() => ViewInputContent, 0], 0, [() => ViewName, 0], 128 | 0],
];
export var CreateViewResponse = [3, n0, _CVRr, 0, [_Vi], [[() => View, 0]]];
export var CreateViewVersionRequest = [
    3,
    n0,
    _CVVR,
    0,
    [_II, _VIi, _VD, _VCS],
    [[0, 1], [0, 1], 0, 0],
];
export var CreateViewVersionResponse = [3, n0, _CVVRr, 0, [_Vi], [[() => View, 0]]];
export var CreateVocabularyRequest = [
    3,
    n0,
    _CVRre,
    0,
    [_CT, _II, _VNo, _LC, _Co, _Ta],
    [[0, 4], [0, 1], 0, 0, 0, 128 | 0],
];
export var CreateVocabularyResponse = [3, n0, _CVRrea, 0, [_VAo, _VI, _S], [0, 0, 0]];
export var Credentials = [
    3,
    n0,
    _Cre,
    8,
    [_ATc, _ATE, _RTef, _RTE],
    [[() => SecurityToken, 0], 4, [() => SecurityToken, 0], 4],
];
export var CrossChannelBehavior = [3, n0, _CCB, 0, [_BT], [0]];
export var CurrentMetric = [3, n0, _CMur, 0, [_N, _Un], [0, 0]];
export var CurrentMetricData = [3, n0, _CMD, 0, [_Met, _V], [() => CurrentMetric, 1]];
export var CurrentMetricResult = [
    3,
    n0,
    _CMRu,
    0,
    [_Dim, _Col],
    [() => Dimensions, () => CurrentMetricDataCollections],
];
export var CurrentMetricSortCriteria = [3, n0, _CMSC, 0, [_SBM, _SO], [0, 0]];
export var Customer = [
    3,
    n0,
    _Cu,
    0,
    [_DI, _Ca],
    [() => DeviceInfo, () => ParticipantCapabilities],
];
export var CustomerQualityMetrics = [3, n0, _CQM, 0, [_Au], [() => AudioQualityMetricsInfo]];
export var CustomerVoiceActivity = [3, n0, _CVA, 0, [_GST, _GET], [4, 4]];
export var DateCondition = [3, n0, _DC, 0, [_FNi, _V, _CTo], [0, 0, 0]];
export var DateReference = [3, n0, _DRa, 0, [_N, _V], [0, 0]];
export var DateTimeCondition = [3, n0, _DTC, 0, [_FNi, _MV, _MVa, _CTo], [0, 0, 0, 0]];
export var DeactivateEvaluationFormRequest = [
    3,
    n0,
    _DEFR,
    0,
    [_II, _EFI, _EFV],
    [[0, 1], [0, 1], 1],
];
export var DeactivateEvaluationFormResponse = [3, n0, _DEFRe, 0, [_EFI, _EFA, _EFV], [0, 0, 1]];
export var DecimalCondition = [3, n0, _DCe, 0, [_FNi, _MV, _MVa, _CTo], [0, 1, 1, 0]];
export var DefaultVocabulary = [3, n0, _DV, 0, [_II, _LC, _VI, _VNo], [0, 0, 0, 0]];
export var DeleteAttachedFileRequest = [
    3,
    n0,
    _DAFR,
    0,
    [_II, _FIi, _ARA],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _aRA,
            },
        ],
    ],
];
export var DeleteAttachedFileResponse = [3, n0, _DAFRe, 0, [], []];
export var DeleteContactEvaluationRequest = [
    3,
    n0,
    _DCER,
    0,
    [_II, _EI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteContactFlowModuleAliasRequest = [
    3,
    n0,
    _DCFMAR,
    0,
    [_II, _CFMI, _AIli],
    [
        [0, 1],
        [0, 1],
        [0, 1],
    ],
];
export var DeleteContactFlowModuleAliasResponse = [3, n0, _DCFMARe, 0, [], []];
export var DeleteContactFlowModuleRequest = [
    3,
    n0,
    _DCFMR,
    0,
    [_II, _CFMI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteContactFlowModuleResponse = [3, n0, _DCFMRe, 0, [], []];
export var DeleteContactFlowModuleVersionRequest = [
    3,
    n0,
    _DCFMVR,
    0,
    [_II, _CFMI, _CFMV],
    [
        [0, 1],
        [0, 1],
        [1, 1],
    ],
];
export var DeleteContactFlowModuleVersionResponse = [3, n0, _DCFMVRe, 0, [], []];
export var DeleteContactFlowRequest = [
    3,
    n0,
    _DCFR,
    0,
    [_II, _CFI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteContactFlowResponse = [3, n0, _DCFRe, 0, [], []];
export var DeleteContactFlowVersionRequest = [
    3,
    n0,
    _DCFVR,
    0,
    [_II, _CFI, _CFV],
    [
        [0, 1],
        [0, 1],
        [1, 1],
    ],
];
export var DeleteContactFlowVersionResponse = [3, n0, _DCFVRe, 0, [], []];
export var DeleteEmailAddressRequest = [
    3,
    n0,
    _DEAR,
    0,
    [_II, _EAI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteEmailAddressResponse = [3, n0, _DEARe, 0, [], []];
export var DeleteEvaluationFormRequest = [
    3,
    n0,
    _DEFRel,
    0,
    [_II, _EFI, _EFV],
    [
        [0, 1],
        [0, 1],
        [
            1,
            {
                [_hQ]: _v,
            },
        ],
    ],
];
export var DeleteHoursOfOperationOverrideRequest = [
    3,
    n0,
    _DHOOOR,
    0,
    [_II, _HOOI, _HOOOI],
    [
        [0, 1],
        [0, 1],
        [0, 1],
    ],
];
export var DeleteHoursOfOperationRequest = [
    3,
    n0,
    _DHOOR,
    0,
    [_II, _HOOI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteInstanceRequest = [
    3,
    n0,
    _DIR,
    0,
    [_II, _CT],
    [
        [0, 1],
        [
            0,
            {
                [_iT]: 1,
                [_hQ]: _cT,
            },
        ],
    ],
];
export var DeleteIntegrationAssociationRequest = [
    3,
    n0,
    _DIAR,
    0,
    [_II, _IAI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeletePredefinedAttributeRequest = [
    3,
    n0,
    _DPAR,
    0,
    [_II, _N],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeletePromptRequest = [
    3,
    n0,
    _DPR,
    0,
    [_II, _PIr],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeletePushNotificationRegistrationRequest = [
    3,
    n0,
    _DPNRR,
    0,
    [_II, _RIeg, _CI],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _cI,
            },
        ],
    ],
];
export var DeletePushNotificationRegistrationResponse = [3, n0, _DPNRRe, 0, [], []];
export var DeleteQueueRequest = [
    3,
    n0,
    _DQR,
    0,
    [_II, _QI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteQuickConnectRequest = [
    3,
    n0,
    _DQCR,
    0,
    [_II, _QCIu],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteRoutingProfileRequest = [
    3,
    n0,
    _DRPR,
    0,
    [_II, _RPI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteRuleRequest = [
    3,
    n0,
    _DRR,
    0,
    [_II, _RIu],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteSecurityProfileRequest = [
    3,
    n0,
    _DSPR,
    0,
    [_II, _SPI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteTaskTemplateRequest = [
    3,
    n0,
    _DTTR,
    0,
    [_II, _TTIa],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteTaskTemplateResponse = [3, n0, _DTTRe, 0, [], []];
export var DeleteTrafficDistributionGroupRequest = [3, n0, _DTDGR, 0, [_TDGI], [[0, 1]]];
export var DeleteTrafficDistributionGroupResponse = [3, n0, _DTDGRe, 0, [], []];
export var DeleteUseCaseRequest = [
    3,
    n0,
    _DUCR,
    0,
    [_II, _IAI, _UCI],
    [
        [0, 1],
        [0, 1],
        [0, 1],
    ],
];
export var DeleteUserHierarchyGroupRequest = [
    3,
    n0,
    _DUHGR,
    0,
    [_HGI, _II],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteUserRequest = [
    3,
    n0,
    _DUR,
    0,
    [_II, _UI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteViewRequest = [
    3,
    n0,
    _DVR,
    0,
    [_II, _VIi],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteViewResponse = [3, n0, _DVRe, 0, [], []];
export var DeleteViewVersionRequest = [
    3,
    n0,
    _DVVR,
    0,
    [_II, _VIi, _VV],
    [
        [0, 1],
        [0, 1],
        [1, 1],
    ],
];
export var DeleteViewVersionResponse = [3, n0, _DVVRe, 0, [], []];
export var DeleteVocabularyRequest = [
    3,
    n0,
    _DVRel,
    0,
    [_II, _VI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DeleteVocabularyResponse = [3, n0, _DVRele, 0, [_VAo, _VI, _S], [0, 0, 0]];
export var DescribeAgentStatusRequest = [
    3,
    n0,
    _DASR,
    0,
    [_II, _ASI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeAgentStatusResponse = [3, n0, _DASRe, 0, [_ASg], [() => AgentStatus]];
export var DescribeAuthenticationProfileRequest = [
    3,
    n0,
    _DAPR,
    0,
    [_API, _II],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeAuthenticationProfileResponse = [
    3,
    n0,
    _DAPRe,
    0,
    [_APu],
    [() => AuthenticationProfile],
];
export var DescribeContactEvaluationRequest = [
    3,
    n0,
    _DCERe,
    0,
    [_II, _EI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeContactEvaluationResponse = [
    3,
    n0,
    _DCERes,
    0,
    [_Ev, _EFv],
    [() => Evaluation, () => EvaluationFormContent],
];
export var DescribeContactFlowModuleAliasRequest = [
    3,
    n0,
    _DCFMARes,
    0,
    [_II, _CFMI, _AIli],
    [
        [0, 1],
        [0, 1],
        [0, 1],
    ],
];
export var DescribeContactFlowModuleAliasResponse = [
    3,
    n0,
    _DCFMAResc,
    0,
    [_CFMAo],
    [() => ContactFlowModuleAliasInfo],
];
export var DescribeContactFlowModuleRequest = [
    3,
    n0,
    _DCFMRes,
    0,
    [_II, _CFMI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeContactFlowModuleResponse = [
    3,
    n0,
    _DCFMResc,
    0,
    [_CFM],
    [() => ContactFlowModule],
];
export var DescribeContactFlowRequest = [
    3,
    n0,
    _DCFRes,
    0,
    [_II, _CFI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeContactFlowResponse = [3, n0, _DCFResc, 0, [_CFo], [() => ContactFlow]];
export var DescribeContactRequest = [
    3,
    n0,
    _DCR,
    0,
    [_II, _CI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeContactResponse = [3, n0, _DCRe, 0, [_Cont], [[() => Contact, 0]]];
export var DescribeEmailAddressRequest = [
    3,
    n0,
    _DEARes,
    0,
    [_II, _EAI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeEmailAddressResponse = [
    3,
    n0,
    _DEAResc,
    0,
    [_EAI, _EAA, _EA, _DN, _D, _CTrea, _MT, _ACli, _Ta],
    [
        0,
        0,
        [() => EmailAddress, 0],
        [() => EmailAddressDisplayName, 0],
        [() => Description, 0],
        0,
        0,
        () => AliasConfigurationList,
        128 | 0,
    ],
];
export var DescribeEvaluationFormRequest = [
    3,
    n0,
    _DEFRes,
    0,
    [_II, _EFI, _EFV],
    [
        [0, 1],
        [0, 1],
        [
            1,
            {
                [_hQ]: _v,
            },
        ],
    ],
];
export var DescribeEvaluationFormResponse = [3, n0, _DEFResc, 0, [_EFv], [() => EvaluationForm]];
export var DescribeHoursOfOperationOverrideRequest = [
    3,
    n0,
    _DHOOORe,
    0,
    [_II, _HOOI, _HOOOI],
    [
        [0, 1],
        [0, 1],
        [0, 1],
    ],
];
export var DescribeHoursOfOperationOverrideResponse = [
    3,
    n0,
    _DHOOORes,
    0,
    [_HOOO],
    [() => HoursOfOperationOverride],
];
export var DescribeHoursOfOperationRequest = [
    3,
    n0,
    _DHOORe,
    0,
    [_II, _HOOI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeHoursOfOperationResponse = [
    3,
    n0,
    _DHOORes,
    0,
    [_HOO],
    [() => HoursOfOperation],
];
export var DescribeInstanceAttributeRequest = [
    3,
    n0,
    _DIARe,
    0,
    [_II, _ATt],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeInstanceAttributeResponse = [3, n0, _DIARes, 0, [_Att], [() => Attribute]];
export var DescribeInstanceRequest = [3, n0, _DIRe, 0, [_II], [[0, 1]]];
export var DescribeInstanceResponse = [
    3,
    n0,
    _DIRes,
    0,
    [_In, _RCe],
    [[() => Instance, 0], () => ReplicationConfiguration],
];
export var DescribeInstanceStorageConfigRequest = [
    3,
    n0,
    _DISCR,
    0,
    [_II, _AIs, _RT],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _rT,
            },
        ],
    ],
];
export var DescribeInstanceStorageConfigResponse = [
    3,
    n0,
    _DISCRe,
    0,
    [_SCt],
    [() => InstanceStorageConfig],
];
export var DescribePhoneNumberRequest = [3, n0, _DPNR, 0, [_PNI], [[0, 1]]];
export var DescribePhoneNumberResponse = [
    3,
    n0,
    _DPNRe,
    0,
    [_CPNS],
    [() => ClaimedPhoneNumberSummary],
];
export var DescribePredefinedAttributeRequest = [
    3,
    n0,
    _DPARe,
    0,
    [_II, _N],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribePredefinedAttributeResponse = [
    3,
    n0,
    _DPARes,
    0,
    [_PA],
    [() => PredefinedAttribute],
];
export var DescribePromptRequest = [
    3,
    n0,
    _DPRe,
    0,
    [_II, _PIr],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribePromptResponse = [3, n0, _DPRes, 0, [_Pro], [() => Prompt]];
export var DescribeQueueRequest = [
    3,
    n0,
    _DQRe,
    0,
    [_II, _QI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeQueueResponse = [3, n0, _DQRes, 0, [_Q], [() => Queue]];
export var DescribeQuickConnectRequest = [
    3,
    n0,
    _DQCRe,
    0,
    [_II, _QCIu],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeQuickConnectResponse = [3, n0, _DQCRes, 0, [_QCu], [() => QuickConnect]];
export var DescribeRoutingProfileRequest = [
    3,
    n0,
    _DRPRe,
    0,
    [_II, _RPI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeRoutingProfileResponse = [3, n0, _DRPRes, 0, [_RP], [() => RoutingProfile]];
export var DescribeRuleRequest = [
    3,
    n0,
    _DRRe,
    0,
    [_II, _RIu],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeRuleResponse = [3, n0, _DRRes, 0, [_Ru], [() => Rule]];
export var DescribeSecurityProfileRequest = [
    3,
    n0,
    _DSPRe,
    0,
    [_SPI, _II],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeSecurityProfileResponse = [3, n0, _DSPRes, 0, [_SP], [() => SecurityProfile]];
export var DescribeTrafficDistributionGroupRequest = [3, n0, _DTDGRes, 0, [_TDGI], [[0, 1]]];
export var DescribeTrafficDistributionGroupResponse = [
    3,
    n0,
    _DTDGResc,
    0,
    [_TDG],
    [() => TrafficDistributionGroup],
];
export var DescribeUserHierarchyGroupRequest = [
    3,
    n0,
    _DUHGRe,
    0,
    [_HGI, _II],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeUserHierarchyGroupResponse = [
    3,
    n0,
    _DUHGRes,
    0,
    [_HGi],
    [() => HierarchyGroup],
];
export var DescribeUserHierarchyStructureRequest = [3, n0, _DUHSR, 0, [_II], [[0, 1]]];
export var DescribeUserHierarchyStructureResponse = [
    3,
    n0,
    _DUHSRe,
    0,
    [_HS],
    [() => HierarchyStructure],
];
export var DescribeUserRequest = [
    3,
    n0,
    _DURe,
    0,
    [_UI, _II],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeUserResponse = [3, n0, _DURes, 0, [_Us], [[() => User, 0]]];
export var DescribeViewRequest = [
    3,
    n0,
    _DVRes,
    0,
    [_II, _VIi],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeViewResponse = [3, n0, _DVResc, 0, [_Vi], [[() => View, 0]]];
export var DescribeVocabularyRequest = [
    3,
    n0,
    _DVRescr,
    0,
    [_II, _VI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var DescribeVocabularyResponse = [3, n0, _DVRescri, 0, [_Vo], [() => Vocabulary]];
export var DestinationNotAllowedException = [
    -3,
    n0,
    _DNAE,
    {
        [_e]: _c,
        [_hE]: 403,
        [_aQE]: [`DestinationNotAllowedException`, 403],
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(DestinationNotAllowedException, __DestinationNotAllowedException);
export var DeviceInfo = [3, n0, _DI, 0, [_PNl, _PV, _OSp], [0, 0, 0]];
export var Dimensions = [
    3,
    n0,
    _Dim,
    0,
    [_Q, _C, _RP, _RSE, _ASg],
    [() => QueueReference, 0, () => RoutingProfileReference, 0, () => AgentStatusIdentifier],
];
export var DisassociateAnalyticsDataSetRequest = [
    3,
    n0,
    _DADSR,
    0,
    [_II, _DSI, _TAI],
    [[0, 1], 0, 0],
];
export var DisassociateApprovedOriginRequest = [
    3,
    n0,
    _DAOR,
    0,
    [_II, _O, _CT],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _o,
            },
        ],
        [
            0,
            {
                [_iT]: 1,
                [_hQ]: _cT,
            },
        ],
    ],
];
export var DisassociateBotRequest = [
    3,
    n0,
    _DBR,
    0,
    [_II, _LB, _LVB, _CT],
    [[0, 1], () => LexBot, () => LexV2Bot, [0, 4]],
];
export var DisassociateEmailAddressAliasRequest = [
    3,
    n0,
    _DEAAR,
    0,
    [_EAI, _II, _ACl, _CT],
    [[0, 1], [0, 1], () => AliasConfiguration, [0, 4]],
];
export var DisassociateEmailAddressAliasResponse = [3, n0, _DEAARi, 0, [], []];
export var DisassociateFlowRequest = [
    3,
    n0,
    _DFR,
    0,
    [_II, _RI, _RT],
    [
        [0, 1],
        [0, 1],
        [0, 1],
    ],
];
export var DisassociateFlowResponse = [3, n0, _DFRi, 0, [], []];
export var DisassociateInstanceStorageConfigRequest = [
    3,
    n0,
    _DISCRi,
    0,
    [_II, _AIs, _RT, _CT],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _rT,
            },
        ],
        [
            0,
            {
                [_iT]: 1,
                [_hQ]: _cT,
            },
        ],
    ],
];
export var DisassociateLambdaFunctionRequest = [
    3,
    n0,
    _DLFR,
    0,
    [_II, _FA, _CT],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _fA,
            },
        ],
        [
            0,
            {
                [_iT]: 1,
                [_hQ]: _cT,
            },
        ],
    ],
];
export var DisassociateLexBotRequest = [
    3,
    n0,
    _DLBR,
    0,
    [_II, _BN, _LR, _CT],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _bN,
            },
        ],
        [
            0,
            {
                [_hQ]: _lR,
            },
        ],
        [
            0,
            {
                [_iT]: 1,
                [_hQ]: _cT,
            },
        ],
    ],
];
export var DisassociatePhoneNumberContactFlowRequest = [
    3,
    n0,
    _DPNCFR,
    0,
    [_PNI, _II],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _iI,
            },
        ],
    ],
];
export var DisassociateQueueQuickConnectsRequest = [
    3,
    n0,
    _DQQCR,
    0,
    [_II, _QI, _QCI],
    [[0, 1], [0, 1], 64 | 0],
];
export var DisassociateRoutingProfileQueuesRequest = [
    3,
    n0,
    _DRPQR,
    0,
    [_II, _RPI, _QR, _MAQR],
    [[0, 1], [0, 1], () => RoutingProfileQueueReferenceList, () => RoutingProfileQueueReferenceList],
];
export var DisassociateSecurityKeyRequest = [
    3,
    n0,
    _DSKR,
    0,
    [_II, _AIs, _CT],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_iT]: 1,
                [_hQ]: _cT,
            },
        ],
    ],
];
export var DisassociateTrafficDistributionGroupUserRequest = [
    3,
    n0,
    _DTDGUR,
    0,
    [_TDGI, _UI, _II],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _UI,
            },
        ],
        [
            0,
            {
                [_hQ]: _II,
            },
        ],
    ],
];
export var DisassociateTrafficDistributionGroupUserResponse = [3, n0, _DTDGURi, 0, [], []];
export var DisassociateUserProficienciesRequest = [
    3,
    n0,
    _DUPR,
    0,
    [_II, _UI, _UP],
    [[0, 1], [0, 1], () => UserProficiencyDisassociateList],
];
export var DisconnectDetails = [3, n0, _DD, 0, [_PDI], [0]];
export var DisconnectReason = [3, n0, _DR, 0, [_Cod], [0]];
export var DismissUserContactRequest = [3, n0, _DUCRi, 0, [_UI, _II, _CI], [[0, 1], [0, 1], 0]];
export var DismissUserContactResponse = [3, n0, _DUCRis, 0, [], []];
export var Distribution = [3, n0, _Dis, 0, [_Reg, _Per], [0, 1]];
export var DownloadUrlMetadata = [3, n0, _DUM, 0, [_Ur, _UE], [0, 0]];
export var DuplicateResourceException = [
    -3,
    n0,
    _DRE,
    {
        [_e]: _c,
        [_hE]: 409,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(DuplicateResourceException, __DuplicateResourceException);
export var EffectiveHoursOfOperations = [
    3,
    n0,
    _EHOO,
    0,
    [_Da, _OH],
    [0, () => OperationalHours],
];
export var EmailAddressInfo = [
    3,
    n0,
    _EAIm,
    0,
    [_EA, _DN],
    [
        [() => EmailAddress, 0],
        [() => EmailAddressDisplayName, 0],
    ],
];
export var EmailAddressMetadata = [
    3,
    n0,
    _EAM,
    0,
    [_EAI, _EAA, _EA, _D, _DN, _ACli],
    [
        0,
        0,
        [() => EmailAddress, 0],
        [() => Description, 0],
        [() => EmailAddressDisplayName, 0],
        () => AliasConfigurationList,
    ],
];
export var EmailAddressSearchCriteria = [
    3,
    n0,
    _EASC,
    0,
    [_OC, _ACn, _SC],
    [() => EmailAddressSearchConditionList, () => EmailAddressSearchConditionList, () => StringCondition],
];
export var EmailAddressSearchFilter = [3, n0, _EASF, 0, [_TF], [() => ControlPlaneTagFilter]];
export var EmailAttachment = [3, n0, _EAm, 0, [_FN, _SUr], [0, 0]];
export var EmailMessageReference = [3, n0, _EMR, 0, [_N, _A], [0, 0]];
export var EmailRecipient = [3, n0, _ERm, 0, [_Ad, _DN], [0, 0]];
export var EmailReference = [3, n0, _ERma, 0, [_N, _V], [0, 0]];
export var EmptyFieldValue = [3, n0, _EFVm, 0, [], []];
export var EncryptionConfig = [3, n0, _ECn, 0, [_ETnc, _KI], [0, 0]];
export var EndAssociatedTasksActionDefinition = [3, n0, _EATAD, 0, [], []];
export var Endpoint = [3, n0, _End, 0, [_T, _Ad], [0, 0]];
export var EndpointInfo = [3, n0, _EIn, 0, [_T, _Ad, _DN], [0, 0, 0]];
export var ErrorResult = [3, n0, _ERr, 0, [_EC, _EM], [0, 0]];
export var Evaluation = [
    3,
    n0,
    _Ev,
    0,
    [_EI, _EAv, _Meta, _An, _No, _St, _Sc, _CTre, _LMT, _ETv, _Ta],
    [
        0,
        0,
        () => EvaluationMetadata,
        () => EvaluationAnswersOutputMap,
        () => EvaluationNotesMap,
        0,
        () => EvaluationScoresMap,
        4,
        4,
        0,
        128 | 0,
    ],
];
export var EvaluationAcknowledgement = [3, n0, _EAva, 0, [_ATck, _AB, _ACc], [4, 0, 0]];
export var EvaluationAcknowledgementSummary = [3, n0, _EAS, 0, [_ATck, _AB, _ACc], [4, 0, 0]];
export var EvaluationAnswerInput = [3, n0, _EAIv, 0, [_V], [() => EvaluationAnswerData]];
export var EvaluationAnswerOutput = [
    3,
    n0,
    _EAO,
    0,
    [_V, _SSV, _SAu],
    [() => EvaluationAnswerData, () => EvaluationAnswerData, () => EvaluationSuggestedAnswersList],
];
export var EvaluationAutomationRuleCategory = [
    3,
    n0,
    _EARC,
    0,
    [_Cat, _Con, _POI],
    [0, 0, () => EvaluationTranscriptPointsOfInterest],
];
export var EvaluationContactLensAnswerAnalysisDetails = [
    3,
    n0,
    _ECLAAD,
    0,
    [_MRC],
    [() => EvaluationAutomationRuleCategoryList],
];
export var EvaluationForm = [
    3,
    n0,
    _EFv,
    0,
    [_EFI, _EFV, _L, _EFA, _Ti, _D, _St, _It, _SS, _CTre, _CB, _LMT, _LMB, _AEC, _Ta],
    [
        0,
        1,
        2,
        0,
        0,
        0,
        0,
        () => EvaluationFormItemsList,
        () => EvaluationFormScoringStrategy,
        4,
        0,
        4,
        0,
        () => EvaluationFormAutoEvaluationConfiguration,
        128 | 0,
    ],
];
export var EvaluationFormAutoEvaluationConfiguration = [3, n0, _EFAEC, 0, [_En], [2]];
export var EvaluationFormContent = [
    3,
    n0,
    _EFC,
    0,
    [_EFV, _EFI, _EFA, _Ti, _D, _It, _SS, _AEC],
    [
        1,
        0,
        0,
        0,
        0,
        () => EvaluationFormItemsList,
        () => EvaluationFormScoringStrategy,
        () => EvaluationFormAutoEvaluationConfiguration,
    ],
];
export var EvaluationFormItemEnablementCondition = [
    3,
    n0,
    _EFIEC,
    0,
    [_Op, _Ope],
    [() => EvaluationFormItemEnablementConditionOperandList, 0],
];
export var EvaluationFormItemEnablementConfiguration = [
    3,
    n0,
    _EFIECv,
    0,
    [_Con, _Act, _DAe],
    [() => EvaluationFormItemEnablementCondition, 0, 0],
];
export var EvaluationFormItemEnablementExpression = [
    3,
    n0,
    _EFIEE,
    0,
    [_So, _Va, _Com],
    [() => EvaluationFormItemEnablementSource, () => EvaluationFormItemEnablementSourceValueList, 0],
];
export var EvaluationFormItemEnablementSource = [3, n0, _EFIES, 0, [_T, _RIef], [0, 0]];
export var EvaluationFormItemEnablementSourceValue = [3, n0, _EFIESV, 0, [_T, _RIef], [0, 0]];
export var EvaluationFormNumericQuestionOption = [
    3,
    n0,
    _EFNQO,
    0,
    [_MV, _MVa, _Sco, _AFut, _AFC],
    [1, 1, 1, 2, () => AutomaticFailConfiguration],
];
export var EvaluationFormNumericQuestionProperties = [
    3,
    n0,
    _EFNQP,
    0,
    [_MV, _MVa, _Opt, _Aut],
    [1, 1, () => EvaluationFormNumericQuestionOptionList, () => EvaluationFormNumericQuestionAutomation],
];
export var EvaluationFormQuestion = [
    3,
    n0,
    _EFQ,
    0,
    [_Ti, _Ins, _RIef, _NAE, _QT, _QTP, _Ena, _W],
    [0, 0, 0, 2, 0, () => EvaluationFormQuestionTypeProperties, () => EvaluationFormItemEnablementConfiguration, 1],
];
export var EvaluationFormQuestionAutomationAnswerSource = [3, n0, _EFQAAS, 0, [_STo], [0]];
export var EvaluationFormScoringStrategy = [3, n0, _EFSS, 0, [_Mo, _St], [0, 0]];
export var EvaluationFormSearchCriteria = [
    3,
    n0,
    _EFSC,
    0,
    [_OC, _ACn, _SC, _NC, _BC, _DTC],
    [
        () => EvaluationFormSearchConditionList,
        () => EvaluationFormSearchConditionList,
        () => StringCondition,
        () => NumberCondition,
        () => BooleanCondition,
        () => DateTimeCondition,
    ],
];
export var EvaluationFormSearchFilter = [
    3,
    n0,
    _EFSF,
    0,
    [_AFt],
    [() => ControlPlaneAttributeFilter],
];
export var EvaluationFormSearchSummary = [
    3,
    n0,
    _EFSSv,
    0,
    [_EFI, _EFA, _Ti, _St, _D, _CTre, _CB, _LMT, _LMB, _LAT, _LAB, _LV, _AV, _AEE, _Ta],
    [0, 0, 0, 0, 0, 4, 0, 4, 0, 4, 0, 1, 1, 2, 128 | 0],
];
export var EvaluationFormSection = [
    3,
    n0,
    _EFS,
    0,
    [_Ti, _RIef, _Ins, _It, _W],
    [0, 0, 0, () => EvaluationFormItemsList, 1],
];
export var EvaluationFormSingleSelectQuestionAutomation = [
    3,
    n0,
    _EFSSQA,
    0,
    [_Opt, _DORI, _ASn],
    [() => EvaluationFormSingleSelectQuestionAutomationOptionList, 0, () => EvaluationFormQuestionAutomationAnswerSource],
];
export var EvaluationFormSingleSelectQuestionOption = [
    3,
    n0,
    _EFSSQO,
    0,
    [_RIef, _Te, _Sco, _AFut, _AFC],
    [0, 0, 1, 2, () => AutomaticFailConfiguration],
];
export var EvaluationFormSingleSelectQuestionProperties = [
    3,
    n0,
    _EFSSQP,
    0,
    [_Opt, _DAi, _Aut],
    [() => EvaluationFormSingleSelectQuestionOptionList, 0, () => EvaluationFormSingleSelectQuestionAutomation],
];
export var EvaluationFormSummary = [
    3,
    n0,
    _EFSv,
    0,
    [_EFI, _EFA, _Ti, _CTre, _CB, _LMT, _LMB, _LAT, _LAB, _LV, _AV],
    [0, 0, 0, 4, 0, 4, 0, 4, 0, 1, 1],
];
export var EvaluationFormTextQuestionAutomation = [
    3,
    n0,
    _EFTQA,
    0,
    [_ASn],
    [() => EvaluationFormQuestionAutomationAnswerSource],
];
export var EvaluationFormTextQuestionProperties = [
    3,
    n0,
    _EFTQP,
    0,
    [_Aut],
    [() => EvaluationFormTextQuestionAutomation],
];
export var EvaluationFormVersionSummary = [
    3,
    n0,
    _EFVS,
    0,
    [_EFA, _EFI, _EFV, _L, _St, _CTre, _CB, _LMT, _LMB],
    [0, 0, 1, 2, 0, 4, 0, 4, 0],
];
export var EvaluationGenAIAnswerAnalysisDetails = [
    3,
    n0,
    _EGAIAAD,
    0,
    [_J, _POI],
    [0, () => EvaluationTranscriptPointsOfInterest],
];
export var EvaluationMetadata = [
    3,
    n0,
    _EMv,
    0,
    [_CI, _EAval, _CAIo, _CSI, _Sco, _AEu, _Ack],
    [0, 0, 0, 0, () => EvaluationScore, () => AutoEvaluationDetails, () => EvaluationAcknowledgement],
];
export var EvaluationNote = [3, n0, _EN, 0, [_V], [0]];
export var EvaluationQuestionInputDetails = [3, n0, _EQID, 0, [_TT], [0]];
export var EvaluationScore = [3, n0, _ES, 0, [_Per, _NA, _AFut], [1, 2, 2]];
export var EvaluationSearchCriteria = [
    3,
    n0,
    _ESC,
    0,
    [_OC, _ACn, _SC, _NC, _BC, _DTC, _DCe],
    [
        () => EvaluationSearchConditionList,
        () => EvaluationSearchConditionList,
        () => StringCondition,
        () => NumberCondition,
        () => BooleanCondition,
        () => DateTimeCondition,
        () => DecimalCondition,
    ],
];
export var EvaluationSearchFilter = [
    3,
    n0,
    _ESF,
    0,
    [_AFt],
    [() => ControlPlaneAttributeFilter],
];
export var EvaluationSearchMetadata = [
    3,
    n0,
    _ESM,
    0,
    [_CI, _EAval, _CAIo, _CSI, _SPc, _SAF, _SNA, _AEE, _AES, _ATck, _AB, _ACc],
    [0, 0, 0, 0, 1, 2, 2, 2, 0, 4, 0, 0],
];
export var EvaluationSearchSummary = [
    3,
    n0,
    _ESS,
    0,
    [_EI, _EAv, _EFI, _EFV, _Meta, _St, _ETv, _CTre, _LMT, _Ta],
    [0, 0, 0, 1, () => EvaluationSearchMetadata, 0, 0, 4, 4, 128 | 0],
];
export var EvaluationSuggestedAnswer = [
    3,
    n0,
    _ESA,
    0,
    [_V, _St, _Inp, _ATn, _ADn],
    [
        () => EvaluationAnswerData,
        0,
        () => EvaluationQuestionInputDetails,
        0,
        () => EvaluationQuestionAnswerAnalysisDetails,
    ],
];
export var EvaluationSuggestedAnswerTranscriptMillisecondOffsets = [
    3,
    n0,
    _ESATMO,
    0,
    [_BOM],
    [1],
];
export var EvaluationSummary = [
    3,
    n0,
    _ESv,
    0,
    [_EI, _EAv, _EFT, _EFI, _CSI, _St, _AEE, _AES, _EAval, _Sco, _Ack, _ETv, _CTre, _LMT],
    [0, 0, 0, 0, 0, 0, 2, 0, 0, () => EvaluationScore, () => EvaluationAcknowledgementSummary, 0, 4, 4],
];
export var EvaluationTranscriptPointOfInterest = [
    3,
    n0,
    _ETPOI,
    0,
    [_MO, _TSr],
    [() => EvaluationSuggestedAnswerTranscriptMillisecondOffsets, 0],
];
export var EventBridgeActionDefinition = [3, n0, _EBAD, 0, [_N], [0]];
export var Expiry = [3, n0, _Ex, 0, [_DIS, _ETx], [1, 4]];
export var Expression = [
    3,
    n0,
    _Exp,
    0,
    [_ACt, _AEn, _OE, _NAC],
    [() => AttributeCondition, () => Expressions, () => Expressions, () => AttributeCondition],
];
export var ExternalInvocationConfiguration = [3, n0, _EIC, 0, [_En], [2]];
export var FailedRequest = [3, n0, _FR, 0, [_RIeq, _FRC, _FRM], [0, 0, 0]];
export var FieldValue = [3, n0, _FV, 0, [_I, _V], [0, () => FieldValueUnion]];
export var FieldValueUnion = [
    3,
    n0,
    _FVU,
    0,
    [_BV, _DVo, _EV, _SV],
    [2, 1, () => EmptyFieldValue, 0],
];
export var Filters = [
    3,
    n0,
    _Fil,
    0,
    [_Qu, _Ch, _RPo, _RSEo, _ASge],
    [64 | 0, 64 | 0, 64 | 0, 64 | 0, 64 | 0],
];
export var FilterV2 = [3, n0, _FVi, 0, [_FK, _FVil], [0, 64 | 0]];
export var FlowAssociationSummary = [3, n0, _FAS, 0, [_RI, _FI, _RT], [0, 0, 0]];
export var GetAttachedFileRequest = [
    3,
    n0,
    _GAFR,
    0,
    [_II, _FIi, _UEIS, _ARA],
    [
        [0, 1],
        [0, 1],
        [
            1,
            {
                [_hQ]: _uEIS,
            },
        ],
        [
            0,
            {
                [_hQ]: _aRA,
            },
        ],
    ],
];
export var GetAttachedFileResponse = [
    3,
    n0,
    _GAFRe,
    0,
    [_FAi, _FIi, _CTr, _FS, _FN, _FSIB, _ARA, _FUCT, _CB, _DUM, _Ta],
    [0, 0, 0, 0, 0, 1, 0, 0, () => CreatedByInfo, () => DownloadUrlMetadata, 128 | 0],
];
export var GetContactAttributesRequest = [
    3,
    n0,
    _GCAR,
    0,
    [_II, _ICI],
    [
        [0, 1],
        [0, 1],
    ],
];
export var GetContactAttributesResponse = [3, n0, _GCARe, 0, [_Attr], [128 | 0]];
export var GetContactMetricsRequest = [
    3,
    n0,
    _GCMR,
    0,
    [_II, _CI, _Metr],
    [0, 0, () => ContactMetrics],
];
export var GetContactMetricsResponse = [
    3,
    n0,
    _GCMRe,
    0,
    [_MR, _I, _A],
    [() => ContactMetricResults, 0, 0],
];
export var GetCurrentMetricDataRequest = [
    3,
    n0,
    _GCMDR,
    0,
    [_II, _Fil, _G, _CMurr, _NT, _MRa, _SCo],
    [[0, 1], () => Filters, 64 | 0, () => CurrentMetrics, 0, 1, () => CurrentMetricSortCriteriaMaxOne],
];
export var GetCurrentMetricDataResponse = [
    3,
    n0,
    _GCMDRe,
    0,
    [_NT, _MR, _DST, _ATC],
    [0, () => CurrentMetricResults, 4, 1],
];
export var GetCurrentUserDataRequest = [
    3,
    n0,
    _GCUDR,
    0,
    [_II, _Fil, _NT, _MRa],
    [[0, 1], () => UserDataFilters, 0, 1],
];
export var GetCurrentUserDataResponse = [
    3,
    n0,
    _GCUDRe,
    0,
    [_NT, _UDL, _ATC],
    [0, () => UserDataList, 1],
];
export var GetEffectiveHoursOfOperationsRequest = [
    3,
    n0,
    _GEHOOR,
    0,
    [_II, _HOOI, _FD, _TD],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _fD,
            },
        ],
        [
            0,
            {
                [_hQ]: _tD,
            },
        ],
    ],
];
export var GetEffectiveHoursOfOperationsResponse = [
    3,
    n0,
    _GEHOORe,
    0,
    [_EHOOL, _TZ],
    [() => EffectiveHoursOfOperationList, 0],
];
export var GetFederationTokenRequest = [3, n0, _GFTR, 0, [_II], [[0, 1]]];
export var GetFederationTokenResponse = [
    3,
    n0,
    _GFTRe,
    0,
    [_Cre, _SIU, _UA, _UI],
    [[() => Credentials, 0], 0, 0, 0],
];
export var GetFlowAssociationRequest = [
    3,
    n0,
    _GFAR,
    0,
    [_II, _RI, _RT],
    [
        [0, 1],
        [0, 1],
        [0, 1],
    ],
];
export var GetFlowAssociationResponse = [3, n0, _GFARe, 0, [_RI, _FI, _RT], [0, 0, 0]];
export var GetMetricDataRequest = [
    3,
    n0,
    _GMDR,
    0,
    [_II, _STtar, _ETnd, _Fil, _G, _HM, _NT, _MRa],
    [[0, 1], 4, 4, () => Filters, 64 | 0, () => HistoricalMetrics, 0, 1],
];
export var GetMetricDataResponse = [
    3,
    n0,
    _GMDRe,
    0,
    [_NT, _MR],
    [0, () => HistoricalMetricResults],
];
export var GetMetricDataV2Request = [
    3,
    n0,
    _GMDVR,
    0,
    [_RAe, _STtar, _ETnd, _Int, _Fil, _G, _Metr, _NT, _MRa],
    [0, 4, 4, () => IntervalDetails, () => FiltersV2List, 64 | 0, () => MetricsV2, 0, 1],
];
export var GetMetricDataV2Response = [3, n0, _GMDVRe, 0, [_NT, _MR], [0, () => MetricResultsV2]];
export var GetPromptFileRequest = [
    3,
    n0,
    _GPFR,
    0,
    [_II, _PIr],
    [
        [0, 1],
        [0, 1],
    ],
];
export var GetPromptFileResponse = [3, n0, _GPFRe, 0, [_PPU, _LMT, _LMR], [0, 4, 0]];
export var GetTaskTemplateRequest = [
    3,
    n0,
    _GTTR,
    0,
    [_II, _TTIa, _SVn],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _sV,
            },
        ],
    ],
];
export var GetTaskTemplateResponse = [
    3,
    n0,
    _GTTRe,
    0,
    [_II, _I, _A, _N, _D, _CFI, _SAFI, _Cons, _Def, _Fi, _St, _LMT, _CTre, _Ta],
    [
        0,
        0,
        0,
        0,
        0,
        0,
        0,
        () => TaskTemplateConstraints,
        () => TaskTemplateDefaults,
        () => TaskTemplateFields,
        0,
        4,
        4,
        128 | 0,
    ],
];
export var GetTrafficDistributionRequest = [3, n0, _GTDR, 0, [_I], [[0, 1]]];
export var GetTrafficDistributionResponse = [
    3,
    n0,
    _GTDRe,
    0,
    [_TCe, _I, _A, _SIC, _ACg],
    [() => TelephonyConfig, 0, 0, () => SignInConfig, () => AgentConfig],
];
export var HierarchyGroup = [
    3,
    n0,
    _HGi,
    0,
    [_I, _A, _N, _LIe, _HP, _Ta, _LMT, _LMR],
    [0, 0, 0, 0, () => HierarchyPath, 128 | 0, 4, 0],
];
export var HierarchyGroupCondition = [3, n0, _HGC, 0, [_V, _HGMT], [0, 0]];
export var HierarchyGroups = [
    3,
    n0,
    _HG,
    0,
    [_Le, _Lev, _Leve, _Level, _Level_],
    [
        () => AgentHierarchyGroup,
        () => AgentHierarchyGroup,
        () => AgentHierarchyGroup,
        () => AgentHierarchyGroup,
        () => AgentHierarchyGroup,
    ],
];
export var HierarchyGroupSummary = [3, n0, _HGS, 0, [_I, _A, _N, _LMT, _LMR], [0, 0, 0, 4, 0]];
export var HierarchyGroupSummaryReference = [3, n0, _HGSR, 0, [_I, _A], [0, 0]];
export var HierarchyLevel = [3, n0, _HL, 0, [_I, _A, _N, _LMT, _LMR], [0, 0, 0, 4, 0]];
export var HierarchyLevelUpdate = [3, n0, _HLU, 0, [_N], [0]];
export var HierarchyPath = [
    3,
    n0,
    _HP,
    0,
    [_LO, _LT, _LTe, _LF, _LFe],
    [
        () => HierarchyGroupSummary,
        () => HierarchyGroupSummary,
        () => HierarchyGroupSummary,
        () => HierarchyGroupSummary,
        () => HierarchyGroupSummary,
    ],
];
export var HierarchyPathReference = [
    3,
    n0,
    _HPR,
    0,
    [_LO, _LT, _LTe, _LF, _LFe],
    [
        () => HierarchyGroupSummaryReference,
        () => HierarchyGroupSummaryReference,
        () => HierarchyGroupSummaryReference,
        () => HierarchyGroupSummaryReference,
        () => HierarchyGroupSummaryReference,
    ],
];
export var HierarchyStructure = [
    3,
    n0,
    _HS,
    0,
    [_LO, _LT, _LTe, _LF, _LFe],
    [() => HierarchyLevel, () => HierarchyLevel, () => HierarchyLevel, () => HierarchyLevel, () => HierarchyLevel],
];
export var HierarchyStructureUpdate = [
    3,
    n0,
    _HSU,
    0,
    [_LO, _LT, _LTe, _LF, _LFe],
    [
        () => HierarchyLevelUpdate,
        () => HierarchyLevelUpdate,
        () => HierarchyLevelUpdate,
        () => HierarchyLevelUpdate,
        () => HierarchyLevelUpdate,
    ],
];
export var HistoricalMetric = [3, n0, _HMi, 0, [_N, _Th, _Sta, _Un], [0, () => Threshold, 0, 0]];
export var HistoricalMetricData = [3, n0, _HMD, 0, [_Met, _V], [() => HistoricalMetric, 1]];
export var HistoricalMetricResult = [
    3,
    n0,
    _HMR,
    0,
    [_Dim, _Col],
    [() => Dimensions, () => HistoricalMetricDataCollections],
];
export var HoursOfOperation = [
    3,
    n0,
    _HOO,
    0,
    [_HOOI, _HOOA, _N, _D, _TZ, _Conf, _Ta, _LMT, _LMR],
    [0, 0, 0, 0, 0, () => HoursOfOperationConfigList, 128 | 0, 4, 0],
];
export var HoursOfOperationConfig = [
    3,
    n0,
    _HOOC,
    0,
    [_Day, _STtar, _ETnd],
    [0, () => HoursOfOperationTimeSlice, () => HoursOfOperationTimeSlice],
];
export var HoursOfOperationOverride = [
    3,
    n0,
    _HOOO,
    0,
    [_HOOOI, _HOOI, _HOOA, _N, _D, _Conf, _EF, _ETf],
    [0, 0, 0, 0, 0, () => HoursOfOperationOverrideConfigList, 0, 0],
];
export var HoursOfOperationOverrideConfig = [
    3,
    n0,
    _HOOOC,
    0,
    [_Day, _STtar, _ETnd],
    [0, () => OverrideTimeSlice, () => OverrideTimeSlice],
];
export var HoursOfOperationOverrideSearchCriteria = [
    3,
    n0,
    _HOOOSC,
    0,
    [_OC, _ACn, _SC, _DC],
    [
        () => HoursOfOperationOverrideSearchConditionList,
        () => HoursOfOperationOverrideSearchConditionList,
        () => StringCondition,
        () => DateCondition,
    ],
];
export var HoursOfOperationSearchCriteria = [
    3,
    n0,
    _HOOSC,
    0,
    [_OC, _ACn, _SC],
    [() => HoursOfOperationSearchConditionList, () => HoursOfOperationSearchConditionList, () => StringCondition],
];
export var HoursOfOperationSearchFilter = [
    3,
    n0,
    _HOOSF,
    0,
    [_TF],
    [() => ControlPlaneTagFilter],
];
export var HoursOfOperationSummary = [
    3,
    n0,
    _HOOS,
    0,
    [_I, _A, _N, _LMT, _LMR],
    [0, 0, 0, 4, 0],
];
export var HoursOfOperationTimeSlice = [3, n0, _HOOTS, 0, [_H, _Mi], [1, 1]];
export var IdempotencyException = [
    -3,
    n0,
    _IE,
    {
        [_e]: _c,
        [_hE]: 409,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(IdempotencyException, __IdempotencyException);
export var ImportPhoneNumberRequest = [
    3,
    n0,
    _IPNR,
    0,
    [_II, _SPNA, _PND, _Ta, _CT],
    [0, 0, 0, 128 | 0, [0, 4]],
];
export var ImportPhoneNumberResponse = [3, n0, _IPNRm, 0, [_PNI, _PNA], [0, 0]];
export var InboundAdditionalRecipients = [
    3,
    n0,
    _IAR,
    0,
    [_TAo, _CAc],
    [
        [() => EmailAddressRecipientList, 0],
        [() => EmailAddressRecipientList, 0],
    ],
];
export var InboundEmailContent = [
    3,
    n0,
    _IEC,
    0,
    [_MST, _RM],
    [0, [() => InboundRawMessage, 0]],
];
export var InboundRawMessage = [
    3,
    n0,
    _IRMn,
    0,
    [_Su, _B, _CTon, _He],
    [[() => InboundSubject, 0], [() => Body, 0], 0, 128 | 0],
];
export var InputPredefinedAttributeConfiguration = [3, n0, _IPAC, 0, [_EVVOA], [2]];
export var Instance = [
    3,
    n0,
    _In,
    0,
    [_I, _A, _IMT, _IAn, _CTre, _SR, _ISn, _SRt, _ICE, _OCE, _IAU, _Ta],
    [0, 0, 0, [() => DirectoryAlias, 0], 4, 0, 0, () => InstanceStatusReason, 2, 2, 0, 128 | 0],
];
export var InstanceStatusReason = [3, n0, _ISR, 0, [_M], [0]];
export var InstanceStorageConfig = [
    3,
    n0,
    _ISC,
    0,
    [_AIs, _STto, _SCon, _KVSC, _KSC, _KFC],
    [0, 0, () => S3Config, () => KinesisVideoStreamConfig, () => KinesisStreamConfig, () => KinesisFirehoseConfig],
];
export var InstanceSummary = [
    3,
    n0,
    _ISns,
    0,
    [_I, _A, _IMT, _IAn, _CTre, _SR, _ISn, _ICE, _OCE, _IAU],
    [0, 0, 0, [() => DirectoryAlias, 0], 4, 0, 0, 2, 2, 0],
];
export var IntegrationAssociationSummary = [
    3,
    n0,
    _IAS,
    0,
    [_IAI, _IAA, _II, _ITn, _IAnt, _SAU, _SAN, _STo],
    [0, 0, 0, 0, 0, 0, 0, 0],
];
export var InternalServiceException = [
    -3,
    n0,
    _ISE,
    {
        [_e]: _s,
        [_hE]: 500,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(InternalServiceException, __InternalServiceException);
export var IntervalDetails = [3, n0, _IDn, 0, [_TZ, _IP], [0, 0]];
export var InvalidContactFlowException = [
    -3,
    n0,
    _ICFE,
    {
        [_e]: _c,
        [_hE]: 400,
    },
    [_p],
    [() => Problems],
];
TypeRegistry.for(n0).registerError(InvalidContactFlowException, __InvalidContactFlowException);
export var InvalidContactFlowModuleException = [
    -3,
    n0,
    _ICFME,
    {
        [_e]: _c,
        [_hE]: 400,
    },
    [_Prob],
    [() => Problems],
];
TypeRegistry.for(n0).registerError(InvalidContactFlowModuleException, __InvalidContactFlowModuleException);
export var InvalidParameterException = [
    -3,
    n0,
    _IPE,
    {
        [_e]: _c,
        [_hE]: 400,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(InvalidParameterException, __InvalidParameterException);
export var InvalidRequestException = [
    -3,
    n0,
    _IRE,
    {
        [_e]: _c,
        [_hE]: 400,
    },
    [_M, _Rea],
    [0, () => InvalidRequestExceptionReason],
];
TypeRegistry.for(n0).registerError(InvalidRequestException, __InvalidRequestException);
export var InvisibleFieldInfo = [3, n0, _IFI, 0, [_I], [() => TaskTemplateFieldIdentifier]];
export var KinesisFirehoseConfig = [3, n0, _KFC, 0, [_FAir], [0]];
export var KinesisStreamConfig = [3, n0, _KSC, 0, [_SAt], [0]];
export var KinesisVideoStreamConfig = [
    3,
    n0,
    _KVSC,
    0,
    [_Pre, _RPH, _ECn],
    [0, 1, () => EncryptionConfig],
];
export var LexBot = [3, n0, _LB, 0, [_N, _LR], [0, 0]];
export var LexBotConfig = [3, n0, _LBC, 0, [_LB, _LVB], [() => LexBot, () => LexV2Bot]];
export var LexV2Bot = [3, n0, _LVB, 0, [_AA], [0]];
export var LimitExceededException = [
    -3,
    n0,
    _LEE,
    {
        [_e]: _c,
        [_hE]: 429,
        [_aQE]: [`LimitExceededException`, 429],
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(LimitExceededException, __LimitExceededException);
export var ListAgentStatusRequest = [
    3,
    n0,
    _LASR,
    0,
    [_II, _NT, _MRa, _AST],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            64 | 0,
            {
                [_hQ]: _AST,
            },
        ],
    ],
];
export var ListAgentStatusResponse = [
    3,
    n0,
    _LASRi,
    0,
    [_NT, _ASSL],
    [0, () => AgentStatusSummaryList],
];
export var ListAnalyticsDataAssociationsRequest = [
    3,
    n0,
    _LADAR,
    0,
    [_II, _DSI, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _DSI,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListAnalyticsDataAssociationsResponse = [
    3,
    n0,
    _LADARi,
    0,
    [_Res, _NT],
    [() => AnalyticsDataAssociationResults, 0],
];
export var ListAnalyticsDataLakeDataSetsRequest = [
    3,
    n0,
    _LADLDSR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListAnalyticsDataLakeDataSetsResponse = [
    3,
    n0,
    _LADLDSRi,
    0,
    [_Res, _NT],
    [() => AnalyticsDataSetsResults, 0],
];
export var ListApprovedOriginsRequest = [
    3,
    n0,
    _LAOR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListApprovedOriginsResponse = [3, n0, _LAORi, 0, [_Or, _NT], [64 | 0, 0]];
export var ListAssociatedContactsRequest = [
    3,
    n0,
    _LACR,
    0,
    [_II, _CI, _MRa, _NT],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _cI,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
    ],
];
export var ListAssociatedContactsResponse = [
    3,
    n0,
    _LACRi,
    0,
    [_CSL, _NT],
    [() => AssociatedContactSummaryList, 0],
];
export var ListAuthenticationProfilesRequest = [
    3,
    n0,
    _LAPR,
    0,
    [_II, _MRa, _NT],
    [
        [0, 1],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
    ],
];
export var ListAuthenticationProfilesResponse = [
    3,
    n0,
    _LAPRi,
    0,
    [_APSL, _NT],
    [() => AuthenticationProfileSummaryList, 0],
];
export var ListBotsRequest = [
    3,
    n0,
    _LBR,
    0,
    [_II, _NT, _MRa, _LVe],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _lV,
            },
        ],
    ],
];
export var ListBotsResponse = [3, n0, _LBRi, 0, [_LBe, _NT], [() => LexBotConfigList, 0]];
export var ListCondition = [3, n0, _LCi, 0, [_TLT, _Cond], [0, () => Conditions]];
export var ListContactEvaluationsRequest = [
    3,
    n0,
    _LCER,
    0,
    [_II, _CI, _NT],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _cI,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
    ],
];
export var ListContactEvaluationsResponse = [
    3,
    n0,
    _LCERi,
    0,
    [_ESL, _NT],
    [() => EvaluationSummaryList, 0],
];
export var ListContactFlowModuleAliasesRequest = [
    3,
    n0,
    _LCFMAR,
    0,
    [_II, _CFMI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListContactFlowModuleAliasesResponse = [
    3,
    n0,
    _LCFMARi,
    0,
    [_CFMASL, _NT],
    [() => ContactFlowModuleAliasSummaryList, 0],
];
export var ListContactFlowModulesRequest = [
    3,
    n0,
    _LCFMR,
    0,
    [_II, _NT, _MRa, _CFMSo],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _st,
            },
        ],
    ],
];
export var ListContactFlowModulesResponse = [
    3,
    n0,
    _LCFMRi,
    0,
    [_CFMSL, _NT],
    [() => ContactFlowModulesSummaryList, 0],
];
export var ListContactFlowModuleVersionsRequest = [
    3,
    n0,
    _LCFMVR,
    0,
    [_II, _CFMI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListContactFlowModuleVersionsResponse = [
    3,
    n0,
    _LCFMVRi,
    0,
    [_CFMVSL, _NT],
    [() => ContactFlowModuleVersionSummaryList, 0],
];
export var ListContactFlowsRequest = [
    3,
    n0,
    _LCFR,
    0,
    [_II, _CFTo, _NT, _MRa],
    [
        [0, 1],
        [
            64 | 0,
            {
                [_hQ]: _cFT,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListContactFlowsResponse = [
    3,
    n0,
    _LCFRi,
    0,
    [_CFSL, _NT],
    [() => ContactFlowSummaryList, 0],
];
export var ListContactFlowVersionsRequest = [
    3,
    n0,
    _LCFVR,
    0,
    [_II, _CFI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListContactFlowVersionsResponse = [
    3,
    n0,
    _LCFVRi,
    0,
    [_CFVSL, _NT],
    [() => ContactFlowVersionSummaryList, 0],
];
export var ListContactReferencesRequest = [
    3,
    n0,
    _LCRR,
    0,
    [_II, _CI, _RTefe, _NT],
    [
        [0, 1],
        [0, 1],
        [
            64 | 0,
            {
                [_hQ]: _rTe,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
    ],
];
export var ListContactReferencesResponse = [
    3,
    n0,
    _LCRRi,
    0,
    [_RSL, _NT],
    [() => ReferenceSummaryList, 0],
];
export var ListDefaultVocabulariesRequest = [
    3,
    n0,
    _LDVR,
    0,
    [_II, _LC, _MRa, _NT],
    [[0, 1], 0, 1, 0],
];
export var ListDefaultVocabulariesResponse = [
    3,
    n0,
    _LDVRi,
    0,
    [_DVL, _NT],
    [() => DefaultVocabularyList, 0],
];
export var ListEvaluationFormsRequest = [
    3,
    n0,
    _LEFR,
    0,
    [_II, _MRa, _NT],
    [
        [0, 1],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
    ],
];
export var ListEvaluationFormsResponse = [
    3,
    n0,
    _LEFRi,
    0,
    [_EFSL, _NT],
    [() => EvaluationFormSummaryList, 0],
];
export var ListEvaluationFormVersionsRequest = [
    3,
    n0,
    _LEFVR,
    0,
    [_II, _EFI, _MRa, _NT],
    [
        [0, 1],
        [0, 1],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
    ],
];
export var ListEvaluationFormVersionsResponse = [
    3,
    n0,
    _LEFVRi,
    0,
    [_EFVSL, _NT],
    [() => EvaluationFormVersionSummaryList, 0],
];
export var ListFlowAssociationsRequest = [
    3,
    n0,
    _LFAR,
    0,
    [_II, _RT, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _RT,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListFlowAssociationsResponse = [
    3,
    n0,
    _LFARi,
    0,
    [_FASL, _NT],
    [() => FlowAssociationSummaryList, 0],
];
export var ListHoursOfOperationOverridesRequest = [
    3,
    n0,
    _LHOOOR,
    0,
    [_II, _HOOI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListHoursOfOperationOverridesResponse = [
    3,
    n0,
    _LHOOORi,
    0,
    [_NT, _HOOOL, _LMR, _LMT],
    [0, () => HoursOfOperationOverrideList, 0, 4],
];
export var ListHoursOfOperationsRequest = [
    3,
    n0,
    _LHOOR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListHoursOfOperationsResponse = [
    3,
    n0,
    _LHOORi,
    0,
    [_HOOSL, _NT],
    [() => HoursOfOperationSummaryList, 0],
];
export var ListInstanceAttributesRequest = [
    3,
    n0,
    _LIAR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListInstanceAttributesResponse = [
    3,
    n0,
    _LIARi,
    0,
    [_Attr, _NT],
    [() => AttributesList, 0],
];
export var ListInstancesRequest = [
    3,
    n0,
    _LIR,
    0,
    [_NT, _MRa],
    [
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListInstancesResponse = [
    3,
    n0,
    _LIRi,
    0,
    [_ISL, _NT],
    [[() => InstanceSummaryList, 0], 0],
];
export var ListInstanceStorageConfigsRequest = [
    3,
    n0,
    _LISCR,
    0,
    [_II, _RT, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _rT,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListInstanceStorageConfigsResponse = [
    3,
    n0,
    _LISCRi,
    0,
    [_SCto, _NT],
    [() => InstanceStorageConfigs, 0],
];
export var ListIntegrationAssociationsRequest = [
    3,
    n0,
    _LIARis,
    0,
    [_II, _ITn, _NT, _MRa, _IAnt],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _iTn,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _iA,
            },
        ],
    ],
];
export var ListIntegrationAssociationsResponse = [
    3,
    n0,
    _LIARist,
    0,
    [_IASL, _NT],
    [() => IntegrationAssociationSummaryList, 0],
];
export var ListLambdaFunctionsRequest = [
    3,
    n0,
    _LLFR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListLambdaFunctionsResponse = [3, n0, _LLFRi, 0, [_LFa, _NT], [64 | 0, 0]];
export var ListLexBotsRequest = [
    3,
    n0,
    _LLBR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListLexBotsResponse = [3, n0, _LLBRi, 0, [_LBe, _NT], [() => LexBotsList, 0]];
export var ListPhoneNumbersRequest = [
    3,
    n0,
    _LPNR,
    0,
    [_II, _PNTh, _PNCCh, _NT, _MRa],
    [
        [0, 1],
        [
            64 | 0,
            {
                [_hQ]: _pNT,
            },
        ],
        [
            64 | 0,
            {
                [_hQ]: _pNCC,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListPhoneNumbersResponse = [
    3,
    n0,
    _LPNRi,
    0,
    [_PNSL, _NT],
    [() => PhoneNumberSummaryList, 0],
];
export var ListPhoneNumbersSummary = [
    3,
    n0,
    _LPNS,
    0,
    [_PNI, _PNA, _PN, _PNCC, _PNT, _TA, _II, _PND, _SPNA],
    [0, 0, 0, 0, 0, 0, 0, 0, 0],
];
export var ListPhoneNumbersV2Request = [
    3,
    n0,
    _LPNVR,
    0,
    [_TA, _II, _MRa, _NT, _PNCCh, _PNTh, _PNP],
    [0, 0, 1, 0, 64 | 0, 64 | 0, 0],
];
export var ListPhoneNumbersV2Response = [
    3,
    n0,
    _LPNVRi,
    0,
    [_NT, _LPNSL],
    [0, () => ListPhoneNumbersSummaryList],
];
export var ListPredefinedAttributesRequest = [
    3,
    n0,
    _LPAR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListPredefinedAttributesResponse = [
    3,
    n0,
    _LPARi,
    0,
    [_NT, _PASL],
    [0, () => PredefinedAttributeSummaryList],
];
export var ListPromptsRequest = [
    3,
    n0,
    _LPR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListPromptsResponse = [3, n0, _LPRi, 0, [_PSL, _NT], [() => PromptSummaryList, 0]];
export var ListQueueQuickConnectsRequest = [
    3,
    n0,
    _LQQCR,
    0,
    [_II, _QI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListQueueQuickConnectsResponse = [
    3,
    n0,
    _LQQCRi,
    0,
    [_NT, _QCSL, _LMT, _LMR],
    [0, () => QuickConnectSummaryList, 4, 0],
];
export var ListQueuesRequest = [
    3,
    n0,
    _LQR,
    0,
    [_II, _QTu, _NT, _MRa],
    [
        [0, 1],
        [
            64 | 0,
            {
                [_hQ]: _qT,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListQueuesResponse = [3, n0, _LQRi, 0, [_QSL, _NT], [() => QueueSummaryList, 0]];
export var ListQuickConnectsRequest = [
    3,
    n0,
    _LQCR,
    0,
    [_II, _NT, _MRa, _QCT],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            64 | 0,
            {
                [_hQ]: _QCT,
            },
        ],
    ],
];
export var ListQuickConnectsResponse = [
    3,
    n0,
    _LQCRi,
    0,
    [_QCSL, _NT],
    [() => QuickConnectSummaryList, 0],
];
export var ListRealtimeContactAnalysisSegmentsV2Request = [
    3,
    n0,
    _LRCASVR,
    0,
    [_II, _CI, _MRa, _NT, _OT, _STeg],
    [[0, 1], [0, 1], 1, 0, 0, 64 | 0],
];
export var ListRealtimeContactAnalysisSegmentsV2Response = [
    3,
    n0,
    _LRCASVRi,
    0,
    [_C, _St, _Seg, _NT],
    [0, 0, () => RealtimeContactAnalysisSegments, 0],
];
export var ListRoutingProfileManualAssignmentQueuesRequest = [
    3,
    n0,
    _LRPMAQR,
    0,
    [_II, _RPI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListRoutingProfileManualAssignmentQueuesResponse = [
    3,
    n0,
    _LRPMAQRi,
    0,
    [_NT, _RPMAQCSL, _LMT, _LMR],
    [0, () => RoutingProfileManualAssignmentQueueConfigSummaryList, 4, 0],
];
export var ListRoutingProfileQueuesRequest = [
    3,
    n0,
    _LRPQR,
    0,
    [_II, _RPI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListRoutingProfileQueuesResponse = [
    3,
    n0,
    _LRPQRi,
    0,
    [_NT, _RPQCSL, _LMT, _LMR],
    [0, () => RoutingProfileQueueConfigSummaryList, 4, 0],
];
export var ListRoutingProfilesRequest = [
    3,
    n0,
    _LRPR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListRoutingProfilesResponse = [
    3,
    n0,
    _LRPRi,
    0,
    [_RPSL, _NT],
    [() => RoutingProfileSummaryList, 0],
];
export var ListRulesRequest = [
    3,
    n0,
    _LRR,
    0,
    [_II, _PS, _ESN, _MRa, _NT],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _pS,
            },
        ],
        [
            0,
            {
                [_hQ]: _eSN,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
    ],
];
export var ListRulesResponse = [3, n0, _LRRi, 0, [_RSLu, _NT], [() => RuleSummaryList, 0]];
export var ListSecurityKeysRequest = [
    3,
    n0,
    _LSKR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListSecurityKeysResponse = [
    3,
    n0,
    _LSKRi,
    0,
    [_SK, _NT],
    [() => SecurityKeysList, 0],
];
export var ListSecurityProfileApplicationsRequest = [
    3,
    n0,
    _LSPAR,
    0,
    [_SPI, _II, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListSecurityProfileApplicationsResponse = [
    3,
    n0,
    _LSPARi,
    0,
    [_App, _NT, _LMT, _LMR],
    [() => Applications, 0, 4, 0],
];
export var ListSecurityProfilePermissionsRequest = [
    3,
    n0,
    _LSPPR,
    0,
    [_SPI, _II, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListSecurityProfilePermissionsResponse = [
    3,
    n0,
    _LSPPRi,
    0,
    [_Pe, _NT, _LMT, _LMR],
    [64 | 0, 0, 4, 0],
];
export var ListSecurityProfilesRequest = [
    3,
    n0,
    _LSPR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListSecurityProfilesResponse = [
    3,
    n0,
    _LSPRi,
    0,
    [_SPSL, _NT],
    [() => SecurityProfileSummaryList, 0],
];
export var ListTagsForResourceRequest = [3, n0, _LTFRR, 0, [_rA], [[0, 1]]];
export var ListTagsForResourceResponse = [3, n0, _LTFRRi, 0, [_t], [128 | 0]];
export var ListTaskTemplatesRequest = [
    3,
    n0,
    _LTTR,
    0,
    [_II, _NT, _MRa, _St, _N],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _sta,
            },
        ],
        [
            0,
            {
                [_hQ]: _n,
            },
        ],
    ],
];
export var ListTaskTemplatesResponse = [
    3,
    n0,
    _LTTRi,
    0,
    [_TTa, _NT],
    [() => TaskTemplateList, 0],
];
export var ListTrafficDistributionGroupsRequest = [
    3,
    n0,
    _LTDGR,
    0,
    [_MRa, _NT, _II],
    [
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            0,
            {
                [_hQ]: _iI,
            },
        ],
    ],
];
export var ListTrafficDistributionGroupsResponse = [
    3,
    n0,
    _LTDGRi,
    0,
    [_NT, _TDGSL],
    [0, () => TrafficDistributionGroupSummaryList],
];
export var ListTrafficDistributionGroupUsersRequest = [
    3,
    n0,
    _LTDGUR,
    0,
    [_TDGI, _MRa, _NT],
    [
        [0, 1],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
    ],
];
export var ListTrafficDistributionGroupUsersResponse = [
    3,
    n0,
    _LTDGURi,
    0,
    [_NT, _TDGUSL],
    [0, () => TrafficDistributionGroupUserSummaryList],
];
export var ListUseCasesRequest = [
    3,
    n0,
    _LUCR,
    0,
    [_II, _IAI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListUseCasesResponse = [
    3,
    n0,
    _LUCRi,
    0,
    [_UCSL, _NT],
    [() => UseCaseSummaryList, 0],
];
export var ListUserHierarchyGroupsRequest = [
    3,
    n0,
    _LUHGR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListUserHierarchyGroupsResponse = [
    3,
    n0,
    _LUHGRi,
    0,
    [_UHGSL, _NT],
    [() => HierarchyGroupSummaryList, 0],
];
export var ListUserProficienciesRequest = [
    3,
    n0,
    _LUPR,
    0,
    [_II, _UI, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListUserProficienciesResponse = [
    3,
    n0,
    _LUPRi,
    0,
    [_NT, _UPL, _LMT, _LMR],
    [0, () => UserProficiencyList, 4, 0],
];
export var ListUsersRequest = [
    3,
    n0,
    _LUR,
    0,
    [_II, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListUsersResponse = [3, n0, _LURi, 0, [_USL, _NT], [() => UserSummaryList, 0]];
export var ListViewsRequest = [
    3,
    n0,
    _LVR,
    0,
    [_II, _T, _NT, _MRa],
    [
        [0, 1],
        [
            0,
            {
                [_hQ]: _ty,
            },
        ],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListViewsResponse = [3, n0, _LVRi, 0, [_VSL, _NT], [[() => ViewsSummaryList, 0], 0]];
export var ListViewVersionsRequest = [
    3,
    n0,
    _LVVR,
    0,
    [_II, _VIi, _NT, _MRa],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _nT,
            },
        ],
        [
            1,
            {
                [_hQ]: _mR,
            },
        ],
    ],
];
export var ListViewVersionsResponse = [
    3,
    n0,
    _LVVRi,
    0,
    [_VVSL, _NT],
    [[() => ViewVersionSummaryList, 0], 0],
];
export var MatchCriteria = [3, n0, _MC, 0, [_ACge], [() => AgentsCriteria]];
export var MaximumResultReturnedException = [
    -3,
    n0,
    _MRRE,
    {
        [_e]: _c,
        [_hE]: 400,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(MaximumResultReturnedException, __MaximumResultReturnedException);
export var MediaConcurrency = [
    3,
    n0,
    _MCed,
    0,
    [_C, _Conc, _CCB],
    [0, 1, () => CrossChannelBehavior],
];
export var MediaPlacement = [3, n0, _MPe, 0, [_AHU, _AFU, _SUi, _TCU, _EIU], [0, 0, 0, 0, 0]];
export var Meeting = [
    3,
    n0,
    _Me,
    0,
    [_MRe, _MPe, _MF, _MI],
    [0, () => MediaPlacement, () => MeetingFeaturesConfiguration, 0],
];
export var MeetingFeaturesConfiguration = [3, n0, _MFC, 0, [_Au], [() => AudioFeatures]];
export var MetricDataV2 = [3, n0, _MDV, 0, [_Met, _V], [() => MetricV2, 1]];
export var MetricFilterV2 = [3, n0, _MFV, 0, [_MFK, _MFVe, _Ne], [0, 64 | 0, 2]];
export var MetricInterval = [3, n0, _MIe, 0, [_Int, _STtar, _ETnd], [0, 4, 4]];
export var MetricResultV2 = [
    3,
    n0,
    _MRV,
    0,
    [_Dim, _MIe, _Col],
    [128 | 0, () => MetricInterval, () => MetricDataCollectionsV2],
];
export var MetricV2 = [
    3,
    n0,
    _MVe,
    0,
    [_N, _Th, _MFe],
    [0, () => ThresholdCollections, () => MetricFiltersV2List],
];
export var MonitorContactRequest = [
    3,
    n0,
    _MCR,
    0,
    [_II, _CI, _UI, _AMC, _CT],
    [0, 0, 0, 64 | 0, [0, 4]],
];
export var MonitorContactResponse = [3, n0, _MCRo, 0, [_CI, _CA], [0, 0]];
export var NameCriteria = [3, n0, _NCa, 0, [_ST, _MTa], [[() => SearchTextList, 0], 0]];
export var NewSessionDetails = [
    3,
    n0,
    _NSD,
    0,
    [_SMCT, _PD, _Attr, _SCtr],
    [64 | 0, () => ParticipantDetails, 128 | 0, () => ChatStreamingConfiguration],
];
export var NotificationRecipientType = [3, n0, _NRT, 0, [_UT, _UIse], [128 | 0, 64 | 0]];
export var NumberCondition = [3, n0, _NC, 0, [_FNi, _MV, _MVa, _CTo], [0, 1, 1, 0]];
export var NumberReference = [3, n0, _NR, 0, [_N, _V], [0, 0]];
export var NumericQuestionPropertyValueAutomation = [3, n0, _NQPVA, 0, [_La], [0]];
export var OperationalHour = [
    3,
    n0,
    _OHp,
    0,
    [_Star, _End_],
    [() => OverrideTimeSlice, () => OverrideTimeSlice],
];
export var OutboundAdditionalRecipients = [
    3,
    n0,
    _OAR,
    0,
    [_CEA],
    [[() => EmailAddressRecipientList, 0]],
];
export var OutboundCallerConfig = [3, n0, _OCC, 0, [_OCIN, _OCINI, _OFI], [0, 0, 0]];
export var OutboundContactNotPermittedException = [
    -3,
    n0,
    _OCNPE,
    {
        [_e]: _c,
        [_hE]: 403,
        [_aQE]: [`OutboundContactNotPermittedException`, 403],
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(OutboundContactNotPermittedException, __OutboundContactNotPermittedException);
export var OutboundEmailConfig = [3, n0, _OEC, 0, [_OEAI], [0]];
export var OutboundEmailContent = [
    3,
    n0,
    _OECu,
    0,
    [_MST, _TMC, _RM],
    [0, () => TemplatedMessageConfig, [() => OutboundRawMessage, 0]],
];
export var OutboundRawMessage = [
    3,
    n0,
    _ORM,
    0,
    [_Su, _B, _CTon],
    [[() => OutboundSubject, 0], [() => Body, 0], 0],
];
export var OutboundStrategy = [3, n0, _OSu, 0, [_T, _Conf], [0, () => OutboundStrategyConfig]];
export var OutboundStrategyConfig = [3, n0, _OSC, 0, [_AF], [() => AgentFirst]];
export var OutputTypeNotFoundException = [
    -3,
    n0,
    _OTNFE,
    {
        [_e]: _c,
        [_hE]: 404,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(OutputTypeNotFoundException, __OutputTypeNotFoundException);
export var OverrideTimeSlice = [3, n0, _OTS, 0, [_H, _Mi], [1, 1]];
export var ParticipantCapabilities = [3, n0, _PCa, 0, [_Vid, _SSc], [0, 0]];
export var ParticipantDetails = [3, n0, _PD, 0, [_DN], [0]];
export var ParticipantDetailsToAdd = [
    3,
    n0,
    _PDTA,
    0,
    [_PR, _DN, _PCa],
    [0, 0, () => ParticipantCapabilities],
];
export var ParticipantMetrics = [
    3,
    n0,
    _PM,
    0,
    [_PI, _PT, _CAon, _MS, _NRu, _MLIC, _TRTIM, _MRTIM, _LMTa],
    [0, 0, 2, 1, 1, 1, 1, 1, 4],
];
export var ParticipantTimerConfiguration = [
    3,
    n0,
    _PTC,
    0,
    [_PR, _TTi, _TV],
    [0, 0, () => ParticipantTimerValue],
];
export var ParticipantTokenCredentials = [3, n0, _PTCa, 0, [_PTa, _Ex], [0, 0]];
export var PauseContactRequest = [3, n0, _PCR, 0, [_CI, _II, _CFI], [0, 0, 0]];
export var PauseContactResponse = [3, n0, _PCRa, 0, [], []];
export var PersistentChat = [3, n0, _PCe, 0, [_RTe, _SCI], [0, 0]];
export var PhoneNumberQuickConnectConfig = [3, n0, _PNQCC, 0, [_PN], [0]];
export var PhoneNumberStatus = [3, n0, _PNS, 0, [_St, _M], [0, 0]];
export var PhoneNumberSummary = [3, n0, _PNSh, 0, [_I, _A, _PN, _PNT, _PNCC], [0, 0, 0, 0, 0]];
export var PostAcceptTimeoutConfig = [3, n0, _PATC, 0, [_DIS], [1]];
export var PredefinedAttribute = [
    3,
    n0,
    _PA,
    0,
    [_N, _Va, _Pu, _ACtt, _LMT, _LMR],
    [0, () => PredefinedAttributeValues, 64 | 0, () => PredefinedAttributeConfiguration, 4, 0],
];
export var PredefinedAttributeConfiguration = [3, n0, _PAC, 0, [_EVVOA, _IRO], [2, 2]];
export var PredefinedAttributeSearchCriteria = [
    3,
    n0,
    _PASC,
    0,
    [_OC, _ACn, _SC],
    [() => PredefinedAttributeSearchConditionList, () => PredefinedAttributeSearchConditionList, () => StringCondition],
];
export var PredefinedAttributeSummary = [3, n0, _PAS, 0, [_N, _LMT, _LMR], [0, 4, 0]];
export var Preview = [3, n0, _Pr, 0, [_PATC, _AUA], [() => PostAcceptTimeoutConfig, 64 | 0]];
export var ProblemDetail = [3, n0, _PDr, 0, [_m], [0]];
export var Prompt = [
    3,
    n0,
    _Pro,
    0,
    [_PARN, _PIr, _N, _D, _Ta, _LMT, _LMR],
    [0, 0, 0, 0, 128 | 0, 4, 0],
];
export var PromptSearchCriteria = [
    3,
    n0,
    _PSC,
    0,
    [_OC, _ACn, _SC],
    [() => PromptSearchConditionList, () => PromptSearchConditionList, () => StringCondition],
];
export var PromptSearchFilter = [3, n0, _PSF, 0, [_TF], [() => ControlPlaneTagFilter]];
export var PromptSummary = [3, n0, _PSr, 0, [_I, _A, _N, _LMT, _LMR], [0, 0, 0, 4, 0]];
export var PropertyValidationException = [
    -3,
    n0,
    _PVE,
    {
        [_e]: _c,
        [_hE]: 400,
    },
    [_M, _PLr],
    [0, () => PropertyValidationExceptionPropertyList],
];
TypeRegistry.for(n0).registerError(PropertyValidationException, __PropertyValidationException);
export var PropertyValidationExceptionProperty = [3, n0, _PVEP, 0, [_PP, _Rea, _M], [0, 0, 0]];
export var PutUserStatusRequest = [3, n0, _PUSR, 0, [_UI, _II, _ASI], [[0, 1], [0, 1], 0]];
export var PutUserStatusResponse = [3, n0, _PUSRu, 0, [], []];
export var QualityMetrics = [
    3,
    n0,
    _QM,
    0,
    [_Ag, _Cu],
    [() => AgentQualityMetrics, () => CustomerQualityMetrics],
];
export var Queue = [
    3,
    n0,
    _Q,
    0,
    [_N, _QA, _QI, _D, _OCC, _OEC, _HOOI, _MCa, _St, _Ta, _LMT, _LMR],
    [0, 0, 0, 0, () => OutboundCallerConfig, () => OutboundEmailConfig, 0, 1, 0, 128 | 0, 4, 0],
];
export var QueueInfo = [3, n0, _QIu, 0, [_I, _ETn], [0, 4]];
export var QueueInfoInput = [3, n0, _QII, 0, [_I], [0]];
export var QueueQuickConnectConfig = [3, n0, _QQCC, 0, [_QI, _CFI], [0, 0]];
export var QueueReference = [3, n0, _QRu, 0, [_I, _A], [0, 0]];
export var QueueSearchCriteria = [
    3,
    n0,
    _QSC,
    0,
    [_OC, _ACn, _SC, _QTC],
    [() => QueueSearchConditionList, () => QueueSearchConditionList, () => StringCondition, 0],
];
export var QueueSearchFilter = [3, n0, _QSF, 0, [_TF], [() => ControlPlaneTagFilter]];
export var QueueSummary = [3, n0, _QSu, 0, [_I, _A, _N, _QTue, _LMT, _LMR], [0, 0, 0, 0, 4, 0]];
export var QuickConnect = [
    3,
    n0,
    _QCu,
    0,
    [_QCARN, _QCIu, _N, _D, _QCC, _Ta, _LMT, _LMR],
    [0, 0, 0, 0, () => QuickConnectConfig, 128 | 0, 4, 0],
];
export var QuickConnectConfig = [
    3,
    n0,
    _QCC,
    0,
    [_QCTu, _UC, _QCue, _PCh],
    [0, () => UserQuickConnectConfig, () => QueueQuickConnectConfig, () => PhoneNumberQuickConnectConfig],
];
export var QuickConnectSearchCriteria = [
    3,
    n0,
    _QCSC,
    0,
    [_OC, _ACn, _SC],
    [() => QuickConnectSearchConditionList, () => QuickConnectSearchConditionList, () => StringCondition],
];
export var QuickConnectSearchFilter = [3, n0, _QCSF, 0, [_TF], [() => ControlPlaneTagFilter]];
export var QuickConnectSummary = [
    3,
    n0,
    _QCS,
    0,
    [_I, _A, _N, _QCTu, _LMT, _LMR],
    [0, 0, 0, 0, 4, 0],
];
export var Range = [3, n0, _R, 0, [_MPL, _MPLa], [1, 1]];
export var ReadOnlyFieldInfo = [3, n0, _ROFI, 0, [_I], [() => TaskTemplateFieldIdentifier]];
export var RealTimeContactAnalysisAttachment = [
    3,
    n0,
    _RTCAA,
    0,
    [_ANt, _CTon, _AItt, _St],
    [0, 0, 0, 0],
];
export var RealTimeContactAnalysisCategoryDetails = [
    3,
    n0,
    _RTCACD,
    0,
    [_POI],
    [() => RealTimeContactAnalysisPointsOfInterest],
];
export var RealTimeContactAnalysisCharacterInterval = [3, n0, _RTCACI, 0, [_BOC, _EOC], [1, 1]];
export var RealTimeContactAnalysisIssueDetected = [
    3,
    n0,
    _RTCAID,
    0,
    [_TIr],
    [() => RealTimeContactAnalysisTranscriptItemsWithContent],
];
export var RealTimeContactAnalysisPointOfInterest = [
    3,
    n0,
    _RTCAPOI,
    0,
    [_TIr],
    [() => RealTimeContactAnalysisTranscriptItemsWithCharacterOffsets],
];
export var RealTimeContactAnalysisSegmentAttachments = [
    3,
    n0,
    _RTCASA,
    0,
    [_I, _PI, _PR, _DN, _Atta, _Tim],
    [0, 0, 0, 0, () => RealTimeContactAnalysisAttachments, () => RealTimeContactAnalysisTimeData],
];
export var RealTimeContactAnalysisSegmentCategories = [
    3,
    n0,
    _RTCASC,
    0,
    [_MD],
    [() => RealTimeContactAnalysisMatchedDetails],
];
export var RealTimeContactAnalysisSegmentEvent = [
    3,
    n0,
    _RTCASE,
    0,
    [_I, _PI, _PR, _DN, _ETve, _Tim],
    [0, 0, 0, 0, 0, () => RealTimeContactAnalysisTimeData],
];
export var RealTimeContactAnalysisSegmentIssues = [
    3,
    n0,
    _RTCASI,
    0,
    [_IDs],
    [() => RealTimeContactAnalysisIssuesDetected],
];
export var RealTimeContactAnalysisSegmentPostContactSummary = [
    3,
    n0,
    _RTCASPCS,
    0,
    [_Co, _St, _FC],
    [0, 0, 0],
];
export var RealTimeContactAnalysisSegmentTranscript = [
    3,
    n0,
    _RTCAST,
    0,
    [_I, _PI, _PR, _DN, _Co, _CTon, _Tim, _Red, _Sen],
    [0, 0, 0, 0, 0, 0, () => RealTimeContactAnalysisTimeData, () => RealTimeContactAnalysisTranscriptItemRedaction, 0],
];
export var RealTimeContactAnalysisTranscriptItemRedaction = [
    3,
    n0,
    _RTCATIR,
    0,
    [_COh],
    [() => RealTimeContactAnalysisCharacterIntervals],
];
export var RealTimeContactAnalysisTranscriptItemWithCharacterOffsets = [
    3,
    n0,
    _RTCATIWCO,
    0,
    [_I, _COh],
    [0, () => RealTimeContactAnalysisCharacterInterval],
];
export var RealTimeContactAnalysisTranscriptItemWithContent = [
    3,
    n0,
    _RTCATIWC,
    0,
    [_Co, _I, _COh],
    [0, 0, () => RealTimeContactAnalysisCharacterInterval],
];
export var RecordingInfo = [
    3,
    n0,
    _RIec,
    0,
    [_STto, _Lo, _MSTe, _PT, _FSN, _FSNr, _STta, _STtop, _St, _DRe],
    [0, 0, 0, 0, 0, 0, 4, 4, 0, 0],
];
export var Reference = [3, n0, _Refe, 0, [_V, _T, _St, _A, _SRt], [0, 0, 0, 0, 0]];
export var ReleasePhoneNumberRequest = [
    3,
    n0,
    _RPNR,
    0,
    [_PNI, _CT],
    [
        [0, 1],
        [
            0,
            {
                [_iT]: 1,
                [_hQ]: _cT,
            },
        ],
    ],
];
export var ReplicateInstanceRequest = [
    3,
    n0,
    _RIR,
    0,
    [_II, _RR, _CT, _RAep],
    [[0, 1], 0, [0, 4], [() => DirectoryAlias, 0]],
];
export var ReplicateInstanceResponse = [3, n0, _RIRe, 0, [_I, _A], [0, 0]];
export var ReplicationConfiguration = [
    3,
    n0,
    _RCe,
    0,
    [_RSSL, _SRo, _GSIE],
    [() => ReplicationStatusSummaryList, 0, 0],
];
export var ReplicationStatusSummary = [3, n0, _RSSe, 0, [_Reg, _RS, _RSR], [0, 0, 0]];
export var RequiredFieldInfo = [3, n0, _RFI, 0, [_I], [() => TaskTemplateFieldIdentifier]];
export var ResourceConflictException = [
    -3,
    n0,
    _RCE,
    {
        [_e]: _c,
        [_hE]: 409,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(ResourceConflictException, __ResourceConflictException);
export var ResourceInUseException = [
    -3,
    n0,
    _RIUE,
    {
        [_e]: _c,
        [_hE]: 409,
    },
    [_M, _RT, _RI],
    [0, 0, 0],
];
TypeRegistry.for(n0).registerError(ResourceInUseException, __ResourceInUseException);
export var ResourceNotFoundException = [
    -3,
    n0,
    _RNFE,
    {
        [_e]: _c,
        [_hE]: 404,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(ResourceNotFoundException, __ResourceNotFoundException);
export var ResourceNotReadyException = [
    -3,
    n0,
    _RNRE,
    {
        [_e]: _c,
        [_hE]: 409,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(ResourceNotReadyException, __ResourceNotReadyException);
export var ResourceTagsSearchCriteria = [3, n0, _RTSC, 0, [_TSC], [() => TagSearchCondition]];
export var ResumeContactRecordingRequest = [
    3,
    n0,
    _RCRR,
    0,
    [_II, _CI, _ICI, _CRT],
    [0, 0, 0, 0],
];
export var ResumeContactRecordingResponse = [3, n0, _RCRRe, 0, [], []];
export var ResumeContactRequest = [3, n0, _RCR, 0, [_CI, _II, _CFI], [0, 0, 0]];
export var ResumeContactResponse = [3, n0, _RCRe, 0, [], []];
export var RoutingCriteria = [3, n0, _RC, 0, [_Ste, _ATct, _Ind], [() => Steps, 4, 1]];
export var RoutingCriteriaInput = [3, n0, _RCIo, 0, [_Ste], [() => RoutingCriteriaInputSteps]];
export var RoutingCriteriaInputStep = [
    3,
    n0,
    _RCIS,
    0,
    [_Ex, _Exp],
    [() => RoutingCriteriaInputStepExpiry, () => Expression],
];
export var RoutingCriteriaInputStepExpiry = [3, n0, _RCISE, 0, [_DIS], [1]];
export var RoutingProfile = [
    3,
    n0,
    _RP,
    0,
    [_II, _N, _RPA, _RPI, _D, _MCe, _DOQI, _Ta, _NOAQ, _NOAMAQ, _NOAU, _AAT, _LMT, _LMR, _ID, _AQI, _AMAQI],
    [0, 0, 0, 0, 0, () => MediaConcurrencies, 0, 128 | 0, 1, 1, 1, 0, 4, 0, 2, 64 | 0, 64 | 0],
];
export var RoutingProfileManualAssignmentQueueConfig = [
    3,
    n0,
    _RPMAQC,
    0,
    [_QRu],
    [() => RoutingProfileQueueReference],
];
export var RoutingProfileManualAssignmentQueueConfigSummary = [
    3,
    n0,
    _RPMAQCS,
    0,
    [_QI, _QA, _QN, _C],
    [0, 0, 0, 0],
];
export var RoutingProfileQueueConfig = [
    3,
    n0,
    _RPQC,
    0,
    [_QRu, _Pri, _Del],
    [() => RoutingProfileQueueReference, 1, 1],
];
export var RoutingProfileQueueConfigSummary = [
    3,
    n0,
    _RPQCS,
    0,
    [_QI, _QA, _QN, _Pri, _Del, _C],
    [0, 0, 0, 1, 1, 0],
];
export var RoutingProfileQueueReference = [3, n0, _RPQR, 0, [_QI, _C], [0, 0]];
export var RoutingProfileReference = [3, n0, _RPR, 0, [_I, _A], [0, 0]];
export var RoutingProfileSearchCriteria = [
    3,
    n0,
    _RPSC,
    0,
    [_OC, _ACn, _SC],
    [() => RoutingProfileSearchConditionList, () => RoutingProfileSearchConditionList, () => StringCondition],
];
export var RoutingProfileSearchFilter = [3, n0, _RPSF, 0, [_TF], [() => ControlPlaneTagFilter]];
export var RoutingProfileSummary = [3, n0, _RPS, 0, [_I, _A, _N, _LMT, _LMR], [0, 0, 0, 4, 0]];
export var Rule = [
    3,
    n0,
    _Ru,
    0,
    [_N, _RIu, _RA, _TES, _Fu, _Ac, _PS, _CTre, _LUTa, _LUB, _Ta],
    [0, 0, 0, () => RuleTriggerEventSource, 0, () => RuleActions, 0, 4, 4, 0, 128 | 0],
];
export var RuleAction = [
    3,
    n0,
    _RAu,
    0,
    [_AT, _TAa, _EBA, _ACCA, _SNAe, _CCA, _UCAp, _ASA, _EATA, _SAEA],
    [
        0,
        () => TaskActionDefinition,
        () => EventBridgeActionDefinition,
        () => AssignContactCategoryActionDefinition,
        () => SendNotificationActionDefinition,
        () => CreateCaseActionDefinition,
        () => UpdateCaseActionDefinition,
        () => AssignSlaActionDefinition,
        () => EndAssociatedTasksActionDefinition,
        () => SubmitAutoEvaluationActionDefinition,
    ],
];
export var RuleSummary = [
    3,
    n0,
    _RSu,
    0,
    [_N, _RIu, _RA, _ESN, _PS, _ASc, _CTre, _LUTa],
    [0, 0, 0, 0, 0, () => ActionSummaries, 4, 4],
];
export var RuleTriggerEventSource = [3, n0, _RTES, 0, [_ESN, _IAI], [0, 0]];
export var S3Config = [3, n0, _SCon, 0, [_BNu, _BP, _ECn], [0, 0, () => EncryptionConfig]];
export var SearchableAgentCriteriaStep = [3, n0, _SACS, 0, [_AIg, _MTa], [64 | 0, 0]];
export var SearchableContactAttributes = [
    3,
    n0,
    _SCA,
    0,
    [_Cri, _MTa],
    [[() => SearchableContactAttributesCriteriaList, 0], 0],
];
export var SearchableContactAttributesCriteria = [
    3,
    n0,
    _SCAC,
    0,
    [_K, _Va],
    [
        [() => SearchableContactAttributeKey, 0],
        [() => SearchableContactAttributeValueList, 0],
    ],
];
export var SearchableRoutingCriteria = [
    3,
    n0,
    _SRC,
    0,
    [_Ste],
    [() => SearchableRoutingCriteriaStepList],
];
export var SearchableRoutingCriteriaStep = [
    3,
    n0,
    _SRCS,
    0,
    [_ACgen],
    [() => SearchableAgentCriteriaStep],
];
export var SearchableSegmentAttributes = [
    3,
    n0,
    _SSA,
    0,
    [_Cri, _MTa],
    [[() => SearchableSegmentAttributesCriteriaList, 0], 0],
];
export var SearchableSegmentAttributesCriteria = [
    3,
    n0,
    _SSAC,
    0,
    [_K, _Va],
    [
        [() => SearchableSegmentAttributeKey, 0],
        [() => SearchableSegmentAttributeValueList, 0],
    ],
];
export var SearchAgentStatusesRequest = [
    3,
    n0,
    _SASR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => AgentStatusSearchFilter, () => AgentStatusSearchCriteria],
];
export var SearchAgentStatusesResponse = [
    3,
    n0,
    _SASRe,
    0,
    [_ASge, _NT, _ATC],
    [() => AgentStatusList, 0, 1],
];
export var SearchAvailablePhoneNumbersRequest = [
    3,
    n0,
    _SAPNR,
    0,
    [_TA, _II, _PNCC, _PNT, _PNP, _MRa, _NT],
    [0, 0, 0, 0, 0, 1, 0],
];
export var SearchAvailablePhoneNumbersResponse = [
    3,
    n0,
    _SAPNRe,
    0,
    [_NT, _ANL],
    [0, () => AvailableNumbersList],
];
export var SearchContactEvaluationsRequest = [
    3,
    n0,
    _SCER,
    0,
    [_II, _NT, _MRa, _SCe, _SF],
    [0, 0, 1, () => EvaluationSearchCriteria, () => EvaluationSearchFilter],
];
export var SearchContactEvaluationsResponse = [
    3,
    n0,
    _SCERe,
    0,
    [_ESSL, _NT, _ATC],
    [() => EvaluationSearchSummaryList, 0, 1],
];
export var SearchContactFlowModulesRequest = [
    3,
    n0,
    _SCFMR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => ContactFlowModuleSearchFilter, () => ContactFlowModuleSearchCriteria],
];
export var SearchContactFlowModulesResponse = [
    3,
    n0,
    _SCFMRe,
    0,
    [_CFMo, _NT, _ATC],
    [() => ContactFlowModuleSearchSummaryList, 0, 1],
];
export var SearchContactFlowsRequest = [
    3,
    n0,
    _SCFR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => ContactFlowSearchFilter, () => ContactFlowSearchCriteria],
];
export var SearchContactFlowsResponse = [
    3,
    n0,
    _SCFRe,
    0,
    [_CFon, _NT, _ATC],
    [() => ContactFlowSearchSummaryList, 0, 1],
];
export var SearchContactsAdditionalTimeRange = [
    3,
    n0,
    _SCATR,
    0,
    [_Cri, _MTa],
    [() => SearchContactsAdditionalTimeRangeCriteriaList, 0],
];
export var SearchContactsAdditionalTimeRangeCriteria = [
    3,
    n0,
    _SCATRC,
    0,
    [_TR, _TCi],
    [() => SearchContactsTimeRange, () => SearchContactsTimestampCondition],
];
export var SearchContactsRequest = [
    3,
    n0,
    _SCR,
    0,
    [_II, _TR, _SCe, _MRa, _NT, _Sor],
    [0, () => SearchContactsTimeRange, [() => SearchCriteria, 0], 1, 0, () => Sort],
];
export var SearchContactsResponse = [
    3,
    n0,
    _SCRe,
    0,
    [_Conta, _NT, _TCo],
    [[() => Contacts, 0], 0, 1],
];
export var SearchContactsTimeRange = [3, n0, _SCTR, 0, [_T, _STtar, _ETnd], [0, 4, 4]];
export var SearchContactsTimestampCondition = [3, n0, _SCTC, 0, [_T, _CTond], [0, 0]];
export var SearchCriteria = [
    3,
    n0,
    _SCe,
    0,
    [_N, _AIg, _AHGg, _Ch, _CAo, _IMn, _QIue, _RC, _ATR, _SCA, _SSA],
    [
        [() => NameCriteria, 0],
        64 | 0,
        () => AgentHierarchyGroups,
        64 | 0,
        [() => ContactAnalysis, 0],
        64 | 0,
        64 | 0,
        () => SearchableRoutingCriteria,
        () => SearchContactsAdditionalTimeRange,
        [() => SearchableContactAttributes, 0],
        [() => SearchableSegmentAttributes, 0],
    ],
];
export var SearchEmailAddressesRequest = [
    3,
    n0,
    _SEAR,
    0,
    [_II, _MRa, _NT, _SCe, _SF],
    [0, 1, 0, () => EmailAddressSearchCriteria, () => EmailAddressSearchFilter],
];
export var SearchEmailAddressesResponse = [
    3,
    n0,
    _SEARe,
    0,
    [_NT, _EAma, _ATC],
    [0, [() => EmailAddressList, 0], 1],
];
export var SearchEvaluationFormsRequest = [
    3,
    n0,
    _SEFR,
    0,
    [_II, _NT, _MRa, _SCe, _SF],
    [0, 0, 1, () => EvaluationFormSearchCriteria, () => EvaluationFormSearchFilter],
];
export var SearchEvaluationFormsResponse = [
    3,
    n0,
    _SEFRe,
    0,
    [_EFSSL, _NT, _ATC],
    [() => EvaluationFormSearchSummaryList, 0, 1],
];
export var SearchHoursOfOperationOverridesRequest = [
    3,
    n0,
    _SHOOOR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => HoursOfOperationSearchFilter, () => HoursOfOperationOverrideSearchCriteria],
];
export var SearchHoursOfOperationOverridesResponse = [
    3,
    n0,
    _SHOOORe,
    0,
    [_HOOOo, _NT, _ATC],
    [() => HoursOfOperationOverrideList, 0, 1],
];
export var SearchHoursOfOperationsRequest = [
    3,
    n0,
    _SHOOR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => HoursOfOperationSearchFilter, () => HoursOfOperationSearchCriteria],
];
export var SearchHoursOfOperationsResponse = [
    3,
    n0,
    _SHOORe,
    0,
    [_HOOo, _NT, _ATC],
    [() => HoursOfOperationList, 0, 1],
];
export var SearchPredefinedAttributesRequest = [
    3,
    n0,
    _SPAR,
    0,
    [_II, _NT, _MRa, _SCe],
    [0, 0, 1, () => PredefinedAttributeSearchCriteria],
];
export var SearchPredefinedAttributesResponse = [
    3,
    n0,
    _SPARe,
    0,
    [_PAr, _NT, _ATC],
    [() => PredefinedAttributeSearchSummaryList, 0, 1],
];
export var SearchPromptsRequest = [
    3,
    n0,
    _SPR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => PromptSearchFilter, () => PromptSearchCriteria],
];
export var SearchPromptsResponse = [
    3,
    n0,
    _SPRe,
    0,
    [_Prom, _NT, _ATC],
    [() => PromptList, 0, 1],
];
export var SearchQueuesRequest = [
    3,
    n0,
    _SQR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => QueueSearchFilter, () => QueueSearchCriteria],
];
export var SearchQueuesResponse = [
    3,
    n0,
    _SQRe,
    0,
    [_Qu, _NT, _ATC],
    [() => QueueSearchSummaryList, 0, 1],
];
export var SearchQuickConnectsRequest = [
    3,
    n0,
    _SQCR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => QuickConnectSearchFilter, () => QuickConnectSearchCriteria],
];
export var SearchQuickConnectsResponse = [
    3,
    n0,
    _SQCRe,
    0,
    [_QCui, _NT, _ATC],
    [() => QuickConnectSearchSummaryList, 0, 1],
];
export var SearchResourceTagsRequest = [
    3,
    n0,
    _SRTR,
    0,
    [_II, _RTes, _NT, _MRa, _SCe],
    [0, 64 | 0, 0, 1, () => ResourceTagsSearchCriteria],
];
export var SearchResourceTagsResponse = [3, n0, _SRTRe, 0, [_Ta, _NT], [() => TagsList, 0]];
export var SearchRoutingProfilesRequest = [
    3,
    n0,
    _SRPR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => RoutingProfileSearchFilter, () => RoutingProfileSearchCriteria],
];
export var SearchRoutingProfilesResponse = [
    3,
    n0,
    _SRPRe,
    0,
    [_RPo, _NT, _ATC],
    [() => RoutingProfileList, 0, 1],
];
export var SearchSecurityProfilesRequest = [
    3,
    n0,
    _SSPR,
    0,
    [_II, _NT, _MRa, _SCe, _SF],
    [0, 0, 1, () => SecurityProfileSearchCriteria, () => SecurityProfilesSearchFilter],
];
export var SearchSecurityProfilesResponse = [
    3,
    n0,
    _SSPRe,
    0,
    [_SPe, _NT, _ATC],
    [() => SecurityProfilesSearchSummaryList, 0, 1],
];
export var SearchUserHierarchyGroupsRequest = [
    3,
    n0,
    _SUHGR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => UserHierarchyGroupSearchFilter, () => UserHierarchyGroupSearchCriteria],
];
export var SearchUserHierarchyGroupsResponse = [
    3,
    n0,
    _SUHGRe,
    0,
    [_UHG, _NT, _ATC],
    [() => UserHierarchyGroupList, 0, 1],
];
export var SearchUsersRequest = [
    3,
    n0,
    _SUR,
    0,
    [_II, _NT, _MRa, _SF, _SCe],
    [0, 0, 1, () => UserSearchFilter, () => UserSearchCriteria],
];
export var SearchUsersResponse = [
    3,
    n0,
    _SURe,
    0,
    [_Use, _NT, _ATC],
    [[() => UserSearchSummaryList, 0], 0, 1],
];
export var SearchVocabulariesRequest = [
    3,
    n0,
    _SVR,
    0,
    [_II, _MRa, _NT, _S, _NSW, _LC],
    [[0, 1], 1, 0, 0, 0, 0],
];
export var SearchVocabulariesResponse = [
    3,
    n0,
    _SVRe,
    0,
    [_VSLo, _NT],
    [() => VocabularySummaryList, 0],
];
export var SecurityKey = [3, n0, _SKe, 0, [_AIs, _K, _CTr], [0, 0, 4]];
export var SecurityProfile = [
    3,
    n0,
    _SP,
    0,
    [_I, _ORI, _A, _SPN, _D, _Ta, _AACT, _TRR, _LMT, _LMR, _HRR, _AACHGI],
    [0, 0, 0, 0, 0, 128 | 0, 128 | 0, 64 | 0, 4, 0, 64 | 0, 0],
];
export var SecurityProfileSearchCriteria = [
    3,
    n0,
    _SPSC,
    0,
    [_OC, _ACn, _SC],
    [() => SecurityProfileSearchConditionList, () => SecurityProfileSearchConditionList, () => StringCondition],
];
export var SecurityProfileSearchSummary = [
    3,
    n0,
    _SPSS,
    0,
    [_I, _ORI, _A, _SPN, _D, _Ta],
    [0, 0, 0, 0, 0, 128 | 0],
];
export var SecurityProfilesSearchFilter = [
    3,
    n0,
    _SPSF,
    0,
    [_TF],
    [() => ControlPlaneTagFilter],
];
export var SecurityProfileSummary = [3, n0, _SPS, 0, [_I, _A, _N, _LMT, _LMR], [0, 0, 0, 4, 0]];
export var SegmentAttributeValue = [
    3,
    n0,
    _SAV,
    0,
    [_VS, _VM, _VIa, _VL, _VAa],
    [0, () => SegmentAttributeValueMap, 1, () => SegmentAttributeValueList, 0],
];
export var SendChatIntegrationEventRequest = [
    3,
    n0,
    _SCIER,
    0,
    [_SI, _DIe, _Sub, _Eve, _NSD],
    [0, 0, 0, () => ChatEvent, () => NewSessionDetails],
];
export var SendChatIntegrationEventResponse = [3, n0, _SCIERe, 0, [_ICI, _NCC], [0, 2]];
export var SendNotificationActionDefinition = [
    3,
    n0,
    _SNAD,
    0,
    [_DM, _Su, _Co, _CTon, _Rec, _Exc],
    [0, 0, 0, 0, () => NotificationRecipientType, () => NotificationRecipientType],
];
export var SendOutboundEmailRequest = [
    3,
    n0,
    _SOER,
    0,
    [_II, _FEA, _DEA, _ARd, _EMm, _TTr, _SCou, _CT],
    [
        [0, 1],
        [() => EmailAddressInfo, 0],
        [() => EmailAddressInfo, 0],
        [() => OutboundAdditionalRecipients, 0],
        [() => OutboundEmailContent, 0],
        0,
        () => SourceCampaign,
        [0, 4],
    ],
];
export var SendOutboundEmailResponse = [3, n0, _SOERe, 0, [], []];
export var ServiceQuotaExceededException = [
    -3,
    n0,
    _SQEE,
    {
        [_e]: _c,
        [_hE]: 402,
    },
    [_M, _Rea],
    [0, () => ServiceQuotaExceededExceptionReason],
];
TypeRegistry.for(n0).registerError(ServiceQuotaExceededException, __ServiceQuotaExceededException);
export var SignInConfig = [3, n0, _SIC, 0, [_Di], [() => SignInDistributionList]];
export var SignInDistribution = [3, n0, _SIDi, 0, [_Reg, _En], [0, 2]];
export var SingleSelectQuestionRuleCategoryAutomation = [
    3,
    n0,
    _SSQRCA,
    0,
    [_Cat, _Con, _ORIp],
    [0, 0, 0],
];
export var Sort = [3, n0, _Sor, 0, [_FNi, _Ord], [0, 0]];
export var SourceCampaign = [3, n0, _SCou, 0, [_CIa, _ORIu], [0, 0]];
export var StartAttachedFileUploadRequest = [
    3,
    n0,
    _SAFUR,
    0,
    [_CT, _II, _FN, _FSIB, _UEIS, _FUCT, _ARA, _CB, _Ta],
    [
        [0, 4],
        [0, 1],
        0,
        1,
        1,
        0,
        [
            0,
            {
                [_hQ]: _aRA,
            },
        ],
        () => CreatedByInfo,
        128 | 0,
    ],
];
export var StartAttachedFileUploadResponse = [
    3,
    n0,
    _SAFURt,
    0,
    [_FAi, _FIi, _CTr, _FS, _CB, _UUM],
    [0, 0, 0, 0, () => CreatedByInfo, () => UploadUrlMetadata],
];
export var StartChatContactRequest = [
    3,
    n0,
    _SCCR,
    0,
    [_II, _CFI, _Attr, _PD, _IMni, _CT, _CDIM, _SMCT, _PCe, _RCI, _SAe, _CIu],
    [
        0,
        0,
        128 | 0,
        () => ParticipantDetails,
        () => ChatMessage,
        [0, 4],
        1,
        64 | 0,
        () => PersistentChat,
        0,
        () => SegmentAttributes,
        [() => CustomerIdNonEmpty, 0],
    ],
];
export var StartChatContactResponse = [3, n0, _SCCRt, 0, [_CI, _PI, _PTa, _CFCI], [0, 0, 0, 0]];
export var StartContactEvaluationRequest = [
    3,
    n0,
    _SCERt,
    0,
    [_II, _CI, _EFI, _AEC, _CT, _Ta],
    [[0, 1], 0, 0, () => AutoEvaluationConfiguration, [0, 4], 128 | 0],
];
export var StartContactEvaluationResponse = [3, n0, _SCERta, 0, [_EI, _EAv], [0, 0]];
export var StartContactRecordingRequest = [
    3,
    n0,
    _SCRR,
    0,
    [_II, _CI, _ICI, _VRC],
    [0, 0, 0, () => VoiceRecordingConfiguration],
];
export var StartContactRecordingResponse = [3, n0, _SCRRt, 0, [], []];
export var StartContactStreamingRequest = [
    3,
    n0,
    _SCSR,
    0,
    [_II, _CI, _CSCh, _CT],
    [0, 0, () => ChatStreamingConfiguration, [0, 4]],
];
export var StartContactStreamingResponse = [3, n0, _SCSRt, 0, [_SIt], [0]];
export var StartEmailContactRequest = [
    3,
    n0,
    _SECR,
    0,
    [_II, _FEA, _DEA, _D, _Ref, _N, _EMm, _ARd, _Atta, _CFI, _RCI, _Attr, _SAe, _CT],
    [
        0,
        [() => EmailAddressInfo, 0],
        [() => EmailAddress, 0],
        [() => Description, 0],
        () => ContactReferences,
        [() => Name, 0],
        [() => InboundEmailContent, 0],
        [() => InboundAdditionalRecipients, 0],
        [() => EmailAttachments, 0],
        0,
        0,
        128 | 0,
        () => SegmentAttributes,
        [0, 4],
    ],
];
export var StartEmailContactResponse = [3, n0, _SECRt, 0, [_CI], [0]];
export var StartOutboundChatContactRequest = [
    3,
    n0,
    _SOCCR,
    0,
    [_SEo, _DE, _II, _SAe, _Attr, _CFI, _CDIM, _PD, _ISM, _RCI, _SMCT, _CT],
    [
        () => Endpoint,
        () => Endpoint,
        0,
        () => SegmentAttributes,
        128 | 0,
        0,
        1,
        () => ParticipantDetails,
        () => ChatMessage,
        0,
        64 | 0,
        [0, 4],
    ],
];
export var StartOutboundChatContactResponse = [3, n0, _SOCCRt, 0, [_CI], [0]];
export var StartOutboundEmailContactRequest = [
    3,
    n0,
    _SOECR,
    0,
    [_II, _CI, _FEA, _DEA, _ARd, _EMm, _CT],
    [
        0,
        0,
        [() => EmailAddressInfo, 0],
        [() => EmailAddressInfo, 0],
        [() => OutboundAdditionalRecipients, 0],
        [() => OutboundEmailContent, 0],
        [0, 4],
    ],
];
export var StartOutboundEmailContactResponse = [3, n0, _SOECRt, 0, [_CI], [0]];
export var StartOutboundVoiceContactRequest = [
    3,
    n0,
    _SOVCR,
    0,
    [_N, _D, _Ref, _RCI, _DPN, _CFI, _II, _CT, _SPNo, _QI, _Attr, _AMDC, _CIa, _TTr, _OSu, _RTIS],
    [
        [() => Name, 0],
        [() => Description, 0],
        () => ContactReferences,
        0,
        0,
        0,
        0,
        [0, 4],
        0,
        0,
        128 | 0,
        () => AnswerMachineDetectionConfig,
        0,
        0,
        () => OutboundStrategy,
        1,
    ],
];
export var StartOutboundVoiceContactResponse = [3, n0, _SOVCRt, 0, [_CI], [0]];
export var StartScreenSharingRequest = [3, n0, _SSSR, 0, [_CT, _II, _CI], [[0, 4], 0, 0]];
export var StartScreenSharingResponse = [3, n0, _SSSRt, 0, [], []];
export var StartTaskContactRequest = [
    3,
    n0,
    _STCR,
    0,
    [_II, _PCI, _CFI, _Attr, _N, _Ref, _D, _CT, _STch, _TTIa, _QCIu, _RCI, _SAe],
    [
        0,
        0,
        0,
        128 | 0,
        [() => Name, 0],
        () => ContactReferences,
        [() => Description, 0],
        [0, 4],
        4,
        0,
        0,
        0,
        () => SegmentAttributes,
    ],
];
export var StartTaskContactResponse = [3, n0, _STCRt, 0, [_CI], [0]];
export var StartWebRTCContactRequest = [
    3,
    n0,
    _SWRTCCR,
    0,
    [_Attr, _CT, _CFI, _II, _ACll, _PD, _RCI, _Ref, _D],
    [
        128 | 0,
        [0, 4],
        0,
        0,
        () => AllowedCapabilities,
        () => ParticipantDetails,
        0,
        () => ContactReferences,
        [() => Description, 0],
    ],
];
export var StartWebRTCContactResponse = [
    3,
    n0,
    _SWRTCCRt,
    0,
    [_CD, _CI, _PI, _PTa],
    [[() => ConnectionData, 0], 0, 0, 0],
];
export var StateTransition = [3, n0, _STtat, 0, [_S, _SST, _SET], [0, 4, 4]];
export var Step = [3, n0, _Step, 0, [_Ex, _Exp, _St], [() => Expiry, () => Expression, 0]];
export var StopContactRecordingRequest = [
    3,
    n0,
    _SCRRto,
    0,
    [_II, _CI, _ICI, _CRT],
    [0, 0, 0, 0],
];
export var StopContactRecordingResponse = [3, n0, _SCRRtop, 0, [], []];
export var StopContactRequest = [
    3,
    n0,
    _SCRt,
    0,
    [_CI, _II, _DR],
    [0, 0, () => DisconnectReason],
];
export var StopContactResponse = [3, n0, _SCRto, 0, [], []];
export var StopContactStreamingRequest = [3, n0, _SCSRto, 0, [_II, _CI, _SIt], [0, 0, 0]];
export var StopContactStreamingResponse = [3, n0, _SCSRtop, 0, [], []];
export var StringCondition = [3, n0, _SC, 0, [_FNi, _V, _CTo], [0, 0, 0]];
export var StringReference = [3, n0, _SRtr, 0, [_N, _V], [0, 0]];
export var SubmitAutoEvaluationActionDefinition = [3, n0, _SAEAD, 0, [_EFI], [0]];
export var SubmitContactEvaluationRequest = [
    3,
    n0,
    _SCERu,
    0,
    [_II, _EI, _An, _No, _SB],
    [[0, 1], [0, 1], () => EvaluationAnswersInputMap, () => EvaluationNotesMap, () => EvaluatorUserUnion],
];
export var SubmitContactEvaluationResponse = [3, n0, _SCERub, 0, [_EI, _EAv], [0, 0]];
export var SuccessfulRequest = [3, n0, _SRu, 0, [_RIeq, _CI], [0, 0]];
export var SuspendContactRecordingRequest = [
    3,
    n0,
    _SCRRu,
    0,
    [_II, _CI, _ICI, _CRT],
    [0, 0, 0, 0],
];
export var SuspendContactRecordingResponse = [3, n0, _SCRRus, 0, [], []];
export var TagCondition = [3, n0, _TCa, 0, [_TK, _TVa], [0, 0]];
export var TagContactRequest = [3, n0, _TCR, 0, [_CI, _II, _Ta], [0, 0, 128 | 0]];
export var TagContactResponse = [3, n0, _TCRa, 0, [], []];
export var TagResourceRequest = [3, n0, _TRRa, 0, [_rA, _t], [[0, 1], 128 | 0]];
export var TagSearchCondition = [3, n0, _TSC, 0, [_tK, _tV, _tKCT, _tVCT], [0, 0, 0, 0]];
export var TagSet = [3, n0, _TSa, 0, [_k, _va], [0, 0]];
export var TaskActionDefinition = [
    3,
    n0,
    _TAD,
    0,
    [_N, _D, _CFI, _Ref],
    [0, 0, 0, () => ContactReferences],
];
export var TaskTemplateConstraints = [
    3,
    n0,
    _TTC,
    0,
    [_RF, _ROF, _IF],
    [() => RequiredTaskTemplateFields, () => ReadOnlyTaskTemplateFields, () => InvisibleTaskTemplateFields],
];
export var TaskTemplateDefaultFieldValue = [
    3,
    n0,
    _TTDFV,
    0,
    [_I, _DVe],
    [() => TaskTemplateFieldIdentifier, 0],
];
export var TaskTemplateDefaults = [
    3,
    n0,
    _TTD,
    0,
    [_DFV],
    [() => TaskTemplateDefaultFieldValueList],
];
export var TaskTemplateField = [
    3,
    n0,
    _TTF,
    0,
    [_I, _D, _T, _SSO],
    [() => TaskTemplateFieldIdentifier, 0, 0, 64 | 0],
];
export var TaskTemplateFieldIdentifier = [3, n0, _TTFI, 0, [_N], [0]];
export var TaskTemplateInfoV2 = [3, n0, _TTIV, 0, [_A, _N], [0, 0]];
export var TaskTemplateMetadata = [
    3,
    n0,
    _TTM,
    0,
    [_I, _A, _N, _D, _St, _LMT, _CTre],
    [0, 0, 0, 0, 0, 4, 4],
];
export var TelephonyConfig = [3, n0, _TCe, 0, [_Di], [() => DistributionList]];
export var TemplateAttributes = [3, n0, _TAe, 0, [_CAu, _CPA], [128 | 0, 0]];
export var TemplatedMessageConfig = [
    3,
    n0,
    _TMC,
    0,
    [_KBI, _MTI, _TAe],
    [0, 0, () => TemplateAttributes],
];
export var Threshold = [3, n0, _Th, 0, [_Comp, _TVh], [0, 1]];
export var ThresholdV2 = [3, n0, _TVhr, 0, [_Comp, _TVh], [0, 1]];
export var ThrottlingException = [
    -3,
    n0,
    _TE,
    {
        [_e]: _c,
        [_hE]: 429,
        [_aQE]: [`ThrottlingException`, 429],
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(ThrottlingException, __ThrottlingException);
export var TooManyRequestsException = [
    -3,
    n0,
    _TMRE,
    {
        [_e]: _c,
        [_hE]: 429,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(TooManyRequestsException, __TooManyRequestsException);
export var TrafficDistributionGroup = [
    3,
    n0,
    _TDG,
    0,
    [_I, _A, _N, _D, _IAns, _St, _Ta, _ID],
    [0, 0, 0, 0, 0, 0, 128 | 0, 2],
];
export var TrafficDistributionGroupSummary = [
    3,
    n0,
    _TDGS,
    0,
    [_I, _A, _N, _IAns, _St, _ID],
    [0, 0, 0, 0, 0, 2],
];
export var TrafficDistributionGroupUserSummary = [3, n0, _TDGUS, 0, [_UI], [0]];
export var Transcript = [3, n0, _Tr, 0, [_Cri, _MTa], [[() => TranscriptCriteriaList, 0], 0]];
export var TranscriptCriteria = [
    3,
    n0,
    _TCr,
    0,
    [_PR, _ST, _MTa],
    [0, [() => SearchTextList, 0], 0],
];
export var TransferContactRequest = [
    3,
    n0,
    _TCRr,
    0,
    [_II, _CI, _QI, _UI, _CFI, _CT],
    [0, 0, 0, 0, 0, [0, 4]],
];
export var TransferContactResponse = [3, n0, _TCRra, 0, [_CI, _CA], [0, 0]];
export var UntagContactRequest = [
    3,
    n0,
    _UCR,
    0,
    [_CI, _II, _TKa],
    [
        [0, 1],
        [0, 1],
        [
            64 | 0,
            {
                [_hQ]: _TKa,
            },
        ],
    ],
];
export var UntagContactResponse = [3, n0, _UCRn, 0, [], []];
export var UntagResourceRequest = [
    3,
    n0,
    _URR,
    0,
    [_rA, _tKa],
    [
        [0, 1],
        [
            64 | 0,
            {
                [_hQ]: _tKa,
            },
        ],
    ],
];
export var UpdateAgentStatusRequest = [
    3,
    n0,
    _UASR,
    0,
    [_II, _ASI, _N, _D, _S, _DO, _RON],
    [[0, 1], [0, 1], 0, 0, 0, 1, 2],
];
export var UpdateAuthenticationProfileRequest = [
    3,
    n0,
    _UAPR,
    0,
    [_API, _II, _N, _D, _AIl, _BI, _PSD, _SID, _SIHE],
    [[0, 1], [0, 1], 0, 0, 64 | 0, 64 | 0, 1, 1, 2],
];
export var UpdateCaseActionDefinition = [3, n0, _UCAD, 0, [_Fi], [() => FieldValues]];
export var UpdateContactAttributesRequest = [
    3,
    n0,
    _UCAR,
    0,
    [_ICI, _II, _Attr],
    [0, 0, 128 | 0],
];
export var UpdateContactAttributesResponse = [3, n0, _UCARp, 0, [], []];
export var UpdateContactEvaluationRequest = [
    3,
    n0,
    _UCER,
    0,
    [_II, _EI, _An, _No, _UB],
    [[0, 1], [0, 1], () => EvaluationAnswersInputMap, () => EvaluationNotesMap, () => EvaluatorUserUnion],
];
export var UpdateContactEvaluationResponse = [3, n0, _UCERp, 0, [_EI, _EAv], [0, 0]];
export var UpdateContactFlowContentRequest = [
    3,
    n0,
    _UCFCR,
    0,
    [_II, _CFI, _Co],
    [[0, 1], [0, 1], 0],
];
export var UpdateContactFlowContentResponse = [3, n0, _UCFCRp, 0, [], []];
export var UpdateContactFlowMetadataRequest = [
    3,
    n0,
    _UCFMR,
    0,
    [_II, _CFI, _N, _D, _CFSo],
    [[0, 1], [0, 1], 0, 0, 0],
];
export var UpdateContactFlowMetadataResponse = [3, n0, _UCFMRp, 0, [], []];
export var UpdateContactFlowModuleAliasRequest = [
    3,
    n0,
    _UCFMAR,
    0,
    [_II, _CFMI, _AIli, _N, _D, _CFMV],
    [[0, 1], [0, 1], [0, 1], 0, 0, 1],
];
export var UpdateContactFlowModuleAliasResponse = [3, n0, _UCFMARp, 0, [], []];
export var UpdateContactFlowModuleContentRequest = [
    3,
    n0,
    _UCFMCR,
    0,
    [_II, _CFMI, _Co, _Se],
    [[0, 1], [0, 1], 0, 0],
];
export var UpdateContactFlowModuleContentResponse = [3, n0, _UCFMCRp, 0, [], []];
export var UpdateContactFlowModuleMetadataRequest = [
    3,
    n0,
    _UCFMMR,
    0,
    [_II, _CFMI, _N, _D, _S],
    [[0, 1], [0, 1], 0, 0, 0],
];
export var UpdateContactFlowModuleMetadataResponse = [3, n0, _UCFMMRp, 0, [], []];
export var UpdateContactFlowNameRequest = [
    3,
    n0,
    _UCFNR,
    0,
    [_II, _CFI, _N, _D],
    [[0, 1], [0, 1], 0, 0],
];
export var UpdateContactFlowNameResponse = [3, n0, _UCFNRp, 0, [], []];
export var UpdateContactRequest = [
    3,
    n0,
    _UCRp,
    0,
    [_II, _CI, _N, _D, _Ref, _SAe, _QIu, _UIs, _CEu, _SE],
    [
        [0, 1],
        [0, 1],
        [() => Name, 0],
        [() => Description, 0],
        () => ContactReferences,
        () => SegmentAttributes,
        () => QueueInfoInput,
        () => UserInfo,
        () => Endpoint,
        () => Endpoint,
    ],
];
export var UpdateContactResponse = [3, n0, _UCRpd, 0, [], []];
export var UpdateContactRoutingDataRequest = [
    3,
    n0,
    _UCRDR,
    0,
    [_II, _CI, _QTAS, _QP, _RC],
    [[0, 1], [0, 1], 1, 1, () => RoutingCriteriaInput],
];
export var UpdateContactRoutingDataResponse = [3, n0, _UCRDRp, 0, [], []];
export var UpdateContactScheduleRequest = [3, n0, _UCSR, 0, [_II, _CI, _STch], [0, 0, 4]];
export var UpdateContactScheduleResponse = [3, n0, _UCSRp, 0, [], []];
export var UpdateEmailAddressMetadataRequest = [
    3,
    n0,
    _UEAMR,
    0,
    [_II, _EAI, _D, _DN, _CT],
    [[0, 1], [0, 1], [() => Description, 0], [() => EmailAddressDisplayName, 0], 0],
];
export var UpdateEmailAddressMetadataResponse = [3, n0, _UEAMRp, 0, [_EAI, _EAA], [0, 0]];
export var UpdateEvaluationFormRequest = [
    3,
    n0,
    _UEFR,
    0,
    [_II, _EFI, _EFV, _CNV, _Ti, _D, _It, _SS, _AEC, _CT],
    [
        [0, 1],
        [0, 1],
        1,
        2,
        0,
        0,
        () => EvaluationFormItemsList,
        () => EvaluationFormScoringStrategy,
        () => EvaluationFormAutoEvaluationConfiguration,
        [0, 4],
    ],
];
export var UpdateEvaluationFormResponse = [3, n0, _UEFRp, 0, [_EFI, _EFA, _EFV], [0, 0, 1]];
export var UpdateHoursOfOperationOverrideRequest = [
    3,
    n0,
    _UHOOOR,
    0,
    [_II, _HOOI, _HOOOI, _N, _D, _Conf, _EF, _ETf],
    [[0, 1], [0, 1], [0, 1], 0, 0, () => HoursOfOperationOverrideConfigList, 0, 0],
];
export var UpdateHoursOfOperationRequest = [
    3,
    n0,
    _UHOOR,
    0,
    [_II, _HOOI, _N, _D, _TZ, _Conf],
    [[0, 1], [0, 1], 0, 0, 0, () => HoursOfOperationConfigList],
];
export var UpdateInstanceAttributeRequest = [
    3,
    n0,
    _UIAR,
    0,
    [_II, _ATt, _V, _CT],
    [[0, 1], [0, 1], 0, [0, 4]],
];
export var UpdateInstanceStorageConfigRequest = [
    3,
    n0,
    _UISCR,
    0,
    [_II, _AIs, _RT, _SCt, _CT],
    [
        [0, 1],
        [0, 1],
        [
            0,
            {
                [_hQ]: _rT,
            },
        ],
        () => InstanceStorageConfig,
        [0, 4],
    ],
];
export var UpdateParticipantAuthenticationRequest = [
    3,
    n0,
    _UPAR,
    0,
    [_S, _II, _Cod, _Err, _ED],
    [0, 0, [() => AuthorizationCode, 0], [() => AuthenticationError, 0], [() => AuthenticationErrorDescription, 0]],
];
export var UpdateParticipantAuthenticationResponse = [3, n0, _UPARp, 0, [], []];
export var UpdateParticipantRoleConfigRequest = [
    3,
    n0,
    _UPRCR,
    0,
    [_II, _CI, _CCh],
    [[0, 1], [0, 1], () => UpdateParticipantRoleConfigChannelInfo],
];
export var UpdateParticipantRoleConfigResponse = [3, n0, _UPRCRp, 0, [], []];
export var UpdatePhoneNumberMetadataRequest = [
    3,
    n0,
    _UPNMR,
    0,
    [_PNI, _PND, _CT],
    [[0, 1], 0, [0, 4]],
];
export var UpdatePhoneNumberRequest = [
    3,
    n0,
    _UPNR,
    0,
    [_PNI, _TA, _II, _CT],
    [[0, 1], 0, 0, [0, 4]],
];
export var UpdatePhoneNumberResponse = [3, n0, _UPNRp, 0, [_PNI, _PNA], [0, 0]];
export var UpdatePredefinedAttributeRequest = [
    3,
    n0,
    _UPARpd,
    0,
    [_II, _N, _Va, _Pu, _ACtt],
    [[0, 1], [0, 1], () => PredefinedAttributeValues, 64 | 0, () => InputPredefinedAttributeConfiguration],
];
export var UpdatePromptRequest = [
    3,
    n0,
    _UPR,
    0,
    [_II, _PIr, _N, _D, _SU],
    [[0, 1], [0, 1], 0, 0, 0],
];
export var UpdatePromptResponse = [3, n0, _UPRp, 0, [_PARN, _PIr], [0, 0]];
export var UpdateQueueHoursOfOperationRequest = [
    3,
    n0,
    _UQHOOR,
    0,
    [_II, _QI, _HOOI],
    [[0, 1], [0, 1], 0],
];
export var UpdateQueueMaxContactsRequest = [
    3,
    n0,
    _UQMCR,
    0,
    [_II, _QI, _MCa],
    [[0, 1], [0, 1], 1],
];
export var UpdateQueueNameRequest = [
    3,
    n0,
    _UQNR,
    0,
    [_II, _QI, _N, _D],
    [[0, 1], [0, 1], 0, 0],
];
export var UpdateQueueOutboundCallerConfigRequest = [
    3,
    n0,
    _UQOCCR,
    0,
    [_II, _QI, _OCC],
    [[0, 1], [0, 1], () => OutboundCallerConfig],
];
export var UpdateQueueOutboundEmailConfigRequest = [
    3,
    n0,
    _UQOECR,
    0,
    [_II, _QI, _OEC],
    [[0, 1], [0, 1], () => OutboundEmailConfig],
];
export var UpdateQueueStatusRequest = [3, n0, _UQSR, 0, [_II, _QI, _St], [[0, 1], [0, 1], 0]];
export var UpdateQuickConnectConfigRequest = [
    3,
    n0,
    _UQCCR,
    0,
    [_II, _QCIu, _QCC],
    [[0, 1], [0, 1], () => QuickConnectConfig],
];
export var UpdateQuickConnectNameRequest = [
    3,
    n0,
    _UQCNR,
    0,
    [_II, _QCIu, _N, _D],
    [[0, 1], [0, 1], 0, 0],
];
export var UpdateRoutingProfileAgentAvailabilityTimerRequest = [
    3,
    n0,
    _URPAATR,
    0,
    [_II, _RPI, _AAT],
    [[0, 1], [0, 1], 0],
];
export var UpdateRoutingProfileConcurrencyRequest = [
    3,
    n0,
    _URPCR,
    0,
    [_II, _RPI, _MCe],
    [[0, 1], [0, 1], () => MediaConcurrencies],
];
export var UpdateRoutingProfileDefaultOutboundQueueRequest = [
    3,
    n0,
    _URPDOQR,
    0,
    [_II, _RPI, _DOQI],
    [[0, 1], [0, 1], 0],
];
export var UpdateRoutingProfileNameRequest = [
    3,
    n0,
    _URPNR,
    0,
    [_II, _RPI, _N, _D],
    [[0, 1], [0, 1], 0, 0],
];
export var UpdateRoutingProfileQueuesRequest = [
    3,
    n0,
    _URPQR,
    0,
    [_II, _RPI, _QC],
    [[0, 1], [0, 1], () => RoutingProfileQueueConfigList],
];
export var UpdateRuleRequest = [
    3,
    n0,
    _URRp,
    0,
    [_RIu, _II, _N, _Fu, _Ac, _PS],
    [[0, 1], [0, 1], 0, 0, () => RuleActions, 0],
];
export var UpdateSecurityProfileRequest = [
    3,
    n0,
    _USPR,
    0,
    [_D, _Pe, _SPI, _II, _AACT, _TRR, _App, _HRR, _AACHGI],
    [0, 64 | 0, [0, 1], [0, 1], 128 | 0, 64 | 0, () => Applications, 64 | 0, 0],
];
export var UpdateTaskTemplateRequest = [
    3,
    n0,
    _UTTR,
    0,
    [_TTIa, _II, _N, _D, _CFI, _SAFI, _Cons, _Def, _St, _Fi],
    [[0, 1], [0, 1], 0, 0, 0, 0, () => TaskTemplateConstraints, () => TaskTemplateDefaults, 0, () => TaskTemplateFields],
];
export var UpdateTaskTemplateResponse = [
    3,
    n0,
    _UTTRp,
    0,
    [_II, _I, _A, _N, _D, _CFI, _SAFI, _Cons, _Def, _Fi, _St, _LMT, _CTre],
    [0, 0, 0, 0, 0, 0, 0, () => TaskTemplateConstraints, () => TaskTemplateDefaults, () => TaskTemplateFields, 0, 4, 4],
];
export var UpdateTrafficDistributionRequest = [
    3,
    n0,
    _UTDR,
    0,
    [_I, _TCe, _SIC, _ACg],
    [[0, 1], () => TelephonyConfig, () => SignInConfig, () => AgentConfig],
];
export var UpdateTrafficDistributionResponse = [3, n0, _UTDRp, 0, [], []];
export var UpdateUserHierarchyGroupNameRequest = [
    3,
    n0,
    _UUHGNR,
    0,
    [_N, _HGI, _II],
    [0, [0, 1], [0, 1]],
];
export var UpdateUserHierarchyRequest = [3, n0, _UUHR, 0, [_HGI, _UI, _II], [0, [0, 1], [0, 1]]];
export var UpdateUserHierarchyStructureRequest = [
    3,
    n0,
    _UUHSR,
    0,
    [_HS, _II],
    [() => HierarchyStructureUpdate, [0, 1]],
];
export var UpdateUserIdentityInfoRequest = [
    3,
    n0,
    _UUIIR,
    0,
    [_IId, _UI, _II],
    [
        [() => UserIdentityInfo, 0],
        [0, 1],
        [0, 1],
    ],
];
export var UpdateUserPhoneConfigRequest = [
    3,
    n0,
    _UUPCR,
    0,
    [_PCh, _UI, _II],
    [() => UserPhoneConfig, [0, 1], [0, 1]],
];
export var UpdateUserProficienciesRequest = [
    3,
    n0,
    _UUPR,
    0,
    [_II, _UI, _UP],
    [[0, 1], [0, 1], () => UserProficiencyList],
];
export var UpdateUserRoutingProfileRequest = [
    3,
    n0,
    _UURPR,
    0,
    [_RPI, _UI, _II],
    [0, [0, 1], [0, 1]],
];
export var UpdateUserSecurityProfilesRequest = [
    3,
    n0,
    _UUSPR,
    0,
    [_SPIe, _UI, _II],
    [64 | 0, [0, 1], [0, 1]],
];
export var UpdateViewContentRequest = [
    3,
    n0,
    _UVCR,
    0,
    [_II, _VIi, _St, _Co],
    [[0, 1], [0, 1], 0, [() => ViewInputContent, 0]],
];
export var UpdateViewContentResponse = [3, n0, _UVCRp, 0, [_Vi], [[() => View, 0]]];
export var UpdateViewMetadataRequest = [
    3,
    n0,
    _UVMR,
    0,
    [_II, _VIi, _N, _D],
    [[0, 1], [0, 1], [() => ViewName, 0], 0],
];
export var UpdateViewMetadataResponse = [3, n0, _UVMRp, 0, [], []];
export var UploadUrlMetadata = [3, n0, _UUM, 0, [_Ur, _UE, _HTI], [0, 0, 128 | 0]];
export var UrlReference = [3, n0, _UR, 0, [_N, _V], [0, 0]];
export var UseCase = [3, n0, _UCs, 0, [_UCI, _UCA, _UCT], [0, 0, 0]];
export var User = [
    3,
    n0,
    _Us,
    0,
    [_I, _A, _U, _IId, _PCh, _DUI, _SPIe, _RPI, _HGI, _Ta, _LMT, _LMR],
    [0, 0, 0, [() => UserIdentityInfo, 0], () => UserPhoneConfig, 0, 64 | 0, 0, 0, 128 | 0, 4, 0],
];
export var UserData = [
    3,
    n0,
    _UD,
    0,
    [_Us, _RP, _HP, _St, _ASBC, _MSBC, _ASBCc, _Conta, _NS],
    [
        () => UserReference,
        () => RoutingProfileReference,
        () => HierarchyPathReference,
        () => AgentStatusReference,
        128 | 1,
        128 | 1,
        128 | 1,
        () => AgentContactReferenceList,
        0,
    ],
];
export var UserDataFilters = [
    3,
    n0,
    _UDF,
    0,
    [_Qu, _CF, _RPo, _Age, _UHG],
    [64 | 0, () => ContactFilter, 64 | 0, 64 | 0, 64 | 0],
];
export var UserHierarchyGroupSearchCriteria = [
    3,
    n0,
    _UHGSC,
    0,
    [_OC, _ACn, _SC],
    [() => UserHierarchyGroupSearchConditionList, () => UserHierarchyGroupSearchConditionList, () => StringCondition],
];
export var UserHierarchyGroupSearchFilter = [
    3,
    n0,
    _UHGSF,
    0,
    [_AFt],
    [() => ControlPlaneAttributeFilter],
];
export var UserIdentityInfo = [
    3,
    n0,
    _UII,
    0,
    [_FNir, _LN, _E, _SEe, _Mob],
    [[() => AgentFirstName, 0], [() => AgentLastName, 0], [() => Email, 0], [() => Email, 0], 0],
];
export var UserIdentityInfoLite = [
    3,
    n0,
    _UIIL,
    0,
    [_FNir, _LN],
    [
        [() => AgentFirstName, 0],
        [() => AgentLastName, 0],
    ],
];
export var UserInfo = [3, n0, _UIs, 0, [_UI], [0]];
export var UserNotFoundException = [
    -3,
    n0,
    _UNFE,
    {
        [_e]: _c,
        [_hE]: 404,
    },
    [_M],
    [0],
];
TypeRegistry.for(n0).registerError(UserNotFoundException, __UserNotFoundException);
export var UserPhoneConfig = [
    3,
    n0,
    _UPC,
    0,
    [_PTh, _AAu, _ACWTL, _DPNe, _PCer],
    [0, 2, 1, 0, 2],
];
export var UserProficiency = [3, n0, _UPs, 0, [_ANtt, _AVt, _Level__], [0, 0, 1]];
export var UserProficiencyDisassociate = [3, n0, _UPD, 0, [_ANtt, _AVt], [0, 0]];
export var UserQuickConnectConfig = [3, n0, _UQCC, 0, [_UI, _CFI], [0, 0]];
export var UserReference = [3, n0, _URs, 0, [_I, _A], [0, 0]];
export var UserSearchCriteria = [
    3,
    n0,
    _USC,
    0,
    [_OC, _ACn, _SC, _LCi, _HGC],
    [
        () => UserSearchConditionList,
        () => UserSearchConditionList,
        () => StringCondition,
        () => ListCondition,
        () => HierarchyGroupCondition,
    ],
];
export var UserSearchFilter = [
    3,
    n0,
    _USF,
    0,
    [_TF, _UAF],
    [() => ControlPlaneTagFilter, () => ControlPlaneUserAttributeFilter],
];
export var UserSearchSummary = [
    3,
    n0,
    _USS,
    0,
    [_A, _DUI, _HGI, _I, _IId, _PCh, _RPI, _SPIe, _Ta, _U],
    [0, 0, 0, 0, [() => UserIdentityInfoLite, 0], () => UserPhoneConfig, 0, 64 | 0, 128 | 0, 0],
];
export var UserSummary = [3, n0, _US, 0, [_I, _A, _U, _LMT, _LMR], [0, 0, 0, 4, 0]];
export var View = [
    3,
    n0,
    _Vi,
    0,
    [_I, _A, _N, _St, _T, _D, _Ve, _VD, _Co, _Ta, _CTre, _LMT, _VCS],
    [0, 0, [() => ViewName, 0], 0, 0, 0, 1, 0, [() => ViewContent, 0], 128 | 0, 4, 4, 0],
];
export var ViewContent = [
    3,
    n0,
    _VC,
    0,
    [_ISnp, _Tem, _Ac],
    [[() => ViewInputSchema, 0], 0, [() => ViewActions, 0]],
];
export var ViewInputContent = [3, n0, _VIC, 0, [_Tem, _Ac], [0, [() => ViewActions, 0]]];
export var ViewSummary = [
    3,
    n0,
    _VSi,
    0,
    [_I, _A, _N, _T, _St, _D],
    [0, 0, [() => ViewName, 0], 0, 0, 0],
];
export var ViewVersionSummary = [
    3,
    n0,
    _VVS,
    0,
    [_I, _A, _D, _N, _T, _Ve, _VD],
    [0, 0, 0, [() => ViewName, 0], 0, 1, 0],
];
export var Vocabulary = [
    3,
    n0,
    _Vo,
    0,
    [_N, _I, _A, _LC, _S, _LMT, _FRa, _Co, _Ta],
    [0, 0, 0, 0, 0, 4, 0, 0, 128 | 0],
];
export var VocabularySummary = [
    3,
    n0,
    _VSo,
    0,
    [_N, _I, _A, _LC, _S, _LMT, _FRa],
    [0, 0, 0, 0, 0, 4, 0],
];
export var VoiceRecordingConfiguration = [3, n0, _VRC, 0, [_VRT, _IRT], [0, 0]];
export var WisdomInfo = [3, n0, _WI, 0, [_SAes], [0]];
export var __Unit = "unit";
export var ConnectServiceException = [-3, _sm, "ConnectServiceException", 0, [], []];
TypeRegistry.for(_sm).registerError(ConnectServiceException, __ConnectServiceException);
export var ActionSummaries = [1, n0, _ASc, 0, () => ActionSummary];
export var AgentContactReferenceList = [1, n0, _ACRL, 0, () => AgentContactReference];
export var AgentIds = 64 | 0;
export var AgentResourceIdList = 64 | 0;
export var AgentsMinOneMaxHundred = 64 | 0;
export var AgentStatuses = 64 | 0;
export var AgentStatusList = [1, n0, _ASL, 0, () => AgentStatus];
export var AgentStatusSearchConditionList = [1, n0, _ASSCL, 0, () => AgentStatusSearchCriteria];
export var AgentStatusSummaryList = [1, n0, _ASSL, 0, () => AgentStatusSummary];
export var AgentStatusTypes = 64 | 0;
export var AliasConfigurationList = [1, n0, _ACL, 0, () => AliasConfiguration];
export var AllowedMonitorCapabilities = 64 | 0;
export var AllowedUserActions = 64 | 0;
export var AnalyticsDataAssociationResults = [1, n0, _ADARn, 0, () => AnalyticsDataAssociationResult];
export var AnalyticsDataSetsResults = [1, n0, _ADSRn, 0, () => AnalyticsDataSetsResult];
export var ApplicationPermissions = 64 | 0;
export var Applications = [1, n0, _App, 0, () => Application];
export var AssociatedContactSummaryList = [1, n0, _ACSL, 0, () => AssociatedContactSummary];
export var AssociatedQueueIdList = 64 | 0;
export var AttachedFileErrorsList = [1, n0, _AFEL, 0, () => AttachedFileError];
export var AttachedFilesList = [1, n0, _AFL, 0, () => AttachedFile];
export var AttributeOrConditionList = [1, n0, _AOCL, 0, () => AttributeAndCondition];
export var AttributesList = [1, n0, _AL, 0, () => Attribute];
export var AuthenticationProfileSummaryList = [1, n0, _APSL, 0, () => AuthenticationProfileSummary];
export var AvailableNumbersList = [1, n0, _ANL, 0, () => AvailableNumberSummary];
export var ChannelList = 64 | 0;
export var Channels = 64 | 0;
export var CommonAttributeOrConditionList = [1, n0, _CAOCL, 0, () => CommonAttributeAndCondition];
export var Conditions = [1, n0, _Cond, 0, () => Condition];
export var ContactDataRequestList = [1, n0, _CDRL, 0, () => ContactDataRequest];
export var ContactFlowModuleAliasSummaryList = [
    1,
    n0,
    _CFMASL,
    0,
    () => ContactFlowModuleAliasSummary,
];
export var ContactFlowModuleSearchConditionList = [
    1,
    n0,
    _CFMSCL,
    0,
    () => ContactFlowModuleSearchCriteria,
];
export var ContactFlowModuleSearchSummaryList = [1, n0, _CFMSSL, 0, () => ContactFlowModule];
export var ContactFlowModulesSummaryList = [1, n0, _CFMSL, 0, () => ContactFlowModuleSummary];
export var ContactFlowModuleVersionSummaryList = [
    1,
    n0,
    _CFMVSL,
    0,
    () => ContactFlowModuleVersionSummary,
];
export var ContactFlowSearchConditionList = [1, n0, _CFSCL, 0, () => ContactFlowSearchCriteria];
export var ContactFlowSearchSummaryList = [1, n0, _CFSSL, 0, () => ContactFlow];
export var ContactFlowSummaryList = [1, n0, _CFSL, 0, () => ContactFlowSummary];
export var ContactFlowTypes = 64 | 0;
export var ContactFlowVersionSummaryList = [1, n0, _CFVSL, 0, () => ContactFlowVersionSummary];
export var ContactMetricResults = [1, n0, _CMRo, 0, () => ContactMetricResult];
export var ContactMetrics = [1, n0, _CMo, 0, () => ContactMetricInfo];
export var Contacts = [1, n0, _Conta, 0, [() => ContactSearchSummary, 0]];
export var ContactStates = 64 | 0;
export var ContactTagKeys = 64 | 0;
export var CurrentMetricDataCollections = [1, n0, _CMDC, 0, () => CurrentMetricData];
export var CurrentMetricResults = [1, n0, _CMRur, 0, () => CurrentMetricResult];
export var CurrentMetrics = [1, n0, _CMurr, 0, () => CurrentMetric];
export var CurrentMetricSortCriteriaMaxOne = [1, n0, _CMSCMO, 0, () => CurrentMetricSortCriteria];
export var DataSetIds = 64 | 0;
export var DefaultVocabularyList = [1, n0, _DVL, 0, () => DefaultVocabulary];
export var DistributionList = [1, n0, _DL, 0, () => Distribution];
export var EffectiveHoursOfOperationList = [1, n0, _EHOOL, 0, () => EffectiveHoursOfOperations];
export var EmailAddressList = [1, n0, _EAL, 0, [() => EmailAddressMetadata, 0]];
export var EmailAddressRecipientList = [1, n0, _EARL, 0, [() => EmailAddressInfo, 0]];
export var EmailAddressSearchConditionList = [1, n0, _EASCL, 0, () => EmailAddressSearchCriteria];
export var EmailAttachments = [1, n0, _EAmai, 8, () => EmailAttachment];
export var EmailRecipientsList = [1, n0, _ERL, 0, () => EmailRecipient];
export var ErrorResults = [1, n0, _ERrr, 0, () => ErrorResult];
export var EvaluationAutomationRuleCategoryList = [
    1,
    n0,
    _EARCL,
    0,
    () => EvaluationAutomationRuleCategory,
];
export var EvaluationFormItemEnablementConditionOperandList = [
    1,
    n0,
    _EFIECOL,
    0,
    () => EvaluationFormItemEnablementConditionOperand,
];
export var EvaluationFormItemEnablementSourceValueList = [
    1,
    n0,
    _EFIESVL,
    0,
    () => EvaluationFormItemEnablementSourceValue,
];
export var EvaluationFormItemsList = [1, n0, _EFIL, 0, () => EvaluationFormItem];
export var EvaluationFormNumericQuestionOptionList = [
    1,
    n0,
    _EFNQOL,
    0,
    () => EvaluationFormNumericQuestionOption,
];
export var EvaluationFormSearchConditionList = [1, n0, _EFSCL, 0, () => EvaluationFormSearchCriteria];
export var EvaluationFormSearchSummaryList = [1, n0, _EFSSL, 0, () => EvaluationFormSearchSummary];
export var EvaluationFormSingleSelectQuestionAutomationOptionList = [
    1,
    n0,
    _EFSSQAOL,
    0,
    () => EvaluationFormSingleSelectQuestionAutomationOption,
];
export var EvaluationFormSingleSelectQuestionOptionList = [
    1,
    n0,
    _EFSSQOL,
    0,
    () => EvaluationFormSingleSelectQuestionOption,
];
export var EvaluationFormSummaryList = [1, n0, _EFSL, 0, () => EvaluationFormSummary];
export var EvaluationFormVersionSummaryList = [1, n0, _EFVSL, 0, () => EvaluationFormVersionSummary];
export var EvaluationSearchConditionList = [1, n0, _ESCL, 0, () => EvaluationSearchCriteria];
export var EvaluationSearchSummaryList = [1, n0, _ESSL, 0, () => EvaluationSearchSummary];
export var EvaluationSuggestedAnswersList = [1, n0, _ESAL, 0, () => EvaluationSuggestedAnswer];
export var EvaluationSummaryList = [1, n0, _ESL, 0, () => EvaluationSummary];
export var EvaluationTranscriptPointsOfInterest = [
    1,
    n0,
    _ETPOIv,
    0,
    () => EvaluationTranscriptPointOfInterest,
];
export var Expressions = [1, n0, _Expr, 0, () => Expression];
export var FailedRequestList = [1, n0, _FRL, 0, () => FailedRequest];
export var FieldValues = [1, n0, _FVie, 0, () => FieldValue];
export var FileIdList = 64 | 0;
export var FiltersV2List = [1, n0, _FVL, 0, () => FilterV2];
export var FilterValueList = 64 | 0;
export var FlowAssociationSummaryList = [1, n0, _FASL, 0, () => FlowAssociationSummary];
export var FunctionArnsList = 64 | 0;
export var Groupings = 64 | 0;
export var GroupingsV2 = 64 | 0;
export var HierarchyGroupIdList = 64 | 0;
export var HierarchyGroupSummaryList = [1, n0, _HGSL, 0, () => HierarchyGroupSummary];
export var HierarchyRestrictedResourceList = 64 | 0;
export var HistoricalMetricDataCollections = [1, n0, _HMDC, 0, () => HistoricalMetricData];
export var HistoricalMetricResults = [1, n0, _HMRi, 0, () => HistoricalMetricResult];
export var HistoricalMetrics = [1, n0, _HM, 0, () => HistoricalMetric];
export var HoursOfOperationConfigList = [1, n0, _HOOCL, 0, () => HoursOfOperationConfig];
export var HoursOfOperationList = [1, n0, _HOOL, 0, () => HoursOfOperation];
export var HoursOfOperationOverrideConfigList = [
    1,
    n0,
    _HOOOCL,
    0,
    () => HoursOfOperationOverrideConfig,
];
export var HoursOfOperationOverrideList = [1, n0, _HOOOL, 0, () => HoursOfOperationOverride];
export var HoursOfOperationOverrideSearchConditionList = [
    1,
    n0,
    _HOOOSCL,
    0,
    () => HoursOfOperationOverrideSearchCriteria,
];
export var HoursOfOperationSearchConditionList = [
    1,
    n0,
    _HOOSCL,
    0,
    () => HoursOfOperationSearchCriteria,
];
export var HoursOfOperationSummaryList = [1, n0, _HOOSL, 0, () => HoursOfOperationSummary];
export var InitiationMethodList = 64 | 0;
export var InstanceStorageConfigs = [1, n0, _ISCn, 0, () => InstanceStorageConfig];
export var InstanceSummaryList = [1, n0, _ISL, 0, [() => InstanceSummary, 0]];
export var IntegrationAssociationSummaryList = [1, n0, _IASL, 0, () => IntegrationAssociationSummary];
export var InvisibleTaskTemplateFields = [1, n0, _ITTF, 0, () => InvisibleFieldInfo];
export var IpCidrList = 64 | 0;
export var LexBotConfigList = [1, n0, _LBCL, 0, () => LexBotConfig];
export var LexBotsList = [1, n0, _LBL, 0, () => LexBot];
export var ListPhoneNumbersSummaryList = [1, n0, _LPNSL, 0, () => ListPhoneNumbersSummary];
export var MediaConcurrencies = [1, n0, _MCe, 0, () => MediaConcurrency];
export var MetricDataCollectionsV2 = [1, n0, _MDCV, 0, () => MetricDataV2];
export var MetricFiltersV2List = [1, n0, _MFVL, 0, () => MetricFilterV2];
export var MetricFilterValueList = 64 | 0;
export var MetricResultsV2 = [1, n0, _MRVe, 0, () => MetricResultV2];
export var MetricsV2 = [1, n0, _MVet, 0, () => MetricV2];
export var OperationalHours = [1, n0, _OH, 0, () => OperationalHour];
export var OriginsList = 64 | 0;
export var ParticipantTimerConfigList = [1, n0, _PTCL, 0, () => ParticipantTimerConfiguration];
export var PermissionsList = 64 | 0;
export var PhoneNumberCountryCodes = 64 | 0;
export var PhoneNumberSummaryList = [1, n0, _PNSL, 0, () => PhoneNumberSummary];
export var PhoneNumberTypes = 64 | 0;
export var PotentialAudioQualityIssues = 64 | 0;
export var PredefinedAttributePurposeNameList = 64 | 0;
export var PredefinedAttributeSearchConditionList = [
    1,
    n0,
    _PASCL,
    0,
    () => PredefinedAttributeSearchCriteria,
];
export var PredefinedAttributeSearchSummaryList = [1, n0, _PASSL, 0, () => PredefinedAttribute];
export var PredefinedAttributeStringValuesList = 64 | 0;
export var PredefinedAttributeSummaryList = [1, n0, _PASL, 0, () => PredefinedAttributeSummary];
export var Problems = [1, n0, _Prob, 0, () => ProblemDetail];
export var PromptList = [1, n0, _PLro, 0, () => Prompt];
export var PromptSearchConditionList = [1, n0, _PSCL, 0, () => PromptSearchCriteria];
export var PromptSummaryList = [1, n0, _PSL, 0, () => PromptSummary];
export var PropertyValidationExceptionPropertyList = [
    1,
    n0,
    _PVEPL,
    0,
    () => PropertyValidationExceptionProperty,
];
export var QueueIdList = 64 | 0;
export var Queues = 64 | 0;
export var QueueSearchConditionList = [1, n0, _QSCL, 0, () => QueueSearchCriteria];
export var QueueSearchSummaryList = [1, n0, _QSSL, 0, () => Queue];
export var QueueSummaryList = [1, n0, _QSL, 0, () => QueueSummary];
export var QueueTypes = 64 | 0;
export var QuickConnectSearchConditionList = [1, n0, _QCSCL, 0, () => QuickConnectSearchCriteria];
export var QuickConnectSearchSummaryList = [1, n0, _QCSSL, 0, () => QuickConnect];
export var QuickConnectsList = 64 | 0;
export var QuickConnectSummaryList = [1, n0, _QCSL, 0, () => QuickConnectSummary];
export var QuickConnectTypes = 64 | 0;
export var ReadOnlyTaskTemplateFields = [1, n0, _ROTTF, 0, () => ReadOnlyFieldInfo];
export var RealTimeContactAnalysisAttachments = [
    1,
    n0,
    _RTCAAe,
    0,
    () => RealTimeContactAnalysisAttachment,
];
export var RealTimeContactAnalysisCharacterIntervals = [
    1,
    n0,
    _RTCACIe,
    0,
    () => RealTimeContactAnalysisCharacterInterval,
];
export var RealTimeContactAnalysisIssuesDetected = [
    1,
    n0,
    _RTCAIDe,
    0,
    () => RealTimeContactAnalysisIssueDetected,
];
export var RealTimeContactAnalysisPointsOfInterest = [
    1,
    n0,
    _RTCAPOIe,
    0,
    () => RealTimeContactAnalysisPointOfInterest,
];
export var RealtimeContactAnalysisSegments = [1, n0, _RCAS, 0, () => RealtimeContactAnalysisSegment];
export var RealTimeContactAnalysisSegmentTypes = 64 | 0;
export var RealTimeContactAnalysisTranscriptItemsWithCharacterOffsets = [
    1,
    n0,
    _RTCATIWCOe,
    0,
    () => RealTimeContactAnalysisTranscriptItemWithCharacterOffsets,
];
export var RealTimeContactAnalysisTranscriptItemsWithContent = [
    1,
    n0,
    _RTCATIWCe,
    0,
    () => RealTimeContactAnalysisTranscriptItemWithContent,
];
export var Recordings = [1, n0, _Re, 0, () => RecordingInfo];
export var ReferenceSummaryList = [1, n0, _RSL, 0, () => ReferenceSummary];
export var ReferenceTypes = 64 | 0;
export var ReplicationStatusSummaryList = [1, n0, _RSSL, 0, () => ReplicationStatusSummary];
export var RequiredTaskTemplateFields = [1, n0, _RTTF, 0, () => RequiredFieldInfo];
export var resourceArnListMaxLimit100 = 64 | 0;
export var ResourceTypeList = 64 | 0;
export var RoutingCriteriaInputSteps = [1, n0, _RCISo, 0, () => RoutingCriteriaInputStep];
export var RoutingExpressions = 64 | 0;
export var RoutingProfileList = [1, n0, _RPL, 0, () => RoutingProfile];
export var RoutingProfileManualAssignmentQueueConfigList = [
    1,
    n0,
    _RPMAQCL,
    0,
    () => RoutingProfileManualAssignmentQueueConfig,
];
export var RoutingProfileManualAssignmentQueueConfigSummaryList = [
    1,
    n0,
    _RPMAQCSL,
    0,
    () => RoutingProfileManualAssignmentQueueConfigSummary,
];
export var RoutingProfileQueueConfigList = [1, n0, _RPQCL, 0, () => RoutingProfileQueueConfig];
export var RoutingProfileQueueConfigSummaryList = [
    1,
    n0,
    _RPQCSL,
    0,
    () => RoutingProfileQueueConfigSummary,
];
export var RoutingProfileQueueReferenceList = [1, n0, _RPQRL, 0, () => RoutingProfileQueueReference];
export var RoutingProfiles = 64 | 0;
export var RoutingProfileSearchConditionList = [1, n0, _RPSCL, 0, () => RoutingProfileSearchCriteria];
export var RoutingProfileSummaryList = [1, n0, _RPSL, 0, () => RoutingProfileSummary];
export var RuleActions = [1, n0, _RAul, 0, () => RuleAction];
export var RuleSummaryList = [1, n0, _RSLu, 0, () => RuleSummary];
export var SearchableContactAttributesCriteriaList = [
    1,
    n0,
    _SCACL,
    0,
    [() => SearchableContactAttributesCriteria, 0],
];
export var SearchableContactAttributeValueList = [
    1,
    n0,
    _SCAVL,
    0,
    [() => SearchableContactAttributeValue, 0],
];
export var SearchableRoutingCriteriaStepList = [
    1,
    n0,
    _SRCSL,
    0,
    () => SearchableRoutingCriteriaStep,
];
export var SearchableSegmentAttributesCriteriaList = [
    1,
    n0,
    _SSACL,
    0,
    [() => SearchableSegmentAttributesCriteria, 0],
];
export var SearchableSegmentAttributeValueList = [
    1,
    n0,
    _SSAVL,
    8,
    [() => SearchableSegmentAttributeValue, 0],
];
export var SearchContactsAdditionalTimeRangeCriteriaList = [
    1,
    n0,
    _SCATRCL,
    0,
    () => SearchContactsAdditionalTimeRangeCriteria,
];
export var SearchTextList = [1, n0, _STL, 0, [() => SearchText, 0]];
export var SecurityKeysList = [1, n0, _SKL, 0, () => SecurityKey];
export var SecurityProfileIds = 64 | 0;
export var SecurityProfileSearchConditionList = [
    1,
    n0,
    _SPSCL,
    0,
    () => SecurityProfileSearchCriteria,
];
export var SecurityProfilesSearchSummaryList = [1, n0, _SPSSL, 0, () => SecurityProfileSearchSummary];
export var SecurityProfileSummaryList = [1, n0, _SPSL, 0, () => SecurityProfileSummary];
export var SegmentAttributeValueList = [1, n0, _SAVL, 0, () => SegmentAttributeValue];
export var SignInDistributionList = [1, n0, _SIDL, 0, () => SignInDistribution];
export var SingleSelectOptions = 64 | 0;
export var SlaFieldValueUnionList = [1, n0, _SFVUL, 0, () => FieldValueUnion];
export var StateTransitions = [1, n0, _STt, 0, () => StateTransition];
export var Steps = [1, n0, _Ste, 0, () => Step];
export var SuccessfulRequestList = [1, n0, _SRL, 0, () => SuccessfulRequest];
export var SupportedMessagingContentTypes = 64 | 0;
export var TagAndConditionList = [1, n0, _TACL, 0, () => TagCondition];
export var TagKeyList = 64 | 0;
export var TagOrConditionList = [1, n0, _TOCL, 0, () => TagAndConditionList];
export var TagRestrictedResourceList = 64 | 0;
export var TagsList = [1, n0, _TLa, 0, () => TagSet];
export var TaskTemplateDefaultFieldValueList = [
    1,
    n0,
    _TTDFVL,
    0,
    () => TaskTemplateDefaultFieldValue,
];
export var TaskTemplateFields = [1, n0, _TTFa, 0, () => TaskTemplateField];
export var TaskTemplateList = [1, n0, _TTL, 0, () => TaskTemplateMetadata];
export var ThresholdCollections = [1, n0, _TCh, 0, () => ThresholdV2];
export var TrafficDistributionGroupSummaryList = [
    1,
    n0,
    _TDGSL,
    0,
    () => TrafficDistributionGroupSummary,
];
export var TrafficDistributionGroupUserSummaryList = [
    1,
    n0,
    _TDGUSL,
    0,
    () => TrafficDistributionGroupUserSummary,
];
export var TranscriptCriteriaList = [1, n0, _TCL, 0, [() => TranscriptCriteria, 0]];
export var UseCaseSummaryList = [1, n0, _UCSL, 0, () => UseCase];
export var UserDataHierarchyGroups = 64 | 0;
export var UserDataList = [1, n0, _UDL, 0, () => UserData];
export var UserHierarchyGroupList = [1, n0, _UHGL, 0, () => HierarchyGroup];
export var UserHierarchyGroupSearchConditionList = [
    1,
    n0,
    _UHGSCL,
    0,
    () => UserHierarchyGroupSearchCriteria,
];
export var UserIdList = 64 | 0;
export var UserProficiencyDisassociateList = [1, n0, _UPDL, 0, () => UserProficiencyDisassociate];
export var UserProficiencyList = [1, n0, _UPL, 0, () => UserProficiency];
export var UserSearchConditionList = [1, n0, _USCL, 0, () => UserSearchCriteria];
export var UserSearchSummaryList = [1, n0, _USSL, 0, [() => UserSearchSummary, 0]];
export var UserSummaryList = [1, n0, _USL, 0, () => UserSummary];
export var ViewActions = [1, n0, _VAi, 0, [() => ViewAction, 0]];
export var ViewsSummaryList = [1, n0, _VSL, 0, [() => ViewSummary, 0]];
export var ViewVersionSummaryList = [1, n0, _VVSL, 0, [() => ViewVersionSummary, 0]];
export var VocabularySummaryList = [1, n0, _VSLo, 0, () => VocabularySummary];
export var AllowedAccessControlTags = 128 | 0;
export var Attributes = 128 | 0;
export var ChannelToCountMap = 128 | 1;
export var ContactEvaluations = [2, n0, _CEon, 0, 0, () => ContactEvaluation];
export var ContactReferences = [2, n0, _CR, 0, 0, () => Reference];
export var ContactSearchSummarySegmentAttributes = [
    2,
    n0,
    _CSSSA,
    8,
    0,
    () => ContactSearchSummarySegmentAttributeValue,
];
export var ContactTagMap = 128 | 0;
export var DimensionsV2Map = 128 | 0;
export var EmailHeaders = 128 | 0;
export var EvaluationAnswersInputMap = [2, n0, _EAIM, 0, 0, () => EvaluationAnswerInput];
export var EvaluationAnswersOutputMap = [2, n0, _EAOM, 0, 0, () => EvaluationAnswerOutput];
export var EvaluationNotesMap = [2, n0, _ENM, 0, 0, () => EvaluationNote];
export var EvaluationScoresMap = [2, n0, _ESMv, 0, 0, () => EvaluationScore];
export var RealTimeContactAnalysisMatchedDetails = [
    2,
    n0,
    _RTCAMD,
    0,
    0,
    () => RealTimeContactAnalysisCategoryDetails,
];
export var SegmentAttributes = [2, n0, _SAe, 0, 0, () => SegmentAttributeValue];
export var SegmentAttributeValueMap = [2, n0, _SAVM, 0, 0, () => SegmentAttributeValue];
export var TagMap = 128 | 0;
export var UrlMetadataSignedHeaders = 128 | 0;
export var UserTagMap = 128 | 0;
export var ContactMetricValue = [3, n0, _CMV, 0, [_Nu], [1]];
export var CreatedByInfo = [3, n0, _CBI, 0, [_CUA, _AWSIA], [0, 0]];
export var EvaluationAnswerData = [3, n0, _EAD, 0, [_SV, _NV, _NA], [0, 1, 2]];
export var EvaluationFormItem = [
    3,
    n0,
    _EFIv,
    0,
    [_Sec, _Que],
    [() => EvaluationFormSection, () => EvaluationFormQuestion],
];
export var EvaluationFormItemEnablementConditionOperand = [
    3,
    n0,
    _EFIECO,
    0,
    [_Exp, _Con],
    [() => EvaluationFormItemEnablementExpression, () => EvaluationFormItemEnablementCondition],
];
export var EvaluationFormNumericQuestionAutomation = [
    3,
    n0,
    _EFNQA,
    0,
    [_PVr, _ASn],
    [() => NumericQuestionPropertyValueAutomation, () => EvaluationFormQuestionAutomationAnswerSource],
];
export var EvaluationFormQuestionTypeProperties = [
    3,
    n0,
    _EFQTP,
    0,
    [_Num, _SSi, _Te],
    [
        () => EvaluationFormNumericQuestionProperties,
        () => EvaluationFormSingleSelectQuestionProperties,
        () => EvaluationFormTextQuestionProperties,
    ],
];
export var EvaluationFormSingleSelectQuestionAutomationOption = [
    3,
    n0,
    _EFSSQAO,
    0,
    [_RCu],
    [() => SingleSelectQuestionRuleCategoryAutomation],
];
export var EvaluationQuestionAnswerAnalysisDetails = [
    3,
    n0,
    _EQAAD,
    0,
    [_GAI, _CLo],
    [() => EvaluationGenAIAnswerAnalysisDetails, () => EvaluationContactLensAnswerAnalysisDetails],
];
export var EvaluatorUserUnion = [3, n0, _EUU, 0, [_CUA], [0]];
export var InvalidRequestExceptionReason = [3, n0, _IRER, 0, [_AFIRER], [0]];
export var ParticipantTimerValue = [3, n0, _PTV, 0, [_PTA, _PTDIM], [0, 1]];
export var PredefinedAttributeValues = [3, n0, _PAV, 0, [_SL], [64 | 0]];
export var RealtimeContactAnalysisSegment = [
    3,
    n0,
    _RCASe,
    0,
    [_Tr, _Cate, _Is, _Eve, _Atta, _PCS],
    [
        () => RealTimeContactAnalysisSegmentTranscript,
        () => RealTimeContactAnalysisSegmentCategories,
        () => RealTimeContactAnalysisSegmentIssues,
        () => RealTimeContactAnalysisSegmentEvent,
        () => RealTimeContactAnalysisSegmentAttachments,
        () => RealTimeContactAnalysisSegmentPostContactSummary,
    ],
];
export var RealTimeContactAnalysisTimeData = [3, n0, _RTCATD, 0, [_ATb], [5]];
export var ReferenceSummary = [
    3,
    n0,
    _RSe,
    0,
    [_Ur, _Attac, _EMm, _Str, _Nu, _Da, _E],
    [
        () => UrlReference,
        () => AttachmentReference,
        () => EmailMessageReference,
        () => StringReference,
        () => NumberReference,
        () => DateReference,
        () => EmailReference,
    ],
];
export var ServiceQuotaExceededExceptionReason = [3, n0, _SQEER, 0, [_AFSQEER], [0]];
export var UpdateParticipantRoleConfigChannelInfo = [
    3,
    n0,
    _UPRCCI,
    0,
    [_Cha],
    [() => ChatParticipantRoleConfig],
];
export var ActivateEvaluationForm = [
    9,
    n0,
    _AEF,
    {
        [_h]: ["POST", "/evaluation-forms/{InstanceId}/{EvaluationFormId}/activate", 200],
    },
    () => ActivateEvaluationFormRequest,
    () => ActivateEvaluationFormResponse,
];
export var AssociateAnalyticsDataSet = [
    9,
    n0,
    _AADS,
    {
        [_h]: ["PUT", "/analytics-data/instance/{InstanceId}/association", 200],
    },
    () => AssociateAnalyticsDataSetRequest,
    () => AssociateAnalyticsDataSetResponse,
];
export var AssociateApprovedOrigin = [
    9,
    n0,
    _AAO,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/approved-origin", 200],
    },
    () => AssociateApprovedOriginRequest,
    () => __Unit,
];
export var AssociateBot = [
    9,
    n0,
    _ABs,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/bot", 200],
    },
    () => AssociateBotRequest,
    () => __Unit,
];
export var AssociateContactWithUser = [
    9,
    n0,
    _ACWU,
    {
        [_h]: ["POST", "/contacts/{InstanceId}/{ContactId}/associate-user", 200],
    },
    () => AssociateContactWithUserRequest,
    () => AssociateContactWithUserResponse,
];
export var AssociateDefaultVocabulary = [
    9,
    n0,
    _ADV,
    {
        [_h]: ["PUT", "/default-vocabulary/{InstanceId}/{LanguageCode}", 200],
    },
    () => AssociateDefaultVocabularyRequest,
    () => AssociateDefaultVocabularyResponse,
];
export var AssociateEmailAddressAlias = [
    9,
    n0,
    _AEAA,
    {
        [_h]: ["POST", "/email-addresses/{InstanceId}/{EmailAddressId}/associate-alias", 200],
    },
    () => AssociateEmailAddressAliasRequest,
    () => AssociateEmailAddressAliasResponse,
];
export var AssociateFlow = [
    9,
    n0,
    _AFs,
    {
        [_h]: ["PUT", "/flow-associations/{InstanceId}", 200],
    },
    () => AssociateFlowRequest,
    () => AssociateFlowResponse,
];
export var AssociateInstanceStorageConfig = [
    9,
    n0,
    _AISC,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/storage-config", 200],
    },
    () => AssociateInstanceStorageConfigRequest,
    () => AssociateInstanceStorageConfigResponse,
];
export var AssociateLambdaFunction = [
    9,
    n0,
    _ALF,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/lambda-function", 200],
    },
    () => AssociateLambdaFunctionRequest,
    () => __Unit,
];
export var AssociateLexBot = [
    9,
    n0,
    _ALB,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/lex-bot", 200],
    },
    () => AssociateLexBotRequest,
    () => __Unit,
];
export var AssociatePhoneNumberContactFlow = [
    9,
    n0,
    _APNCF,
    {
        [_h]: ["PUT", "/phone-number/{PhoneNumberId}/contact-flow", 200],
    },
    () => AssociatePhoneNumberContactFlowRequest,
    () => __Unit,
];
export var AssociateQueueQuickConnects = [
    9,
    n0,
    _AQQC,
    {
        [_h]: ["POST", "/queues/{InstanceId}/{QueueId}/associate-quick-connects", 200],
    },
    () => AssociateQueueQuickConnectsRequest,
    () => __Unit,
];
export var AssociateRoutingProfileQueues = [
    9,
    n0,
    _ARPQ,
    {
        [_h]: ["POST", "/routing-profiles/{InstanceId}/{RoutingProfileId}/associate-queues", 200],
    },
    () => AssociateRoutingProfileQueuesRequest,
    () => __Unit,
];
export var AssociateSecurityKey = [
    9,
    n0,
    _ASK,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/security-key", 200],
    },
    () => AssociateSecurityKeyRequest,
    () => AssociateSecurityKeyResponse,
];
export var AssociateTrafficDistributionGroupUser = [
    9,
    n0,
    _ATDGU,
    {
        [_h]: ["PUT", "/traffic-distribution-group/{TrafficDistributionGroupId}/user", 200],
    },
    () => AssociateTrafficDistributionGroupUserRequest,
    () => AssociateTrafficDistributionGroupUserResponse,
];
export var AssociateUserProficiencies = [
    9,
    n0,
    _AUP,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/associate-proficiencies", 200],
    },
    () => AssociateUserProficienciesRequest,
    () => __Unit,
];
export var BatchAssociateAnalyticsDataSet = [
    9,
    n0,
    _BAADS,
    {
        [_h]: ["PUT", "/analytics-data/instance/{InstanceId}/associations", 200],
    },
    () => BatchAssociateAnalyticsDataSetRequest,
    () => BatchAssociateAnalyticsDataSetResponse,
];
export var BatchDisassociateAnalyticsDataSet = [
    9,
    n0,
    _BDADS,
    {
        [_h]: ["POST", "/analytics-data/instance/{InstanceId}/associations", 200],
    },
    () => BatchDisassociateAnalyticsDataSetRequest,
    () => BatchDisassociateAnalyticsDataSetResponse,
];
export var BatchGetAttachedFileMetadata = [
    9,
    n0,
    _BGAFM,
    {
        [_h]: ["POST", "/attached-files/{InstanceId}", 200],
    },
    () => BatchGetAttachedFileMetadataRequest,
    () => BatchGetAttachedFileMetadataResponse,
];
export var BatchGetFlowAssociation = [
    9,
    n0,
    _BGFA,
    {
        [_h]: ["POST", "/flow-associations-batch/{InstanceId}", 200],
    },
    () => BatchGetFlowAssociationRequest,
    () => BatchGetFlowAssociationResponse,
];
export var BatchPutContact = [
    9,
    n0,
    _BPC,
    {
        [_h]: ["PUT", "/contact/batch/{InstanceId}", 200],
    },
    () => BatchPutContactRequest,
    () => BatchPutContactResponse,
];
export var ClaimPhoneNumber = [
    9,
    n0,
    _CPN,
    {
        [_h]: ["POST", "/phone-number/claim", 200],
    },
    () => ClaimPhoneNumberRequest,
    () => ClaimPhoneNumberResponse,
];
export var CompleteAttachedFileUpload = [
    9,
    n0,
    _CAFU,
    {
        [_h]: ["POST", "/attached-files/{InstanceId}/{FileId}", 200],
    },
    () => CompleteAttachedFileUploadRequest,
    () => CompleteAttachedFileUploadResponse,
];
export var CreateAgentStatus = [
    9,
    n0,
    _CAS,
    {
        [_h]: ["PUT", "/agent-status/{InstanceId}", 200],
    },
    () => CreateAgentStatusRequest,
    () => CreateAgentStatusResponse,
];
export var CreateContact = [
    9,
    n0,
    _CCr,
    {
        [_h]: ["PUT", "/contact/create-contact", 200],
    },
    () => CreateContactRequest,
    () => CreateContactResponse,
];
export var CreateContactFlow = [
    9,
    n0,
    _CCF,
    {
        [_h]: ["PUT", "/contact-flows/{InstanceId}", 200],
    },
    () => CreateContactFlowRequest,
    () => CreateContactFlowResponse,
];
export var CreateContactFlowModule = [
    9,
    n0,
    _CCFM,
    {
        [_h]: ["PUT", "/contact-flow-modules/{InstanceId}", 200],
    },
    () => CreateContactFlowModuleRequest,
    () => CreateContactFlowModuleResponse,
];
export var CreateContactFlowModuleAlias = [
    9,
    n0,
    _CCFMA,
    {
        [_h]: ["PUT", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/alias", 200],
    },
    () => CreateContactFlowModuleAliasRequest,
    () => CreateContactFlowModuleAliasResponse,
];
export var CreateContactFlowModuleVersion = [
    9,
    n0,
    _CCFMV,
    {
        [_h]: ["PUT", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/version", 200],
    },
    () => CreateContactFlowModuleVersionRequest,
    () => CreateContactFlowModuleVersionResponse,
];
export var CreateContactFlowVersion = [
    9,
    n0,
    _CCFV,
    {
        [_h]: ["PUT", "/contact-flows/{InstanceId}/{ContactFlowId}/version", 200],
    },
    () => CreateContactFlowVersionRequest,
    () => CreateContactFlowVersionResponse,
];
export var CreateEmailAddress = [
    9,
    n0,
    _CEAr,
    {
        [_h]: ["PUT", "/email-addresses/{InstanceId}", 200],
    },
    () => CreateEmailAddressRequest,
    () => CreateEmailAddressResponse,
];
export var CreateEvaluationForm = [
    9,
    n0,
    _CEF,
    {
        [_h]: ["PUT", "/evaluation-forms/{InstanceId}", 200],
    },
    () => CreateEvaluationFormRequest,
    () => CreateEvaluationFormResponse,
];
export var CreateHoursOfOperation = [
    9,
    n0,
    _CHOO,
    {
        [_h]: ["PUT", "/hours-of-operations/{InstanceId}", 200],
    },
    () => CreateHoursOfOperationRequest,
    () => CreateHoursOfOperationResponse,
];
export var CreateHoursOfOperationOverride = [
    9,
    n0,
    _CHOOO,
    {
        [_h]: ["PUT", "/hours-of-operations/{InstanceId}/{HoursOfOperationId}/overrides", 200],
    },
    () => CreateHoursOfOperationOverrideRequest,
    () => CreateHoursOfOperationOverrideResponse,
];
export var CreateInstance = [
    9,
    n0,
    _CIr,
    {
        [_h]: ["PUT", "/instance", 200],
    },
    () => CreateInstanceRequest,
    () => CreateInstanceResponse,
];
export var CreateIntegrationAssociation = [
    9,
    n0,
    _CIA,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/integration-associations", 200],
    },
    () => CreateIntegrationAssociationRequest,
    () => CreateIntegrationAssociationResponse,
];
export var CreateParticipant = [
    9,
    n0,
    _CP,
    {
        [_h]: ["POST", "/contact/create-participant", 200],
    },
    () => CreateParticipantRequest,
    () => CreateParticipantResponse,
];
export var CreatePersistentContactAssociation = [
    9,
    n0,
    _CPCA,
    {
        [_h]: ["POST", "/contact/persistent-contact-association/{InstanceId}/{InitialContactId}", 200],
    },
    () => CreatePersistentContactAssociationRequest,
    () => CreatePersistentContactAssociationResponse,
];
export var CreatePredefinedAttribute = [
    9,
    n0,
    _CPAr,
    {
        [_h]: ["PUT", "/predefined-attributes/{InstanceId}", 200],
    },
    () => CreatePredefinedAttributeRequest,
    () => __Unit,
];
export var CreatePrompt = [
    9,
    n0,
    _CPr,
    {
        [_h]: ["PUT", "/prompts/{InstanceId}", 200],
    },
    () => CreatePromptRequest,
    () => CreatePromptResponse,
];
export var CreatePushNotificationRegistration = [
    9,
    n0,
    _CPNRr,
    {
        [_h]: ["PUT", "/push-notification/{InstanceId}/registrations", 200],
    },
    () => CreatePushNotificationRegistrationRequest,
    () => CreatePushNotificationRegistrationResponse,
];
export var CreateQueue = [
    9,
    n0,
    _CQ,
    {
        [_h]: ["PUT", "/queues/{InstanceId}", 200],
    },
    () => CreateQueueRequest,
    () => CreateQueueResponse,
];
export var CreateQuickConnect = [
    9,
    n0,
    _CQC,
    {
        [_h]: ["PUT", "/quick-connects/{InstanceId}", 200],
    },
    () => CreateQuickConnectRequest,
    () => CreateQuickConnectResponse,
];
export var CreateRoutingProfile = [
    9,
    n0,
    _CRP,
    {
        [_h]: ["PUT", "/routing-profiles/{InstanceId}", 200],
    },
    () => CreateRoutingProfileRequest,
    () => CreateRoutingProfileResponse,
];
export var CreateRule = [
    9,
    n0,
    _CRr,
    {
        [_h]: ["POST", "/rules/{InstanceId}", 200],
    },
    () => CreateRuleRequest,
    () => CreateRuleResponse,
];
export var CreateSecurityProfile = [
    9,
    n0,
    _CSP,
    {
        [_h]: ["PUT", "/security-profiles/{InstanceId}", 200],
    },
    () => CreateSecurityProfileRequest,
    () => CreateSecurityProfileResponse,
];
export var CreateTaskTemplate = [
    9,
    n0,
    _CTT,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/task/template", 200],
    },
    () => CreateTaskTemplateRequest,
    () => CreateTaskTemplateResponse,
];
export var CreateTrafficDistributionGroup = [
    9,
    n0,
    _CTDG,
    {
        [_h]: ["PUT", "/traffic-distribution-group", 200],
    },
    () => CreateTrafficDistributionGroupRequest,
    () => CreateTrafficDistributionGroupResponse,
];
export var CreateUseCase = [
    9,
    n0,
    _CUC,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/integration-associations/{IntegrationAssociationId}/use-cases", 200],
    },
    () => CreateUseCaseRequest,
    () => CreateUseCaseResponse,
];
export var CreateUser = [
    9,
    n0,
    _CU,
    {
        [_h]: ["PUT", "/users/{InstanceId}", 200],
    },
    () => CreateUserRequest,
    () => CreateUserResponse,
];
export var CreateUserHierarchyGroup = [
    9,
    n0,
    _CUHG,
    {
        [_h]: ["PUT", "/user-hierarchy-groups/{InstanceId}", 200],
    },
    () => CreateUserHierarchyGroupRequest,
    () => CreateUserHierarchyGroupResponse,
];
export var CreateView = [
    9,
    n0,
    _CV,
    {
        [_h]: ["PUT", "/views/{InstanceId}", 200],
    },
    () => CreateViewRequest,
    () => CreateViewResponse,
];
export var CreateViewVersion = [
    9,
    n0,
    _CVV,
    {
        [_h]: ["PUT", "/views/{InstanceId}/{ViewId}/versions", 200],
    },
    () => CreateViewVersionRequest,
    () => CreateViewVersionResponse,
];
export var CreateVocabulary = [
    9,
    n0,
    _CVr,
    {
        [_h]: ["POST", "/vocabulary/{InstanceId}", 200],
    },
    () => CreateVocabularyRequest,
    () => CreateVocabularyResponse,
];
export var DeactivateEvaluationForm = [
    9,
    n0,
    _DEF,
    {
        [_h]: ["POST", "/evaluation-forms/{InstanceId}/{EvaluationFormId}/deactivate", 200],
    },
    () => DeactivateEvaluationFormRequest,
    () => DeactivateEvaluationFormResponse,
];
export var DeleteAttachedFile = [
    9,
    n0,
    _DAF,
    {
        [_h]: ["DELETE", "/attached-files/{InstanceId}/{FileId}", 200],
    },
    () => DeleteAttachedFileRequest,
    () => DeleteAttachedFileResponse,
];
export var DeleteContactEvaluation = [
    9,
    n0,
    _DCE,
    {
        [_h]: ["DELETE", "/contact-evaluations/{InstanceId}/{EvaluationId}", 200],
    },
    () => DeleteContactEvaluationRequest,
    () => __Unit,
];
export var DeleteContactFlow = [
    9,
    n0,
    _DCF,
    {
        [_h]: ["DELETE", "/contact-flows/{InstanceId}/{ContactFlowId}", 200],
    },
    () => DeleteContactFlowRequest,
    () => DeleteContactFlowResponse,
];
export var DeleteContactFlowModule = [
    9,
    n0,
    _DCFM,
    {
        [_h]: ["DELETE", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}", 200],
    },
    () => DeleteContactFlowModuleRequest,
    () => DeleteContactFlowModuleResponse,
];
export var DeleteContactFlowModuleAlias = [
    9,
    n0,
    _DCFMA,
    {
        [_h]: ["DELETE", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/alias/{AliasId}", 200],
    },
    () => DeleteContactFlowModuleAliasRequest,
    () => DeleteContactFlowModuleAliasResponse,
];
export var DeleteContactFlowModuleVersion = [
    9,
    n0,
    _DCFMV,
    {
        [_h]: [
            "DELETE",
            "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/version/{ContactFlowModuleVersion}",
            200,
        ],
    },
    () => DeleteContactFlowModuleVersionRequest,
    () => DeleteContactFlowModuleVersionResponse,
];
export var DeleteContactFlowVersion = [
    9,
    n0,
    _DCFV,
    {
        [_h]: ["DELETE", "/contact-flows/{InstanceId}/{ContactFlowId}/version/{ContactFlowVersion}", 200],
    },
    () => DeleteContactFlowVersionRequest,
    () => DeleteContactFlowVersionResponse,
];
export var DeleteEmailAddress = [
    9,
    n0,
    _DEAe,
    {
        [_h]: ["DELETE", "/email-addresses/{InstanceId}/{EmailAddressId}", 200],
    },
    () => DeleteEmailAddressRequest,
    () => DeleteEmailAddressResponse,
];
export var DeleteEvaluationForm = [
    9,
    n0,
    _DEFe,
    {
        [_h]: ["DELETE", "/evaluation-forms/{InstanceId}/{EvaluationFormId}", 200],
    },
    () => DeleteEvaluationFormRequest,
    () => __Unit,
];
export var DeleteHoursOfOperation = [
    9,
    n0,
    _DHOO,
    {
        [_h]: ["DELETE", "/hours-of-operations/{InstanceId}/{HoursOfOperationId}", 200],
    },
    () => DeleteHoursOfOperationRequest,
    () => __Unit,
];
export var DeleteHoursOfOperationOverride = [
    9,
    n0,
    _DHOOO,
    {
        [_h]: [
            "DELETE",
            "/hours-of-operations/{InstanceId}/{HoursOfOperationId}/overrides/{HoursOfOperationOverrideId}",
            200,
        ],
    },
    () => DeleteHoursOfOperationOverrideRequest,
    () => __Unit,
];
export var DeleteInstance = [
    9,
    n0,
    _DIel,
    {
        [_h]: ["DELETE", "/instance/{InstanceId}", 200],
    },
    () => DeleteInstanceRequest,
    () => __Unit,
];
export var DeleteIntegrationAssociation = [
    9,
    n0,
    _DIA,
    {
        [_h]: ["DELETE", "/instance/{InstanceId}/integration-associations/{IntegrationAssociationId}", 200],
    },
    () => DeleteIntegrationAssociationRequest,
    () => __Unit,
];
export var DeletePredefinedAttribute = [
    9,
    n0,
    _DPA,
    {
        [_h]: ["DELETE", "/predefined-attributes/{InstanceId}/{Name}", 200],
    },
    () => DeletePredefinedAttributeRequest,
    () => __Unit,
];
export var DeletePrompt = [
    9,
    n0,
    _DP,
    {
        [_h]: ["DELETE", "/prompts/{InstanceId}/{PromptId}", 200],
    },
    () => DeletePromptRequest,
    () => __Unit,
];
export var DeletePushNotificationRegistration = [
    9,
    n0,
    _DPNRel,
    {
        [_h]: ["DELETE", "/push-notification/{InstanceId}/registrations/{RegistrationId}", 200],
    },
    () => DeletePushNotificationRegistrationRequest,
    () => DeletePushNotificationRegistrationResponse,
];
export var DeleteQueue = [
    9,
    n0,
    _DQ,
    {
        [_h]: ["DELETE", "/queues/{InstanceId}/{QueueId}", 200],
    },
    () => DeleteQueueRequest,
    () => __Unit,
];
export var DeleteQuickConnect = [
    9,
    n0,
    _DQC,
    {
        [_h]: ["DELETE", "/quick-connects/{InstanceId}/{QuickConnectId}", 200],
    },
    () => DeleteQuickConnectRequest,
    () => __Unit,
];
export var DeleteRoutingProfile = [
    9,
    n0,
    _DRP,
    {
        [_h]: ["DELETE", "/routing-profiles/{InstanceId}/{RoutingProfileId}", 200],
    },
    () => DeleteRoutingProfileRequest,
    () => __Unit,
];
export var DeleteRule = [
    9,
    n0,
    _DRel,
    {
        [_h]: ["DELETE", "/rules/{InstanceId}/{RuleId}", 200],
    },
    () => DeleteRuleRequest,
    () => __Unit,
];
export var DeleteSecurityProfile = [
    9,
    n0,
    _DSP,
    {
        [_h]: ["DELETE", "/security-profiles/{InstanceId}/{SecurityProfileId}", 200],
    },
    () => DeleteSecurityProfileRequest,
    () => __Unit,
];
export var DeleteTaskTemplate = [
    9,
    n0,
    _DTT,
    {
        [_h]: ["DELETE", "/instance/{InstanceId}/task/template/{TaskTemplateId}", 200],
    },
    () => DeleteTaskTemplateRequest,
    () => DeleteTaskTemplateResponse,
];
export var DeleteTrafficDistributionGroup = [
    9,
    n0,
    _DTDG,
    {
        [_h]: ["DELETE", "/traffic-distribution-group/{TrafficDistributionGroupId}", 200],
    },
    () => DeleteTrafficDistributionGroupRequest,
    () => DeleteTrafficDistributionGroupResponse,
];
export var DeleteUseCase = [
    9,
    n0,
    _DUC,
    {
        [_h]: [
            "DELETE",
            "/instance/{InstanceId}/integration-associations/{IntegrationAssociationId}/use-cases/{UseCaseId}",
            200,
        ],
    },
    () => DeleteUseCaseRequest,
    () => __Unit,
];
export var DeleteUser = [
    9,
    n0,
    _DU,
    {
        [_h]: ["DELETE", "/users/{InstanceId}/{UserId}", 200],
    },
    () => DeleteUserRequest,
    () => __Unit,
];
export var DeleteUserHierarchyGroup = [
    9,
    n0,
    _DUHG,
    {
        [_h]: ["DELETE", "/user-hierarchy-groups/{InstanceId}/{HierarchyGroupId}", 200],
    },
    () => DeleteUserHierarchyGroupRequest,
    () => __Unit,
];
export var DeleteView = [
    9,
    n0,
    _DVel,
    {
        [_h]: ["DELETE", "/views/{InstanceId}/{ViewId}", 200],
    },
    () => DeleteViewRequest,
    () => DeleteViewResponse,
];
export var DeleteViewVersion = [
    9,
    n0,
    _DVV,
    {
        [_h]: ["DELETE", "/views/{InstanceId}/{ViewId}/versions/{ViewVersion}", 200],
    },
    () => DeleteViewVersionRequest,
    () => DeleteViewVersionResponse,
];
export var DeleteVocabulary = [
    9,
    n0,
    _DVele,
    {
        [_h]: ["POST", "/vocabulary-remove/{InstanceId}/{VocabularyId}", 200],
    },
    () => DeleteVocabularyRequest,
    () => DeleteVocabularyResponse,
];
export var DescribeAgentStatus = [
    9,
    n0,
    _DAS,
    {
        [_h]: ["GET", "/agent-status/{InstanceId}/{AgentStatusId}", 200],
    },
    () => DescribeAgentStatusRequest,
    () => DescribeAgentStatusResponse,
];
export var DescribeAuthenticationProfile = [
    9,
    n0,
    _DAP,
    {
        [_h]: ["GET", "/authentication-profiles/{InstanceId}/{AuthenticationProfileId}", 200],
    },
    () => DescribeAuthenticationProfileRequest,
    () => DescribeAuthenticationProfileResponse,
];
export var DescribeContact = [
    9,
    n0,
    _DCes,
    {
        [_h]: ["GET", "/contacts/{InstanceId}/{ContactId}", 200],
    },
    () => DescribeContactRequest,
    () => DescribeContactResponse,
];
export var DescribeContactEvaluation = [
    9,
    n0,
    _DCEe,
    {
        [_h]: ["GET", "/contact-evaluations/{InstanceId}/{EvaluationId}", 200],
    },
    () => DescribeContactEvaluationRequest,
    () => DescribeContactEvaluationResponse,
];
export var DescribeContactFlow = [
    9,
    n0,
    _DCFe,
    {
        [_h]: ["GET", "/contact-flows/{InstanceId}/{ContactFlowId}", 200],
    },
    () => DescribeContactFlowRequest,
    () => DescribeContactFlowResponse,
];
export var DescribeContactFlowModule = [
    9,
    n0,
    _DCFMe,
    {
        [_h]: ["GET", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}", 200],
    },
    () => DescribeContactFlowModuleRequest,
    () => DescribeContactFlowModuleResponse,
];
export var DescribeContactFlowModuleAlias = [
    9,
    n0,
    _DCFMAe,
    {
        [_h]: ["GET", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/alias/{AliasId}", 200],
    },
    () => DescribeContactFlowModuleAliasRequest,
    () => DescribeContactFlowModuleAliasResponse,
];
export var DescribeEmailAddress = [
    9,
    n0,
    _DEAes,
    {
        [_h]: ["GET", "/email-addresses/{InstanceId}/{EmailAddressId}", 200],
    },
    () => DescribeEmailAddressRequest,
    () => DescribeEmailAddressResponse,
];
export var DescribeEvaluationForm = [
    9,
    n0,
    _DEFes,
    {
        [_h]: ["GET", "/evaluation-forms/{InstanceId}/{EvaluationFormId}", 200],
    },
    () => DescribeEvaluationFormRequest,
    () => DescribeEvaluationFormResponse,
];
export var DescribeHoursOfOperation = [
    9,
    n0,
    _DHOOe,
    {
        [_h]: ["GET", "/hours-of-operations/{InstanceId}/{HoursOfOperationId}", 200],
    },
    () => DescribeHoursOfOperationRequest,
    () => DescribeHoursOfOperationResponse,
];
export var DescribeHoursOfOperationOverride = [
    9,
    n0,
    _DHOOOe,
    {
        [_h]: ["GET", "/hours-of-operations/{InstanceId}/{HoursOfOperationId}/overrides/{HoursOfOperationOverrideId}", 200],
    },
    () => DescribeHoursOfOperationOverrideRequest,
    () => DescribeHoursOfOperationOverrideResponse,
];
export var DescribeInstance = [
    9,
    n0,
    _DIes,
    {
        [_h]: ["GET", "/instance/{InstanceId}", 200],
    },
    () => DescribeInstanceRequest,
    () => DescribeInstanceResponse,
];
export var DescribeInstanceAttribute = [
    9,
    n0,
    _DIAe,
    {
        [_h]: ["GET", "/instance/{InstanceId}/attribute/{AttributeType}", 200],
    },
    () => DescribeInstanceAttributeRequest,
    () => DescribeInstanceAttributeResponse,
];
export var DescribeInstanceStorageConfig = [
    9,
    n0,
    _DISC,
    {
        [_h]: ["GET", "/instance/{InstanceId}/storage-config/{AssociationId}", 200],
    },
    () => DescribeInstanceStorageConfigRequest,
    () => DescribeInstanceStorageConfigResponse,
];
export var DescribePhoneNumber = [
    9,
    n0,
    _DPNes,
    {
        [_h]: ["GET", "/phone-number/{PhoneNumberId}", 200],
    },
    () => DescribePhoneNumberRequest,
    () => DescribePhoneNumberResponse,
];
export var DescribePredefinedAttribute = [
    9,
    n0,
    _DPAe,
    {
        [_h]: ["GET", "/predefined-attributes/{InstanceId}/{Name}", 200],
    },
    () => DescribePredefinedAttributeRequest,
    () => DescribePredefinedAttributeResponse,
];
export var DescribePrompt = [
    9,
    n0,
    _DPe,
    {
        [_h]: ["GET", "/prompts/{InstanceId}/{PromptId}", 200],
    },
    () => DescribePromptRequest,
    () => DescribePromptResponse,
];
export var DescribeQueue = [
    9,
    n0,
    _DQe,
    {
        [_h]: ["GET", "/queues/{InstanceId}/{QueueId}", 200],
    },
    () => DescribeQueueRequest,
    () => DescribeQueueResponse,
];
export var DescribeQuickConnect = [
    9,
    n0,
    _DQCe,
    {
        [_h]: ["GET", "/quick-connects/{InstanceId}/{QuickConnectId}", 200],
    },
    () => DescribeQuickConnectRequest,
    () => DescribeQuickConnectResponse,
];
export var DescribeRoutingProfile = [
    9,
    n0,
    _DRPe,
    {
        [_h]: ["GET", "/routing-profiles/{InstanceId}/{RoutingProfileId}", 200],
    },
    () => DescribeRoutingProfileRequest,
    () => DescribeRoutingProfileResponse,
];
export var DescribeRule = [
    9,
    n0,
    _DRes,
    {
        [_h]: ["GET", "/rules/{InstanceId}/{RuleId}", 200],
    },
    () => DescribeRuleRequest,
    () => DescribeRuleResponse,
];
export var DescribeSecurityProfile = [
    9,
    n0,
    _DSPe,
    {
        [_h]: ["GET", "/security-profiles/{InstanceId}/{SecurityProfileId}", 200],
    },
    () => DescribeSecurityProfileRequest,
    () => DescribeSecurityProfileResponse,
];
export var DescribeTrafficDistributionGroup = [
    9,
    n0,
    _DTDGe,
    {
        [_h]: ["GET", "/traffic-distribution-group/{TrafficDistributionGroupId}", 200],
    },
    () => DescribeTrafficDistributionGroupRequest,
    () => DescribeTrafficDistributionGroupResponse,
];
export var DescribeUser = [
    9,
    n0,
    _DUe,
    {
        [_h]: ["GET", "/users/{InstanceId}/{UserId}", 200],
    },
    () => DescribeUserRequest,
    () => DescribeUserResponse,
];
export var DescribeUserHierarchyGroup = [
    9,
    n0,
    _DUHGe,
    {
        [_h]: ["GET", "/user-hierarchy-groups/{InstanceId}/{HierarchyGroupId}", 200],
    },
    () => DescribeUserHierarchyGroupRequest,
    () => DescribeUserHierarchyGroupResponse,
];
export var DescribeUserHierarchyStructure = [
    9,
    n0,
    _DUHS,
    {
        [_h]: ["GET", "/user-hierarchy-structure/{InstanceId}", 200],
    },
    () => DescribeUserHierarchyStructureRequest,
    () => DescribeUserHierarchyStructureResponse,
];
export var DescribeView = [
    9,
    n0,
    _DVes,
    {
        [_h]: ["GET", "/views/{InstanceId}/{ViewId}", 200],
    },
    () => DescribeViewRequest,
    () => DescribeViewResponse,
];
export var DescribeVocabulary = [
    9,
    n0,
    _DVesc,
    {
        [_h]: ["GET", "/vocabulary/{InstanceId}/{VocabularyId}", 200],
    },
    () => DescribeVocabularyRequest,
    () => DescribeVocabularyResponse,
];
export var DisassociateAnalyticsDataSet = [
    9,
    n0,
    _DADS,
    {
        [_h]: ["POST", "/analytics-data/instance/{InstanceId}/association", 200],
    },
    () => DisassociateAnalyticsDataSetRequest,
    () => __Unit,
];
export var DisassociateApprovedOrigin = [
    9,
    n0,
    _DAO,
    {
        [_h]: ["DELETE", "/instance/{InstanceId}/approved-origin", 200],
    },
    () => DisassociateApprovedOriginRequest,
    () => __Unit,
];
export var DisassociateBot = [
    9,
    n0,
    _DB,
    {
        [_h]: ["POST", "/instance/{InstanceId}/bot", 200],
    },
    () => DisassociateBotRequest,
    () => __Unit,
];
export var DisassociateEmailAddressAlias = [
    9,
    n0,
    _DEAA,
    {
        [_h]: ["POST", "/email-addresses/{InstanceId}/{EmailAddressId}/disassociate-alias", 200],
    },
    () => DisassociateEmailAddressAliasRequest,
    () => DisassociateEmailAddressAliasResponse,
];
export var DisassociateFlow = [
    9,
    n0,
    _DF,
    {
        [_h]: ["DELETE", "/flow-associations/{InstanceId}/{ResourceId}/{ResourceType}", 200],
    },
    () => DisassociateFlowRequest,
    () => DisassociateFlowResponse,
];
export var DisassociateInstanceStorageConfig = [
    9,
    n0,
    _DISCi,
    {
        [_h]: ["DELETE", "/instance/{InstanceId}/storage-config/{AssociationId}", 200],
    },
    () => DisassociateInstanceStorageConfigRequest,
    () => __Unit,
];
export var DisassociateLambdaFunction = [
    9,
    n0,
    _DLF,
    {
        [_h]: ["DELETE", "/instance/{InstanceId}/lambda-function", 200],
    },
    () => DisassociateLambdaFunctionRequest,
    () => __Unit,
];
export var DisassociateLexBot = [
    9,
    n0,
    _DLB,
    {
        [_h]: ["DELETE", "/instance/{InstanceId}/lex-bot", 200],
    },
    () => DisassociateLexBotRequest,
    () => __Unit,
];
export var DisassociatePhoneNumberContactFlow = [
    9,
    n0,
    _DPNCF,
    {
        [_h]: ["DELETE", "/phone-number/{PhoneNumberId}/contact-flow", 200],
    },
    () => DisassociatePhoneNumberContactFlowRequest,
    () => __Unit,
];
export var DisassociateQueueQuickConnects = [
    9,
    n0,
    _DQQC,
    {
        [_h]: ["POST", "/queues/{InstanceId}/{QueueId}/disassociate-quick-connects", 200],
    },
    () => DisassociateQueueQuickConnectsRequest,
    () => __Unit,
];
export var DisassociateRoutingProfileQueues = [
    9,
    n0,
    _DRPQ,
    {
        [_h]: ["POST", "/routing-profiles/{InstanceId}/{RoutingProfileId}/disassociate-queues", 200],
    },
    () => DisassociateRoutingProfileQueuesRequest,
    () => __Unit,
];
export var DisassociateSecurityKey = [
    9,
    n0,
    _DSK,
    {
        [_h]: ["DELETE", "/instance/{InstanceId}/security-key/{AssociationId}", 200],
    },
    () => DisassociateSecurityKeyRequest,
    () => __Unit,
];
export var DisassociateTrafficDistributionGroupUser = [
    9,
    n0,
    _DTDGU,
    {
        [_h]: ["DELETE", "/traffic-distribution-group/{TrafficDistributionGroupId}/user", 200],
    },
    () => DisassociateTrafficDistributionGroupUserRequest,
    () => DisassociateTrafficDistributionGroupUserResponse,
];
export var DisassociateUserProficiencies = [
    9,
    n0,
    _DUP,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/disassociate-proficiencies", 200],
    },
    () => DisassociateUserProficienciesRequest,
    () => __Unit,
];
export var DismissUserContact = [
    9,
    n0,
    _DUCi,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/contact", 200],
    },
    () => DismissUserContactRequest,
    () => DismissUserContactResponse,
];
export var GetAttachedFile = [
    9,
    n0,
    _GAF,
    {
        [_h]: ["GET", "/attached-files/{InstanceId}/{FileId}", 200],
    },
    () => GetAttachedFileRequest,
    () => GetAttachedFileResponse,
];
export var GetContactAttributes = [
    9,
    n0,
    _GCA,
    {
        [_h]: ["GET", "/contact/attributes/{InstanceId}/{InitialContactId}", 200],
    },
    () => GetContactAttributesRequest,
    () => GetContactAttributesResponse,
];
export var GetContactMetrics = [
    9,
    n0,
    _GCM,
    {
        [_h]: ["POST", "/metrics/contact", 200],
    },
    () => GetContactMetricsRequest,
    () => GetContactMetricsResponse,
];
export var GetCurrentMetricData = [
    9,
    n0,
    _GCMD,
    {
        [_h]: ["POST", "/metrics/current/{InstanceId}", 200],
    },
    () => GetCurrentMetricDataRequest,
    () => GetCurrentMetricDataResponse,
];
export var GetCurrentUserData = [
    9,
    n0,
    _GCUD,
    {
        [_h]: ["POST", "/metrics/userdata/{InstanceId}", 200],
    },
    () => GetCurrentUserDataRequest,
    () => GetCurrentUserDataResponse,
];
export var GetEffectiveHoursOfOperations = [
    9,
    n0,
    _GEHOO,
    {
        [_h]: ["GET", "/effective-hours-of-operations/{InstanceId}/{HoursOfOperationId}", 200],
    },
    () => GetEffectiveHoursOfOperationsRequest,
    () => GetEffectiveHoursOfOperationsResponse,
];
export var GetFederationToken = [
    9,
    n0,
    _GFT,
    {
        [_h]: ["GET", "/user/federate/{InstanceId}", 200],
    },
    () => GetFederationTokenRequest,
    () => GetFederationTokenResponse,
];
export var GetFlowAssociation = [
    9,
    n0,
    _GFA,
    {
        [_h]: ["GET", "/flow-associations/{InstanceId}/{ResourceId}/{ResourceType}", 200],
    },
    () => GetFlowAssociationRequest,
    () => GetFlowAssociationResponse,
];
export var GetMetricData = [
    9,
    n0,
    _GMD,
    {
        [_h]: ["POST", "/metrics/historical/{InstanceId}", 200],
    },
    () => GetMetricDataRequest,
    () => GetMetricDataResponse,
];
export var GetMetricDataV2 = [
    9,
    n0,
    _GMDV,
    {
        [_h]: ["POST", "/metrics/data", 200],
    },
    () => GetMetricDataV2Request,
    () => GetMetricDataV2Response,
];
export var GetPromptFile = [
    9,
    n0,
    _GPF,
    {
        [_h]: ["GET", "/prompts/{InstanceId}/{PromptId}/file", 200],
    },
    () => GetPromptFileRequest,
    () => GetPromptFileResponse,
];
export var GetTaskTemplate = [
    9,
    n0,
    _GTT,
    {
        [_h]: ["GET", "/instance/{InstanceId}/task/template/{TaskTemplateId}", 200],
    },
    () => GetTaskTemplateRequest,
    () => GetTaskTemplateResponse,
];
export var GetTrafficDistribution = [
    9,
    n0,
    _GTD,
    {
        [_h]: ["GET", "/traffic-distribution/{Id}", 200],
    },
    () => GetTrafficDistributionRequest,
    () => GetTrafficDistributionResponse,
];
export var ImportPhoneNumber = [
    9,
    n0,
    _IPN,
    {
        [_h]: ["POST", "/phone-number/import", 200],
    },
    () => ImportPhoneNumberRequest,
    () => ImportPhoneNumberResponse,
];
export var ListAgentStatuses = [
    9,
    n0,
    _LAS,
    {
        [_h]: ["GET", "/agent-status/{InstanceId}", 200],
    },
    () => ListAgentStatusRequest,
    () => ListAgentStatusResponse,
];
export var ListAnalyticsDataAssociations = [
    9,
    n0,
    _LADA,
    {
        [_h]: ["GET", "/analytics-data/instance/{InstanceId}/association", 200],
    },
    () => ListAnalyticsDataAssociationsRequest,
    () => ListAnalyticsDataAssociationsResponse,
];
export var ListAnalyticsDataLakeDataSets = [
    9,
    n0,
    _LADLDS,
    {
        [_h]: ["GET", "/analytics-data/instance/{InstanceId}/datasets", 200],
    },
    () => ListAnalyticsDataLakeDataSetsRequest,
    () => ListAnalyticsDataLakeDataSetsResponse,
];
export var ListApprovedOrigins = [
    9,
    n0,
    _LAO,
    {
        [_h]: ["GET", "/instance/{InstanceId}/approved-origins", 200],
    },
    () => ListApprovedOriginsRequest,
    () => ListApprovedOriginsResponse,
];
export var ListAssociatedContacts = [
    9,
    n0,
    _LAC,
    {
        [_h]: ["GET", "/contact/associated/{InstanceId}", 200],
    },
    () => ListAssociatedContactsRequest,
    () => ListAssociatedContactsResponse,
];
export var ListAuthenticationProfiles = [
    9,
    n0,
    _LAP,
    {
        [_h]: ["GET", "/authentication-profiles-summary/{InstanceId}", 200],
    },
    () => ListAuthenticationProfilesRequest,
    () => ListAuthenticationProfilesResponse,
];
export var ListBots = [
    9,
    n0,
    _LBi,
    {
        [_h]: ["GET", "/instance/{InstanceId}/bots", 200],
    },
    () => ListBotsRequest,
    () => ListBotsResponse,
];
export var ListContactEvaluations = [
    9,
    n0,
    _LCE,
    {
        [_h]: ["GET", "/contact-evaluations/{InstanceId}", 200],
    },
    () => ListContactEvaluationsRequest,
    () => ListContactEvaluationsResponse,
];
export var ListContactFlowModuleAliases = [
    9,
    n0,
    _LCFMA,
    {
        [_h]: ["GET", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/aliases", 200],
    },
    () => ListContactFlowModuleAliasesRequest,
    () => ListContactFlowModuleAliasesResponse,
];
export var ListContactFlowModules = [
    9,
    n0,
    _LCFM,
    {
        [_h]: ["GET", "/contact-flow-modules-summary/{InstanceId}", 200],
    },
    () => ListContactFlowModulesRequest,
    () => ListContactFlowModulesResponse,
];
export var ListContactFlowModuleVersions = [
    9,
    n0,
    _LCFMV,
    {
        [_h]: ["GET", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/versions", 200],
    },
    () => ListContactFlowModuleVersionsRequest,
    () => ListContactFlowModuleVersionsResponse,
];
export var ListContactFlows = [
    9,
    n0,
    _LCF,
    {
        [_h]: ["GET", "/contact-flows-summary/{InstanceId}", 200],
    },
    () => ListContactFlowsRequest,
    () => ListContactFlowsResponse,
];
export var ListContactFlowVersions = [
    9,
    n0,
    _LCFV,
    {
        [_h]: ["GET", "/contact-flows/{InstanceId}/{ContactFlowId}/versions", 200],
    },
    () => ListContactFlowVersionsRequest,
    () => ListContactFlowVersionsResponse,
];
export var ListContactReferences = [
    9,
    n0,
    _LCR,
    {
        [_h]: ["GET", "/contact/references/{InstanceId}/{ContactId}", 200],
    },
    () => ListContactReferencesRequest,
    () => ListContactReferencesResponse,
];
export var ListDefaultVocabularies = [
    9,
    n0,
    _LDV,
    {
        [_h]: ["POST", "/default-vocabulary-summary/{InstanceId}", 200],
    },
    () => ListDefaultVocabulariesRequest,
    () => ListDefaultVocabulariesResponse,
];
export var ListEvaluationForms = [
    9,
    n0,
    _LEF,
    {
        [_h]: ["GET", "/evaluation-forms/{InstanceId}", 200],
    },
    () => ListEvaluationFormsRequest,
    () => ListEvaluationFormsResponse,
];
export var ListEvaluationFormVersions = [
    9,
    n0,
    _LEFV,
    {
        [_h]: ["GET", "/evaluation-forms/{InstanceId}/{EvaluationFormId}/versions", 200],
    },
    () => ListEvaluationFormVersionsRequest,
    () => ListEvaluationFormVersionsResponse,
];
export var ListFlowAssociations = [
    9,
    n0,
    _LFA,
    {
        [_h]: ["GET", "/flow-associations-summary/{InstanceId}", 200],
    },
    () => ListFlowAssociationsRequest,
    () => ListFlowAssociationsResponse,
];
export var ListHoursOfOperationOverrides = [
    9,
    n0,
    _LHOOO,
    {
        [_h]: ["GET", "/hours-of-operations/{InstanceId}/{HoursOfOperationId}/overrides", 200],
    },
    () => ListHoursOfOperationOverridesRequest,
    () => ListHoursOfOperationOverridesResponse,
];
export var ListHoursOfOperations = [
    9,
    n0,
    _LHOO,
    {
        [_h]: ["GET", "/hours-of-operations-summary/{InstanceId}", 200],
    },
    () => ListHoursOfOperationsRequest,
    () => ListHoursOfOperationsResponse,
];
export var ListInstanceAttributes = [
    9,
    n0,
    _LIA,
    {
        [_h]: ["GET", "/instance/{InstanceId}/attributes", 200],
    },
    () => ListInstanceAttributesRequest,
    () => ListInstanceAttributesResponse,
];
export var ListInstances = [
    9,
    n0,
    _LIi,
    {
        [_h]: ["GET", "/instance", 200],
    },
    () => ListInstancesRequest,
    () => ListInstancesResponse,
];
export var ListInstanceStorageConfigs = [
    9,
    n0,
    _LISC,
    {
        [_h]: ["GET", "/instance/{InstanceId}/storage-configs", 200],
    },
    () => ListInstanceStorageConfigsRequest,
    () => ListInstanceStorageConfigsResponse,
];
export var ListIntegrationAssociations = [
    9,
    n0,
    _LIAi,
    {
        [_h]: ["GET", "/instance/{InstanceId}/integration-associations", 200],
    },
    () => ListIntegrationAssociationsRequest,
    () => ListIntegrationAssociationsResponse,
];
export var ListLambdaFunctions = [
    9,
    n0,
    _LLF,
    {
        [_h]: ["GET", "/instance/{InstanceId}/lambda-functions", 200],
    },
    () => ListLambdaFunctionsRequest,
    () => ListLambdaFunctionsResponse,
];
export var ListLexBots = [
    9,
    n0,
    _LLB,
    {
        [_h]: ["GET", "/instance/{InstanceId}/lex-bots", 200],
    },
    () => ListLexBotsRequest,
    () => ListLexBotsResponse,
];
export var ListPhoneNumbers = [
    9,
    n0,
    _LPN,
    {
        [_h]: ["GET", "/phone-numbers-summary/{InstanceId}", 200],
    },
    () => ListPhoneNumbersRequest,
    () => ListPhoneNumbersResponse,
];
export var ListPhoneNumbersV2 = [
    9,
    n0,
    _LPNV,
    {
        [_h]: ["POST", "/phone-number/list", 200],
    },
    () => ListPhoneNumbersV2Request,
    () => ListPhoneNumbersV2Response,
];
export var ListPredefinedAttributes = [
    9,
    n0,
    _LPA,
    {
        [_h]: ["GET", "/predefined-attributes/{InstanceId}", 200],
    },
    () => ListPredefinedAttributesRequest,
    () => ListPredefinedAttributesResponse,
];
export var ListPrompts = [
    9,
    n0,
    _LP,
    {
        [_h]: ["GET", "/prompts-summary/{InstanceId}", 200],
    },
    () => ListPromptsRequest,
    () => ListPromptsResponse,
];
export var ListQueueQuickConnects = [
    9,
    n0,
    _LQQC,
    {
        [_h]: ["GET", "/queues/{InstanceId}/{QueueId}/quick-connects", 200],
    },
    () => ListQueueQuickConnectsRequest,
    () => ListQueueQuickConnectsResponse,
];
export var ListQueues = [
    9,
    n0,
    _LQ,
    {
        [_h]: ["GET", "/queues-summary/{InstanceId}", 200],
    },
    () => ListQueuesRequest,
    () => ListQueuesResponse,
];
export var ListQuickConnects = [
    9,
    n0,
    _LQC,
    {
        [_h]: ["GET", "/quick-connects/{InstanceId}", 200],
    },
    () => ListQuickConnectsRequest,
    () => ListQuickConnectsResponse,
];
export var ListRealtimeContactAnalysisSegmentsV2 = [
    9,
    n0,
    _LRCASV,
    {
        [_h]: ["POST", "/contact/list-real-time-analysis-segments-v2/{InstanceId}/{ContactId}", 200],
    },
    () => ListRealtimeContactAnalysisSegmentsV2Request,
    () => ListRealtimeContactAnalysisSegmentsV2Response,
];
export var ListRoutingProfileManualAssignmentQueues = [
    9,
    n0,
    _LRPMAQ,
    {
        [_h]: ["GET", "/routing-profiles/{InstanceId}/{RoutingProfileId}/manual-assignment-queues", 200],
    },
    () => ListRoutingProfileManualAssignmentQueuesRequest,
    () => ListRoutingProfileManualAssignmentQueuesResponse,
];
export var ListRoutingProfileQueues = [
    9,
    n0,
    _LRPQ,
    {
        [_h]: ["GET", "/routing-profiles/{InstanceId}/{RoutingProfileId}/queues", 200],
    },
    () => ListRoutingProfileQueuesRequest,
    () => ListRoutingProfileQueuesResponse,
];
export var ListRoutingProfiles = [
    9,
    n0,
    _LRP,
    {
        [_h]: ["GET", "/routing-profiles-summary/{InstanceId}", 200],
    },
    () => ListRoutingProfilesRequest,
    () => ListRoutingProfilesResponse,
];
export var ListRules = [
    9,
    n0,
    _LRi,
    {
        [_h]: ["GET", "/rules/{InstanceId}", 200],
    },
    () => ListRulesRequest,
    () => ListRulesResponse,
];
export var ListSecurityKeys = [
    9,
    n0,
    _LSK,
    {
        [_h]: ["GET", "/instance/{InstanceId}/security-keys", 200],
    },
    () => ListSecurityKeysRequest,
    () => ListSecurityKeysResponse,
];
export var ListSecurityProfileApplications = [
    9,
    n0,
    _LSPA,
    {
        [_h]: ["GET", "/security-profiles-applications/{InstanceId}/{SecurityProfileId}", 200],
    },
    () => ListSecurityProfileApplicationsRequest,
    () => ListSecurityProfileApplicationsResponse,
];
export var ListSecurityProfilePermissions = [
    9,
    n0,
    _LSPP,
    {
        [_h]: ["GET", "/security-profiles-permissions/{InstanceId}/{SecurityProfileId}", 200],
    },
    () => ListSecurityProfilePermissionsRequest,
    () => ListSecurityProfilePermissionsResponse,
];
export var ListSecurityProfiles = [
    9,
    n0,
    _LSP,
    {
        [_h]: ["GET", "/security-profiles-summary/{InstanceId}", 200],
    },
    () => ListSecurityProfilesRequest,
    () => ListSecurityProfilesResponse,
];
export var ListTagsForResource = [
    9,
    n0,
    _LTFR,
    {
        [_h]: ["GET", "/tags/{resourceArn}", 200],
    },
    () => ListTagsForResourceRequest,
    () => ListTagsForResourceResponse,
];
export var ListTaskTemplates = [
    9,
    n0,
    _LTT,
    {
        [_h]: ["GET", "/instance/{InstanceId}/task/template", 200],
    },
    () => ListTaskTemplatesRequest,
    () => ListTaskTemplatesResponse,
];
export var ListTrafficDistributionGroups = [
    9,
    n0,
    _LTDG,
    {
        [_h]: ["GET", "/traffic-distribution-groups", 200],
    },
    () => ListTrafficDistributionGroupsRequest,
    () => ListTrafficDistributionGroupsResponse,
];
export var ListTrafficDistributionGroupUsers = [
    9,
    n0,
    _LTDGU,
    {
        [_h]: ["GET", "/traffic-distribution-group/{TrafficDistributionGroupId}/user", 200],
    },
    () => ListTrafficDistributionGroupUsersRequest,
    () => ListTrafficDistributionGroupUsersResponse,
];
export var ListUseCases = [
    9,
    n0,
    _LUC,
    {
        [_h]: ["GET", "/instance/{InstanceId}/integration-associations/{IntegrationAssociationId}/use-cases", 200],
    },
    () => ListUseCasesRequest,
    () => ListUseCasesResponse,
];
export var ListUserHierarchyGroups = [
    9,
    n0,
    _LUHG,
    {
        [_h]: ["GET", "/user-hierarchy-groups-summary/{InstanceId}", 200],
    },
    () => ListUserHierarchyGroupsRequest,
    () => ListUserHierarchyGroupsResponse,
];
export var ListUserProficiencies = [
    9,
    n0,
    _LUP,
    {
        [_h]: ["GET", "/users/{InstanceId}/{UserId}/proficiencies", 200],
    },
    () => ListUserProficienciesRequest,
    () => ListUserProficienciesResponse,
];
export var ListUsers = [
    9,
    n0,
    _LU,
    {
        [_h]: ["GET", "/users-summary/{InstanceId}", 200],
    },
    () => ListUsersRequest,
    () => ListUsersResponse,
];
export var ListViews = [
    9,
    n0,
    _LVi,
    {
        [_h]: ["GET", "/views/{InstanceId}", 200],
    },
    () => ListViewsRequest,
    () => ListViewsResponse,
];
export var ListViewVersions = [
    9,
    n0,
    _LVV,
    {
        [_h]: ["GET", "/views/{InstanceId}/{ViewId}/versions", 200],
    },
    () => ListViewVersionsRequest,
    () => ListViewVersionsResponse,
];
export var MonitorContact = [
    9,
    n0,
    _MCo,
    {
        [_h]: ["POST", "/contact/monitor", 200],
    },
    () => MonitorContactRequest,
    () => MonitorContactResponse,
];
export var PauseContact = [
    9,
    n0,
    _PCau,
    {
        [_h]: ["POST", "/contact/pause", 200],
    },
    () => PauseContactRequest,
    () => PauseContactResponse,
];
export var PutUserStatus = [
    9,
    n0,
    _PUS,
    {
        [_h]: ["PUT", "/users/{InstanceId}/{UserId}/status", 200],
    },
    () => PutUserStatusRequest,
    () => PutUserStatusResponse,
];
export var ReleasePhoneNumber = [
    9,
    n0,
    _RPN,
    {
        [_h]: ["DELETE", "/phone-number/{PhoneNumberId}", 200],
    },
    () => ReleasePhoneNumberRequest,
    () => __Unit,
];
export var ReplicateInstance = [
    9,
    n0,
    _RIep,
    {
        [_h]: ["POST", "/instance/{InstanceId}/replicate", 200],
    },
    () => ReplicateInstanceRequest,
    () => ReplicateInstanceResponse,
];
export var ResumeContact = [
    9,
    n0,
    _RCes,
    {
        [_h]: ["POST", "/contact/resume", 200],
    },
    () => ResumeContactRequest,
    () => ResumeContactResponse,
];
export var ResumeContactRecording = [
    9,
    n0,
    _RCRes,
    {
        [_h]: ["POST", "/contact/resume-recording", 200],
    },
    () => ResumeContactRecordingRequest,
    () => ResumeContactRecordingResponse,
];
export var SearchAgentStatuses = [
    9,
    n0,
    _SAS,
    {
        [_h]: ["POST", "/search-agent-statuses", 200],
    },
    () => SearchAgentStatusesRequest,
    () => SearchAgentStatusesResponse,
];
export var SearchAvailablePhoneNumbers = [
    9,
    n0,
    _SAPN,
    {
        [_h]: ["POST", "/phone-number/search-available", 200],
    },
    () => SearchAvailablePhoneNumbersRequest,
    () => SearchAvailablePhoneNumbersResponse,
];
export var SearchContactEvaluations = [
    9,
    n0,
    _SCE,
    {
        [_h]: ["POST", "/search-contact-evaluations", 200],
    },
    () => SearchContactEvaluationsRequest,
    () => SearchContactEvaluationsResponse,
];
export var SearchContactFlowModules = [
    9,
    n0,
    _SCFM,
    {
        [_h]: ["POST", "/search-contact-flow-modules", 200],
    },
    () => SearchContactFlowModulesRequest,
    () => SearchContactFlowModulesResponse,
];
export var SearchContactFlows = [
    9,
    n0,
    _SCF,
    {
        [_h]: ["POST", "/search-contact-flows", 200],
    },
    () => SearchContactFlowsRequest,
    () => SearchContactFlowsResponse,
];
export var SearchContacts = [
    9,
    n0,
    _SCea,
    {
        [_h]: ["POST", "/search-contacts", 200],
    },
    () => SearchContactsRequest,
    () => SearchContactsResponse,
];
export var SearchEmailAddresses = [
    9,
    n0,
    _SEAe,
    {
        [_h]: ["POST", "/search-email-addresses", 200],
    },
    () => SearchEmailAddressesRequest,
    () => SearchEmailAddressesResponse,
];
export var SearchEvaluationForms = [
    9,
    n0,
    _SEF,
    {
        [_h]: ["POST", "/search-evaluation-forms", 200],
    },
    () => SearchEvaluationFormsRequest,
    () => SearchEvaluationFormsResponse,
];
export var SearchHoursOfOperationOverrides = [
    9,
    n0,
    _SHOOO,
    {
        [_h]: ["POST", "/search-hours-of-operation-overrides", 200],
    },
    () => SearchHoursOfOperationOverridesRequest,
    () => SearchHoursOfOperationOverridesResponse,
];
export var SearchHoursOfOperations = [
    9,
    n0,
    _SHOO,
    {
        [_h]: ["POST", "/search-hours-of-operations", 200],
    },
    () => SearchHoursOfOperationsRequest,
    () => SearchHoursOfOperationsResponse,
];
export var SearchPredefinedAttributes = [
    9,
    n0,
    _SPAe,
    {
        [_h]: ["POST", "/search-predefined-attributes", 200],
    },
    () => SearchPredefinedAttributesRequest,
    () => SearchPredefinedAttributesResponse,
];
export var SearchPrompts = [
    9,
    n0,
    _SPea,
    {
        [_h]: ["POST", "/search-prompts", 200],
    },
    () => SearchPromptsRequest,
    () => SearchPromptsResponse,
];
export var SearchQueues = [
    9,
    n0,
    _SQ,
    {
        [_h]: ["POST", "/search-queues", 200],
    },
    () => SearchQueuesRequest,
    () => SearchQueuesResponse,
];
export var SearchQuickConnects = [
    9,
    n0,
    _SQC,
    {
        [_h]: ["POST", "/search-quick-connects", 200],
    },
    () => SearchQuickConnectsRequest,
    () => SearchQuickConnectsResponse,
];
export var SearchResourceTags = [
    9,
    n0,
    _SRT,
    {
        [_h]: ["POST", "/search-resource-tags", 200],
    },
    () => SearchResourceTagsRequest,
    () => SearchResourceTagsResponse,
];
export var SearchRoutingProfiles = [
    9,
    n0,
    _SRP,
    {
        [_h]: ["POST", "/search-routing-profiles", 200],
    },
    () => SearchRoutingProfilesRequest,
    () => SearchRoutingProfilesResponse,
];
export var SearchSecurityProfiles = [
    9,
    n0,
    _SSP,
    {
        [_h]: ["POST", "/search-security-profiles", 200],
    },
    () => SearchSecurityProfilesRequest,
    () => SearchSecurityProfilesResponse,
];
export var SearchUserHierarchyGroups = [
    9,
    n0,
    _SUHG,
    {
        [_h]: ["POST", "/search-user-hierarchy-groups", 200],
    },
    () => SearchUserHierarchyGroupsRequest,
    () => SearchUserHierarchyGroupsResponse,
];
export var SearchUsers = [
    9,
    n0,
    _SUe,
    {
        [_h]: ["POST", "/search-users", 200],
    },
    () => SearchUsersRequest,
    () => SearchUsersResponse,
];
export var SearchVocabularies = [
    9,
    n0,
    _SVe,
    {
        [_h]: ["POST", "/vocabulary-summary/{InstanceId}", 200],
    },
    () => SearchVocabulariesRequest,
    () => SearchVocabulariesResponse,
];
export var SendChatIntegrationEvent = [
    9,
    n0,
    _SCIE,
    {
        [_h]: ["POST", "/chat-integration-event", 200],
    },
    () => SendChatIntegrationEventRequest,
    () => SendChatIntegrationEventResponse,
];
export var SendOutboundEmail = [
    9,
    n0,
    _SOE,
    {
        [_h]: ["PUT", "/instance/{InstanceId}/outbound-email", 200],
    },
    () => SendOutboundEmailRequest,
    () => SendOutboundEmailResponse,
];
export var StartAttachedFileUpload = [
    9,
    n0,
    _SAFU,
    {
        [_h]: ["PUT", "/attached-files/{InstanceId}", 200],
    },
    () => StartAttachedFileUploadRequest,
    () => StartAttachedFileUploadResponse,
];
export var StartChatContact = [
    9,
    n0,
    _SCC,
    {
        [_h]: ["PUT", "/contact/chat", 200],
    },
    () => StartChatContactRequest,
    () => StartChatContactResponse,
];
export var StartContactEvaluation = [
    9,
    n0,
    _SCEt,
    {
        [_h]: ["PUT", "/contact-evaluations/{InstanceId}", 200],
    },
    () => StartContactEvaluationRequest,
    () => StartContactEvaluationResponse,
];
export var StartContactRecording = [
    9,
    n0,
    _SCRta,
    {
        [_h]: ["POST", "/contact/start-recording", 200],
    },
    () => StartContactRecordingRequest,
    () => StartContactRecordingResponse,
];
export var StartContactStreaming = [
    9,
    n0,
    _SCS,
    {
        [_h]: ["POST", "/contact/start-streaming", 200],
    },
    () => StartContactStreamingRequest,
    () => StartContactStreamingResponse,
];
export var StartEmailContact = [
    9,
    n0,
    _SEC,
    {
        [_h]: ["PUT", "/contact/email", 200],
    },
    () => StartEmailContactRequest,
    () => StartEmailContactResponse,
];
export var StartOutboundChatContact = [
    9,
    n0,
    _SOCC,
    {
        [_h]: ["PUT", "/contact/outbound-chat", 200],
    },
    () => StartOutboundChatContactRequest,
    () => StartOutboundChatContactResponse,
];
export var StartOutboundEmailContact = [
    9,
    n0,
    _SOEC,
    {
        [_h]: ["PUT", "/contact/outbound-email", 200],
    },
    () => StartOutboundEmailContactRequest,
    () => StartOutboundEmailContactResponse,
];
export var StartOutboundVoiceContact = [
    9,
    n0,
    _SOVC,
    {
        [_h]: ["PUT", "/contact/outbound-voice", 200],
    },
    () => StartOutboundVoiceContactRequest,
    () => StartOutboundVoiceContactResponse,
];
export var StartScreenSharing = [
    9,
    n0,
    _SSS,
    {
        [_h]: ["PUT", "/contact/screen-sharing", 200],
    },
    () => StartScreenSharingRequest,
    () => StartScreenSharingResponse,
];
export var StartTaskContact = [
    9,
    n0,
    _STC,
    {
        [_h]: ["PUT", "/contact/task", 200],
    },
    () => StartTaskContactRequest,
    () => StartTaskContactResponse,
];
export var StartWebRTCContact = [
    9,
    n0,
    _SWRTCC,
    {
        [_h]: ["PUT", "/contact/webrtc", 200],
    },
    () => StartWebRTCContactRequest,
    () => StartWebRTCContactResponse,
];
export var StopContact = [
    9,
    n0,
    _SCtop,
    {
        [_h]: ["POST", "/contact/stop", 200],
    },
    () => StopContactRequest,
    () => StopContactResponse,
];
export var StopContactRecording = [
    9,
    n0,
    _SCRtop,
    {
        [_h]: ["POST", "/contact/stop-recording", 200],
    },
    () => StopContactRecordingRequest,
    () => StopContactRecordingResponse,
];
export var StopContactStreaming = [
    9,
    n0,
    _SCSt,
    {
        [_h]: ["POST", "/contact/stop-streaming", 200],
    },
    () => StopContactStreamingRequest,
    () => StopContactStreamingResponse,
];
export var SubmitContactEvaluation = [
    9,
    n0,
    _SCEu,
    {
        [_h]: ["POST", "/contact-evaluations/{InstanceId}/{EvaluationId}/submit", 200],
    },
    () => SubmitContactEvaluationRequest,
    () => SubmitContactEvaluationResponse,
];
export var SuspendContactRecording = [
    9,
    n0,
    _SCRu,
    {
        [_h]: ["POST", "/contact/suspend-recording", 200],
    },
    () => SuspendContactRecordingRequest,
    () => SuspendContactRecordingResponse,
];
export var TagContact = [
    9,
    n0,
    _TCag,
    {
        [_h]: ["POST", "/contact/tags", 200],
    },
    () => TagContactRequest,
    () => TagContactResponse,
];
export var TagResource = [
    9,
    n0,
    _TRa,
    {
        [_h]: ["POST", "/tags/{resourceArn}", 200],
    },
    () => TagResourceRequest,
    () => __Unit,
];
export var TransferContact = [
    9,
    n0,
    _TCra,
    {
        [_h]: ["POST", "/contact/transfer", 200],
    },
    () => TransferContactRequest,
    () => TransferContactResponse,
];
export var UntagContact = [
    9,
    n0,
    _UCn,
    {
        [_h]: ["DELETE", "/contact/tags/{InstanceId}/{ContactId}", 200],
    },
    () => UntagContactRequest,
    () => UntagContactResponse,
];
export var UntagResource = [
    9,
    n0,
    _URn,
    {
        [_h]: ["DELETE", "/tags/{resourceArn}", 200],
    },
    () => UntagResourceRequest,
    () => __Unit,
];
export var UpdateAgentStatus = [
    9,
    n0,
    _UAS,
    {
        [_h]: ["POST", "/agent-status/{InstanceId}/{AgentStatusId}", 200],
    },
    () => UpdateAgentStatusRequest,
    () => __Unit,
];
export var UpdateAuthenticationProfile = [
    9,
    n0,
    _UAP,
    {
        [_h]: ["POST", "/authentication-profiles/{InstanceId}/{AuthenticationProfileId}", 200],
    },
    () => UpdateAuthenticationProfileRequest,
    () => __Unit,
];
export var UpdateContact = [
    9,
    n0,
    _UCp,
    {
        [_h]: ["POST", "/contacts/{InstanceId}/{ContactId}", 200],
    },
    () => UpdateContactRequest,
    () => UpdateContactResponse,
];
export var UpdateContactAttributes = [
    9,
    n0,
    _UCApd,
    {
        [_h]: ["POST", "/contact/attributes", 200],
    },
    () => UpdateContactAttributesRequest,
    () => UpdateContactAttributesResponse,
];
export var UpdateContactEvaluation = [
    9,
    n0,
    _UCE,
    {
        [_h]: ["POST", "/contact-evaluations/{InstanceId}/{EvaluationId}", 200],
    },
    () => UpdateContactEvaluationRequest,
    () => UpdateContactEvaluationResponse,
];
export var UpdateContactFlowContent = [
    9,
    n0,
    _UCFC,
    {
        [_h]: ["POST", "/contact-flows/{InstanceId}/{ContactFlowId}/content", 200],
    },
    () => UpdateContactFlowContentRequest,
    () => UpdateContactFlowContentResponse,
];
export var UpdateContactFlowMetadata = [
    9,
    n0,
    _UCFM,
    {
        [_h]: ["POST", "/contact-flows/{InstanceId}/{ContactFlowId}/metadata", 200],
    },
    () => UpdateContactFlowMetadataRequest,
    () => UpdateContactFlowMetadataResponse,
];
export var UpdateContactFlowModuleAlias = [
    9,
    n0,
    _UCFMA,
    {
        [_h]: ["POST", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/alias/{AliasId}", 200],
    },
    () => UpdateContactFlowModuleAliasRequest,
    () => UpdateContactFlowModuleAliasResponse,
];
export var UpdateContactFlowModuleContent = [
    9,
    n0,
    _UCFMC,
    {
        [_h]: ["POST", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/content", 200],
    },
    () => UpdateContactFlowModuleContentRequest,
    () => UpdateContactFlowModuleContentResponse,
];
export var UpdateContactFlowModuleMetadata = [
    9,
    n0,
    _UCFMM,
    {
        [_h]: ["POST", "/contact-flow-modules/{InstanceId}/{ContactFlowModuleId}/metadata", 200],
    },
    () => UpdateContactFlowModuleMetadataRequest,
    () => UpdateContactFlowModuleMetadataResponse,
];
export var UpdateContactFlowName = [
    9,
    n0,
    _UCFN,
    {
        [_h]: ["POST", "/contact-flows/{InstanceId}/{ContactFlowId}/name", 200],
    },
    () => UpdateContactFlowNameRequest,
    () => UpdateContactFlowNameResponse,
];
export var UpdateContactRoutingData = [
    9,
    n0,
    _UCRD,
    {
        [_h]: ["POST", "/contacts/{InstanceId}/{ContactId}/routing-data", 200],
    },
    () => UpdateContactRoutingDataRequest,
    () => UpdateContactRoutingDataResponse,
];
export var UpdateContactSchedule = [
    9,
    n0,
    _UCS,
    {
        [_h]: ["POST", "/contact/schedule", 200],
    },
    () => UpdateContactScheduleRequest,
    () => UpdateContactScheduleResponse,
];
export var UpdateEmailAddressMetadata = [
    9,
    n0,
    _UEAM,
    {
        [_h]: ["POST", "/email-addresses/{InstanceId}/{EmailAddressId}", 200],
    },
    () => UpdateEmailAddressMetadataRequest,
    () => UpdateEmailAddressMetadataResponse,
];
export var UpdateEvaluationForm = [
    9,
    n0,
    _UEF,
    {
        [_h]: ["PUT", "/evaluation-forms/{InstanceId}/{EvaluationFormId}", 200],
    },
    () => UpdateEvaluationFormRequest,
    () => UpdateEvaluationFormResponse,
];
export var UpdateHoursOfOperation = [
    9,
    n0,
    _UHOO,
    {
        [_h]: ["POST", "/hours-of-operations/{InstanceId}/{HoursOfOperationId}", 200],
    },
    () => UpdateHoursOfOperationRequest,
    () => __Unit,
];
export var UpdateHoursOfOperationOverride = [
    9,
    n0,
    _UHOOO,
    {
        [_h]: [
            "POST",
            "/hours-of-operations/{InstanceId}/{HoursOfOperationId}/overrides/{HoursOfOperationOverrideId}",
            200,
        ],
    },
    () => UpdateHoursOfOperationOverrideRequest,
    () => __Unit,
];
export var UpdateInstanceAttribute = [
    9,
    n0,
    _UIA,
    {
        [_h]: ["POST", "/instance/{InstanceId}/attribute/{AttributeType}", 200],
    },
    () => UpdateInstanceAttributeRequest,
    () => __Unit,
];
export var UpdateInstanceStorageConfig = [
    9,
    n0,
    _UISC,
    {
        [_h]: ["POST", "/instance/{InstanceId}/storage-config/{AssociationId}", 200],
    },
    () => UpdateInstanceStorageConfigRequest,
    () => __Unit,
];
export var UpdateParticipantAuthentication = [
    9,
    n0,
    _UPA,
    {
        [_h]: ["POST", "/contact/update-participant-authentication", 200],
    },
    () => UpdateParticipantAuthenticationRequest,
    () => UpdateParticipantAuthenticationResponse,
];
export var UpdateParticipantRoleConfig = [
    9,
    n0,
    _UPRC,
    {
        [_h]: ["PUT", "/contact/participant-role-config/{InstanceId}/{ContactId}", 200],
    },
    () => UpdateParticipantRoleConfigRequest,
    () => UpdateParticipantRoleConfigResponse,
];
export var UpdatePhoneNumber = [
    9,
    n0,
    _UPN,
    {
        [_h]: ["PUT", "/phone-number/{PhoneNumberId}", 200],
    },
    () => UpdatePhoneNumberRequest,
    () => UpdatePhoneNumberResponse,
];
export var UpdatePhoneNumberMetadata = [
    9,
    n0,
    _UPNM,
    {
        [_h]: ["PUT", "/phone-number/{PhoneNumberId}/metadata", 200],
    },
    () => UpdatePhoneNumberMetadataRequest,
    () => __Unit,
];
export var UpdatePredefinedAttribute = [
    9,
    n0,
    _UPAp,
    {
        [_h]: ["POST", "/predefined-attributes/{InstanceId}/{Name}", 200],
    },
    () => UpdatePredefinedAttributeRequest,
    () => __Unit,
];
export var UpdatePrompt = [
    9,
    n0,
    _UPp,
    {
        [_h]: ["POST", "/prompts/{InstanceId}/{PromptId}", 200],
    },
    () => UpdatePromptRequest,
    () => UpdatePromptResponse,
];
export var UpdateQueueHoursOfOperation = [
    9,
    n0,
    _UQHOO,
    {
        [_h]: ["POST", "/queues/{InstanceId}/{QueueId}/hours-of-operation", 200],
    },
    () => UpdateQueueHoursOfOperationRequest,
    () => __Unit,
];
export var UpdateQueueMaxContacts = [
    9,
    n0,
    _UQMC,
    {
        [_h]: ["POST", "/queues/{InstanceId}/{QueueId}/max-contacts", 200],
    },
    () => UpdateQueueMaxContactsRequest,
    () => __Unit,
];
export var UpdateQueueName = [
    9,
    n0,
    _UQN,
    {
        [_h]: ["POST", "/queues/{InstanceId}/{QueueId}/name", 200],
    },
    () => UpdateQueueNameRequest,
    () => __Unit,
];
export var UpdateQueueOutboundCallerConfig = [
    9,
    n0,
    _UQOCC,
    {
        [_h]: ["POST", "/queues/{InstanceId}/{QueueId}/outbound-caller-config", 200],
    },
    () => UpdateQueueOutboundCallerConfigRequest,
    () => __Unit,
];
export var UpdateQueueOutboundEmailConfig = [
    9,
    n0,
    _UQOEC,
    {
        [_h]: ["POST", "/queues/{InstanceId}/{QueueId}/outbound-email-config", 200],
    },
    () => UpdateQueueOutboundEmailConfigRequest,
    () => __Unit,
];
export var UpdateQueueStatus = [
    9,
    n0,
    _UQS,
    {
        [_h]: ["POST", "/queues/{InstanceId}/{QueueId}/status", 200],
    },
    () => UpdateQueueStatusRequest,
    () => __Unit,
];
export var UpdateQuickConnectConfig = [
    9,
    n0,
    _UQCCp,
    {
        [_h]: ["POST", "/quick-connects/{InstanceId}/{QuickConnectId}/config", 200],
    },
    () => UpdateQuickConnectConfigRequest,
    () => __Unit,
];
export var UpdateQuickConnectName = [
    9,
    n0,
    _UQCN,
    {
        [_h]: ["POST", "/quick-connects/{InstanceId}/{QuickConnectId}/name", 200],
    },
    () => UpdateQuickConnectNameRequest,
    () => __Unit,
];
export var UpdateRoutingProfileAgentAvailabilityTimer = [
    9,
    n0,
    _URPAAT,
    {
        [_h]: ["POST", "/routing-profiles/{InstanceId}/{RoutingProfileId}/agent-availability-timer", 200],
    },
    () => UpdateRoutingProfileAgentAvailabilityTimerRequest,
    () => __Unit,
];
export var UpdateRoutingProfileConcurrency = [
    9,
    n0,
    _URPC,
    {
        [_h]: ["POST", "/routing-profiles/{InstanceId}/{RoutingProfileId}/concurrency", 200],
    },
    () => UpdateRoutingProfileConcurrencyRequest,
    () => __Unit,
];
export var UpdateRoutingProfileDefaultOutboundQueue = [
    9,
    n0,
    _URPDOQ,
    {
        [_h]: ["POST", "/routing-profiles/{InstanceId}/{RoutingProfileId}/default-outbound-queue", 200],
    },
    () => UpdateRoutingProfileDefaultOutboundQueueRequest,
    () => __Unit,
];
export var UpdateRoutingProfileName = [
    9,
    n0,
    _URPN,
    {
        [_h]: ["POST", "/routing-profiles/{InstanceId}/{RoutingProfileId}/name", 200],
    },
    () => UpdateRoutingProfileNameRequest,
    () => __Unit,
];
export var UpdateRoutingProfileQueues = [
    9,
    n0,
    _URPQ,
    {
        [_h]: ["POST", "/routing-profiles/{InstanceId}/{RoutingProfileId}/queues", 200],
    },
    () => UpdateRoutingProfileQueuesRequest,
    () => __Unit,
];
export var UpdateRule = [
    9,
    n0,
    _URp,
    {
        [_h]: ["PUT", "/rules/{InstanceId}/{RuleId}", 200],
    },
    () => UpdateRuleRequest,
    () => __Unit,
];
export var UpdateSecurityProfile = [
    9,
    n0,
    _USP,
    {
        [_h]: ["POST", "/security-profiles/{InstanceId}/{SecurityProfileId}", 200],
    },
    () => UpdateSecurityProfileRequest,
    () => __Unit,
];
export var UpdateTaskTemplate = [
    9,
    n0,
    _UTT,
    {
        [_h]: ["POST", "/instance/{InstanceId}/task/template/{TaskTemplateId}", 200],
    },
    () => UpdateTaskTemplateRequest,
    () => UpdateTaskTemplateResponse,
];
export var UpdateTrafficDistribution = [
    9,
    n0,
    _UTD,
    {
        [_h]: ["PUT", "/traffic-distribution/{Id}", 200],
    },
    () => UpdateTrafficDistributionRequest,
    () => UpdateTrafficDistributionResponse,
];
export var UpdateUserHierarchy = [
    9,
    n0,
    _UUH,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/hierarchy", 200],
    },
    () => UpdateUserHierarchyRequest,
    () => __Unit,
];
export var UpdateUserHierarchyGroupName = [
    9,
    n0,
    _UUHGN,
    {
        [_h]: ["POST", "/user-hierarchy-groups/{InstanceId}/{HierarchyGroupId}/name", 200],
    },
    () => UpdateUserHierarchyGroupNameRequest,
    () => __Unit,
];
export var UpdateUserHierarchyStructure = [
    9,
    n0,
    _UUHS,
    {
        [_h]: ["POST", "/user-hierarchy-structure/{InstanceId}", 200],
    },
    () => UpdateUserHierarchyStructureRequest,
    () => __Unit,
];
export var UpdateUserIdentityInfo = [
    9,
    n0,
    _UUII,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/identity-info", 200],
    },
    () => UpdateUserIdentityInfoRequest,
    () => __Unit,
];
export var UpdateUserPhoneConfig = [
    9,
    n0,
    _UUPC,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/phone-config", 200],
    },
    () => UpdateUserPhoneConfigRequest,
    () => __Unit,
];
export var UpdateUserProficiencies = [
    9,
    n0,
    _UUP,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/proficiencies", 200],
    },
    () => UpdateUserProficienciesRequest,
    () => __Unit,
];
export var UpdateUserRoutingProfile = [
    9,
    n0,
    _UURP,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/routing-profile", 200],
    },
    () => UpdateUserRoutingProfileRequest,
    () => __Unit,
];
export var UpdateUserSecurityProfiles = [
    9,
    n0,
    _UUSP,
    {
        [_h]: ["POST", "/users/{InstanceId}/{UserId}/security-profiles", 200],
    },
    () => UpdateUserSecurityProfilesRequest,
    () => __Unit,
];
export var UpdateViewContent = [
    9,
    n0,
    _UVC,
    {
        [_h]: ["POST", "/views/{InstanceId}/{ViewId}", 200],
    },
    () => UpdateViewContentRequest,
    () => UpdateViewContentResponse,
];
export var UpdateViewMetadata = [
    9,
    n0,
    _UVM,
    {
        [_h]: ["POST", "/views/{InstanceId}/{ViewId}/metadata", 200],
    },
    () => UpdateViewMetadataRequest,
    () => UpdateViewMetadataResponse,
];
